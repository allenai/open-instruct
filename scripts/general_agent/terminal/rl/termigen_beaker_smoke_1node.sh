...
# Beaker smoke (stage 1 of terminal-MOPD): REGULAR terminal RL on ONE released
# terminal dataset. Default allenai/open-instruct-termigen; run the
# endless-terminals twin with:
#   DATASET=allenai/open-instruct-endless-terminals EXP_NAME=endless_beaker_smoke ./...
# tmax-2b student, 1 node / 8 GPUs (4 learners + 4 engines), ~16 steps.
# Task images come from the repo's task-data.tar.gz via task_data_hf_repo;
# MIRROR_URL host must serve hamishi740/termigen + hamishivi/swerl-endless-terminals
# (verified on aus-106 2026-08-13).

BEAKER_IMAGE="${1:?Usage: $0 <beaker-image>}"

DATASET=${DATASET:-allenai/open-instruct-termigen}
EXP_NAME=${EXP_NAME:-termigen_beaker_smoke_tmax2b}

uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "Terminal-MOPD stage-1 smoke: tmax-2b GRPO on $DATASET (1 node)" \
       --pure_docker_mode \
    ...
    --tool_configs "{\"task_data_hf_repo\": \"$DATASET\", \"test_timeout\": 120, \"image\": \"python:3.12-slim\"}" \
