...
| Native MoE checkpoint writer | Opt-in `core.checkpoint_profile` records construction, planning, writing, metadata and optimizer reload timings. `core.checkpoint_constant_memory_planning`, `core.checkpoint_compact_storage`, `core.checkpoint_dedup_save_to_lowest_rank`, `core.checkpoint_thread_count` and `core.checkpoint_process_count` select the candidate save policy. Defaults preserve existing behavior; the separate dense trainer rejects overrides. See the [qualification record](measurements/miles-checkpoint-perf-20260911.md) before enabling candidates. |
