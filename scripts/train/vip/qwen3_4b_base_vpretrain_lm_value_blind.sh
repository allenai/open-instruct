#!/bin/bash
# Qwen3-4B-Base value pretrain only (1000 steps) with LM value model (P(Yes/No) as value)
# Blind conditioning: yes/no question only (no answer, no sibling rollouts)
# 1 node (8 GPUs): 4 learner + 4 vLLM engines, 32 prompts
DDMM=$(date +"%d%m")
exp_name=vip_vpretrain_lm_value_blind_${DDMM}_qwen3_4b_math
...
uv run python mason.py \
    ...
    --gt_conditioning_template lm_yesno_blind \
