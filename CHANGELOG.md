...
### Added
- Support SFT of `allenai/Olmo-Hybrid-7B` on the olmo-core path: an `olmo3_hybrid_7B` config and HF <-> olmo-core state conversion for `model_type: olmo_hybrid`, neither of which olmo-core provides, plus a GPU check that a config reproduces its HF reference's logits (https://github.com/allenai/open-instruct/pull/1822).
...
### Fixed
- Save HF checkpoints with `save_original_format=False` so weights are written under the modern names; `save_pretrained` otherwise applies transformers' `conversion_mapping` in reverse and produces checkpoints vLLM refuses to load. Verify after writing that every converted tensor reached the file (https://github.com/allenai/open-instruct/pull/1820).
