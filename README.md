...
Our setup mostly follows our [Dockerfile](./Dockerfile). *Note that Open Instruct is a research codebase and does not guarantee backward compatibility.*

### Installation

We recommend using [uv](https://docs.astral.sh/uv/) for installation. You can install with `uv sync`.

* **Docker installation**: You can use the Dockerfile to build a Docker image. You can build the image with the following command:
