...
from typing import Any, Iterable, List, NewType, Optional, Sequence, Tuple, Union
...
# Conventions for FLOP calculations (fixed; not switches)
FLOP_PER_MAC = 2
...
def get_device_name(device_name: str) -> str:
    """Get standardized device name from various GPU naming formats."""
...
def load_model_dims(model_name: str) -> ModelDims:
    ...
    if model_name.startswith("gs://"):
        ...
        result = subprocess.run(
            ["gsutil", "cp", config_path, "/tmp/config.json"], capture_output=True, text=True, check=True
        )
        if result.returncode != 0:
            raise ValueError(f"Failed to download config.json from {config_path}")
