...
def distill_pretokenized_v1(row: dict[str, Any], tokenizer: PreTrainedTokenizer, max_seq_length: int):
    """Transform pre-tokenized distillation rows with compressed teacher logits."""
    input_ids = row[INPUT_IDS_KEY]
    if len(input_ids) > max_seq_length:
        raise ValueError(f"Sequence length {len(input_ids)} exceeds max_seq_length {max_seq_length}.")
    ...
    if isinstance(messages_raw, str) and messages_raw:
        try:
            loaded = json.loads(messages_raw)
