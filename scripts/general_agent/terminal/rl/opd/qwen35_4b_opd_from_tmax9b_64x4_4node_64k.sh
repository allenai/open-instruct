...
# OPD shape test: 64 unique prompts x 4 samples (Thinking Machines' recipe).
#
# Middle point of the shape sweep on the SAME student<-teacher pair:
#   8 x 32  (baseline)  best TB2.1 0.202, KL floor ~0.095, eval band +-0.04
#   64 x 4  (this run)  8x the baseline's per-step prompt diversity
#   256 x 1 (running)   max diversity, but measured 313 stale-drops/step and
#                       ~25% sandbox reset timeouts from 256 unique task images
#                       per step -- wasted rollouts AND a fast-prompt selection
#                       bias (only quick rollouts survive the age cutoff), which
#                       contaminates the diversity measurement it was meant to make.
# 64 x 4 keeps most of the diversity while cutting image churn and staleness ~4x,
# and (unlike n=1) leaves a usable group baseline so this shape can later be
# combined with the RLVR reward term.
#
# Compare: KL trajectory/floor, eval oscillation band at lr 1e-6, and
# stale_results_dropped / reset-timeout rates vs both other shapes.
#
...
EXP_NAME=swerl_qwen35_4b_opd_from_tmax9b_64x4_4node_64k
...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "OPD shape: 64x4 rollouts (TM recipe) base Qwen3.5-4B <- tmax-9b, pure distill; vs the 8x32 and 256x1 arms; 4-node; 64k${RESUME_NOTE}" \
       --pure_docker_mode \
       --workspace ai2/general-tool-use \
       --priority urgent \
       --preemptible \
       ...
       --env MIRROR_URL=jupiter-cs-aus-207.reviz.ai2.in:5000 \
