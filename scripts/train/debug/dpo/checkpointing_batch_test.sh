...
BATCH_SIZE=${2:-2}
CHECKPOINTING_MODE=${3:-full}
BLOCK_INTERVAL=${4:-2}

MODEL_NAME=/weka/oe-adapt-default/scottg/olmo/merging/ckpts/olmo3-7b-instruct-sft-1115
EXP_NAME=olmo3-7b-DPO-checkpointing-test-${CHECKPOINTING_MODE}-bs${BATCH_SIZE}

uv run python mason.py \
    --cluster ai2/jupiter \
    --description "DPO checkpointing=${CHECKPOINTING_MODE}, batch_size=${BATCH_SIZE}, 20 steps" \
    ...
    --gradient_checkpointing_mode $CHECKPOINTING_MODE \
    --gradient_checkpointing_block_interval $BLOCK_INTERVAL \
    --with_tracking
