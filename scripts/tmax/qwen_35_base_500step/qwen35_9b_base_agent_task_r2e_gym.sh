...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "SWERL agent-task-r2e-gym GRPO with Qwen3.5-9B" \
       --pure_docker_mode \
    ...
    --dataset_mixer_list hamishivi/agent-task-r2e-gym 1.0 \
    ...
    --save_traces \
    --tools swerl_sandbox \
    --tool_configs '{"task_data_hf_repo": "hamishivi/agent-task-r2e-gym", "test_timeout": 120, "image": "python:3.12-slim"}' \
    --pool_size 512 \
    --max_steps 100 \
