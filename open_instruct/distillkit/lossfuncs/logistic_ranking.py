...
def sparse_logistic_ranking_loss(
    ...
    teacher_probs = torch.exp(teacher_target_values) if log_target else teacher_target_values
    student_logits_at_targets = torch.gather(student_logits, dim=2, index=teacher_target_ids)

    actually_supported_mask = teacher_probs > eps
    teacher_prefers_k_over_l_mask = teacher_probs.unsqueeze(-1) > (teacher_probs.unsqueeze(-2) + eps)
    student_logit_diff_k_minus_l = student_logits_at_targets.unsqueeze(-1) - student_logits_at_targets.unsqueeze(-2)
    ...
    if sequence_mask is None:
        active_sequence_mask = torch.ones_like(sum_pair_loss_per_pos, dtype=torch.bool, device=student_logits.device)
    else:
    ...
    num_contributing_pairs = (
        valid_preference_pair_mask.float() * active_sequence_mask.float().unsqueeze(-1).unsqueeze(-1)
    ).sum()
