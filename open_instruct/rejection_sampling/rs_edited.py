...
from open_instruct.rejection_sampling.generation_edited import generate_with_anthropic, generate_with_gemini
...
def process_shard_api(model_name_or_path: str, args: Args, shard: List[str]) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Processes a shard (subset) of data using API-based models.
    ...
    # Determine the appropriate function to call based on the model
    if model_name_or_path in OPENAI_MODEL_LIST:
    ...
    elif model_name_or_path in ANTHROPIC_MODEL_LIST:
        model_responses_scores = asyncio.run(
            generate_with_anthropic(model_name_or_path, data_list_model_responses, args, gen_args)
        )
        reference_responses_scores = asyncio.run(
            generate_with_anthropic(model_name_or_path, data_list_reference_responses, args, gen_args)
        )

    elif model_name_or_path in GEMINI_MODEL_LIST:
        # prompt_token_ids_model = [
        #     {"prompt_token_ids": tokenizer.apply_chat_template(x["messages"][:-1])}
        #     for x in data_list_model_responses
        # ]
        # prompt_token_ids_ref = [
        #     {"prompt_token_ids": tokenizer.apply_chat_template(x["messages"][:-1])}
        #     for x in data_list_reference_responses
        # ]
        model_responses_scores = asyncio.run(
            generate_with_gemini(model_name_or_path, data_list_model_responses, gen_args)
        )
        reference_responses_scores = asyncio.run(
            generate_with_gemini(model_name_or_path, data_list_reference_responses, gen_args)
        )
