...
def save_config(args, tokenizer_config, model_config, timestamp: int):
    ...
    config_dict = {'args': dataclasses.asdict(args), 'tokenizer_config': dataclasses.asdict(tokenizer_config), 'model_config': dataclasses.asdict(model_config), 'timestamp': timestamp}
...
def setup_dataset(args: grpo_fast.Args, tokenizer_config: dataset_transformation.TokenizerConfig) -> datasets.Dataset:
    ...
    dataset = dataset_transformation.get_cached_dataset_tulu(dataset_mixer_list=args.dataset_mixer_list, dataset_mixer_list_splits=args.dataset_mixer_list_splits, tc=tokenizer_config, dataset_transform_fn=args.dataset_transform_fn, transform_fn_args=transform_fn_args, dataset_cache_mode=args.dataset_cache_mode, dataset_local_cache_dir=args.dataset_local_cache_dir, dataset_skip_cache=args.dataset_skip_cache)
    dataset = dataset.shuffle(seed=args.seed)
    logger.info(f'Dataset loaded with {len(dataset)} samples')
    return dataset

def setup_vllm_engines(args: grpo_fast.Args, tokenizer_config: dataset_transformation.TokenizerConfig, model_config: model_utils.ModelConfig, max_model_len: int=20480) -> tuple[list[ray.actor.ActorHandle], ray_queue.Queue, ray_queue.Queue, ray.actor.ActorHandle]:
    ...
    queues_to_monitor = {'Param Prompt Queue': param_prompt_Q, 'Inference Results Queue': inference_results_Q, 'Evaluation Results Queue': evaluation_results_Q}
    ...
    vllm_engines = vllm_utils.create_vllm_engines(num_engines=args.vllm_num_engines, tensor_parallel_size=args.vllm_tensor_parallel_size, enforce_eager=args.vllm_enforce_eager, tokenizer_name_or_path=tokenizer_name_or_path, pretrain=model_config.model_name_or_path, revision=model_config.model_revision, seed=args.seed, enable_prefix_caching=args.vllm_enable_prefix_caching, max_model_len=max_model_len, vllm_gpu_memory_utilization=args.vllm_gpu_memory_utilization, single_gpu_mode=args.single_gpu_mode, pg=pg if args.single_gpu_mode else None, tools={}, max_tool_calls=args.max_tool_calls, prompt_queue=param_prompt_Q, results_queue=inference_results_Q, eval_results_queue=evaluation_results_Q, actor_manager=actor_manager, inference_batch_size=args.inference_batch_size, use_fp8_kv_cache=args.use_fp8_kv_cache, inflight_updates=args.inflight_updates)
...
def simulate_weight_sync(actor_manager: ray.actor.ActorHandle, vllm_engines: list[ray.actor.ActorHandle], args: grpo_fast.Args) -> float:
...
def submission_thread(param_prompt_Q: ray_queue.Queue, dataset: datasets.Dataset, generation_config: vllm.SamplingParams, stop_event: threading.Event, batch_size: int, start_batch_idx: int, num_batches: int) -> None:
...
def run_benchmark(dataset: datasets.Dataset, vllm_engines: list[ray.actor.ActorHandle], param_prompt_Q: ray_queue.Queue, inference_results_Q: ray_queue.Queue, actor_manager: ray.actor.ActorHandle, args: grpo_fast.Args, model_config: model_utils.ModelConfig, timestamp: int, num_batches: int=5) -> list[dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f'Warmup batch completed with {total_warmup_responses} total responses from {len(warmup_results)} prompts')
        logger.info(f'Submitting {num_batches - 1} batches for main benchmark...')
        submission_future = executor.submit(submission_thread, param_prompt_Q, dataset, generation_config, stop_event, args.num_unique_prompts_rollout, 1, num_batches - 1)
        for batch_idx in range(1, num_batches):
            ...
            result_dict = {'tokens_per_second': tokens_per_second, 'generation_time': batch_generation_time, 'weight_sync_time': weight_sync_time, 'num_new_tokens': total_new_tokens, 'finish_reasons': collections.Counter(all_finish_reasons), 'response_lengths': all_response_lengths, 'dataset_indices': all_dataset_indices}
            model_flops = model_dims.flops(all_prompt_lengths, all_response_lengths, samples_per_prompt=args.num_samples_per_prompt_rollout)
            ...
            model_memory_bytes = model_dims.memory_bytes(all_prompt_lengths, all_response_lengths, samples_per_prompt=args.num_samples_per_prompt_rollout)
            ...
            logger.info(f'Batch {batch_idx}/{num_batches - 1}: {result_dict['tokens_per_second']:.2f} new tokens/sec, MFU: {result_dict['mfu']:.2f}%, MBU: {result_dict['mbu']:.2f}%, generation time: {batch_generation_time:.2f}s, weight sync time: {weight_sync_time:.2f}s, total new tokens: {total_new_tokens}')
...
def aggregate_results(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    aggregated_results = {'total_mfu': 0.0, 'total_mbu': 0.0, 'total_tokens_per_second': 0.0, 'total_generation_time': 0.0, 'total_weight_sync_time': 0.0, 'total_num_new_tokens': 0, 'finish_reasons': collections.defaultdict(int), 'response_lengths': [], 'prompt_lengths': []}
    ...
    aggregated_results['avg_tokens_per_second'] = aggregated_results['total_num_new_tokens'] / aggregated_results['total_generation_time'] if aggregated_results['total_generation_time'] > 0 else 0
...
def print_summary(results: list[dict[str, Any]], total_time: float, args: grpo_fast.Args, model_config: model_utils.ModelConfig, model_dims: utils.ModelDims) -> None:
    ...
    print(f'GPU peak FLOPs: {model_dims.device_flops / 1000000000000.0:.0f} TFLOPs')
    gpu_specs = utils.GPU_SPECS[model_dims.device_name]
    print(f'GPU memory size: {gpu_specs['memory_size'] / 1000000000.0:.0f} GB')
    print(f'GPU memory bandwidth: {model_dims.device_memory_bandwidth / 1000000000000.0:.2f} TB/s')
    print('-' * 60)
...
def main() -> None:
    ...
    parser = utils.ArgumentParserPlus((grpo_fast.Args, dataset_transformation.TokenizerConfig, model_utils.ModelConfig))
    ...
    vllm_engines, param_prompt_Q, inference_results_Q, actor_manager = setup_vllm_engines(args, tokenizer_config, model_config)
    timestamp = int(time.time())
    save_config(args, tokenizer_config, model_config, timestamp)
    run_benchmark(dataset, vllm_engines, param_prompt_Q, inference_results_Q, actor_manager, args, model_config, timestamp)
