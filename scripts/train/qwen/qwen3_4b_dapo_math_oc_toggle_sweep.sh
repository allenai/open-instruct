...
# Sweeps Toggle hyperparameters on the qwen3_4b_dapo_math_oc baseline.
# - toggle_lambda fixed at 0.9 (just under normalized max reward of 1.0).
# - token_budget_percentile in {50, 80}.
# - toggle_m in {10, 50, 100}.

EXP_NAME_PREFIX="${EXP_NAME_PREFIX:-qwen3_4b_base_dapo_toggle}"
...
TOGGLE_LAMBDA="${TOGGLE_LAMBDA:-0.9}"
...
for PERCENTILE in 50 80; do
    for M in 750; do
        ...
        uv run open_instruct/grpo_fast.py \
            ...
            --push_to_hub False \
            --toggle_m ${M} \
            --toggle_lambda ${TOGGLE_LAMBDA} \
            --token_budget_percentile ${PERCENTILE} \
            --toggle_warmup_steps ${TOGGLE_WARMUP_STEPS} \
            "$@"
    done
done
