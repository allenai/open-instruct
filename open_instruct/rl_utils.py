...
def masked_group_mean(values: torch.Tensor, mask: torch.Tensor, group_ids: torch.Tensor) -> torch.Tensor:
    ...
    2. Sum values per group.
    3. Count items per group.
    4. Compute mean per group.
    5. Compute mean of group means.
    """
    # Flatten everything
    flat_values = values.flatten()
    flat_mask = mask.flatten().bool()
    flat_group_ids = group_ids.flatten()

    # Filter invalid items
    valid_values = flat_values[flat_mask]
    valid_group_ids = flat_group_ids[flat_mask]

    if valid_values.numel() == 0:
        return torch.tensor(0.0, device=values.device)

    # Map group_ids to contiguous range 0..num_groups-1
    _, inverse_indices = torch.unique(valid_group_ids, return_inverse=True)
    num_groups = inverse_indices.max().item() + 1

    # Sum values and counts per group
    group_sums = torch.zeros(num_groups, device=values.device, dtype=values.dtype)
    group_counts = torch.zeros(num_groups, device=values.device, dtype=values.dtype)

    group_sums.scatter_add_(0, inverse_indices, valid_values)
    group_counts.scatter_add_(0, inverse_indices, torch.ones_like(valid_values))
