...
[Workflow evidence](measurements/researcher-workflow-20260911.md) records
the exact qualified path; conversion/save/export/restart remain separate.

Use the [detailed audit](measurements/feature-parity-audit-20260911.md),
[run-control guide](run-controls.md), and
[complete knob inventory](measurements/knob-inventory.md) for option mappings and limits.
...
Both [local tiny-model trials](measurements/core-datasources-local-20260910.json)
and [full-SFT EP2 trials](measurements/core-datasources-sft-20260910.json)
...
A separate [GSM8K/math/IF mixture](measurements/mixture-20260910.json)
...
| Serving admission and graphs | Actual 64 concurrent requests, decode graphs through 64, 524288 KV slots and 128 recurrent slots passed the larger B300 trial without observed OOM/retractions. [Evidence](measurements/admission64-20260911.json) | Prefix caching remains disabled in profiles; cache/replay/publication invalidation and full-model colocated graphs need their own acceptance. |
| Compiler caches | Default-on fingerprinted per-worker Triton restore/publication with WEKA `tmp-30d` retention. Full-SFT EP2/TP1 reuse and local automatic miss/reuse/changed-setting gates passed. [Evidence](measurements/startup-full-sft-20260911.json) | Multi-node and TP>1 serving remain unqualified. Broader compiler families are not covered; cache publication still costs minutes on WEKA. [Cache guide](compiler-cache.md) |
| Native checkpoint/resume | Schema-2 architecture/topology checks, optimizer/scheduler/RNG/cursor, completion marker. Full-model fast save/read gate passed exact restored state and two subsequent fixed-input updates/HF exports. [Evidence](measurements/checkpoint-fast-audit-20260911.json) | Synchronous saves, same topology. Background writes, retention and token-per-expert cadence absent. Process writer unqualified at full model. Exact continuation used separate persistent per-rank caches; it does not promise identical future sampled rollouts. |
...
- **Dynamic rows:** identical full-model scoring across 154,531 logprobs;
  ...
  kernels can still compile. [Measurement](measurements/control-exercise-20260911.md)
- **Checkpoints:** qualified direct save fell from 437.128 to 118.466 seconds;
  ...
  Ray startup/restart measurement. [Record](measurements/checkpoint-perf-20260911.md)
- **Caches:** the full-SFT EP2/TP1 cold/restored trial reduced driver-entry to
  ...
  persistence is now opt-out. [Evidence](measurements/startup-full-sft-20260911.json)
