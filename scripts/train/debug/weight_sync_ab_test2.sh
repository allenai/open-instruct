#!/bin/bash
# A/B test 2: match the slow grpo_p64_2node_v2 config exactly.
# Changes from large_test_script.sh:
#   load_ref_policy=true (was false)
#   num_mini_batches=4 (was 1)
#   sequence_parallel_size=1 (was 2)
# If weight_sync is ~100s, one of these configs (or their combination) is the cause.
num_prompts=25376
exp_name=weight_sync_ab_match_slow
...
uv run python mason.py \
	...
	--preemptible \
        --num_nodes 2 \
	--description "Weight sync A/B: match slow grpo_p64 config" \
        --timeout 1h \
        --max_retries 0 \
