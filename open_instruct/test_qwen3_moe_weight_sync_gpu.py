...
Run only this test on Beaker with:

    ./scripts/train/build_image_and_launch.sh scripts/test/run_gpu_pytest.sh \
        open_instruct/test_qwen3_moe_weight_sync_gpu.py

The test uses a tiny fused Qwen3-MoE learner under DeepSpeed ZeRO-3 and a
colocated vLLM engine initialized with dummy weights. It synchronizes through
Open-Instruct's packed IPC path, then independently perturbs dense, router, and
every expert projection to prove each learner tensor reaches the live engine.
"""
...
class Qwen3MoeZero3Trainer:
    """Own a partitioned learner and drive the production IPC sync path."""

    def __init__(self, model_path: str, distributed_port: int):
        os.environ.update(
            {
                "LOCAL_RANK": "0",
                "RANK": "0",
