...
### Added
- Add `--fixed_prompt_batches` to GRPO: with `async_steps>1`, form each training batch from exactly the prompt set queued for that step (early results of later batches are parked in the data-preparation actor, nothing is dropped for age) instead of the first responses to finish, so batch composition no longer follows response length while the generators keep working `async_steps` batches ahead. `PromptRequest`/`GenerationResult` carry a `batch_index`; logs `results_parked_for_later_batches`. Pure OPD with `async_steps>1` now warns unless this or `synchronous_rollouts` is set.
