...
### Fixed
- The factuality judge (and the refusal judge, which copies its template) scored every verdict 0.0: the template instructs the judge to answer exactly "True"/"False" and quotes the score slot, but the extractor called `float()` on it and fell to the 0.0 error sentinel, so correct and incorrect answers were indistinguishable. `SCORE` now coerces boolean verdicts to 1.0/0.0 (numeric semantics unchanged), the regex fallback accepts True/False, and new tests pin both the collapse and the numeric paths (https://github.com/allenai/open-instruct/pull/1833).
