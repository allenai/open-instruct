...
class AssistantSpanDerivationError(ValueError):
    """Raised when a conversation's assistant label spans cannot be derived reliably.

    Callers that tokenize a whole dataset convert this into "drop the row" rather than
    letting one conversation abort the map (see `_tokenize_row_or_mask_out`).
    """
...
def _assistant_token_spans_from_prefix_lengths(
    ...
    This is the pre-#1746 derivation, with `tools` threaded through (its absence is what
    made the token-count method wrong for tool-using conversations, and what motivated the
    switch to char offsets). It does not require the template to render prefix-stably,
    because it never treats a partial render as a substring of the full one -- it only
    counts how many tokens the prefix produced.
...
def _verify_assistant_spans_cover_content(
    ...
    """Raise if a derived span does not actually cover its assistant turn's content.

    The token-count derivation is silently wrong for templates whose per-turn rendering is
    not a simple concatenation (`olmo_thinker` drops the first tokens of each turn and
    trains a mid-word fragment). Checking the decoded span against the message content turns
    that silent mis-masking into a loud error.

    Templates that rewrite assistant content (e.g. stripping intermediate thinking) render
    something other than `content`; those turns are skipped rather than reported, since
    there is nothing to compare against.

    Spans are checked in both directions. Too narrow (the decoded span does not contain the
    turn's own content) means tokens were dropped from the loss. Too wide (it contains a
    *later* message's content) means the span ran past the end of the turn and the prompt is
    ...
    for message_idx, start, end in spans:
        ...
        # The span must begin exactly at the content, not part-way through the assistant header.
        # A template that emits no generation prompt puts the boundary a token or two early,
        # which leaks header text (": ", "assistant: ") into the loss without dropping content,
        # so the containment check above cannot see it. Leading whitespace is allowed because a
        # tokenizer may merge the space before the content into its first token.
        ...
        for later_idx in range(message_idx + 1, len(messages)):
            ...
            if later_content and later_content in decoded:
                raise AssistantSpanDerivationError(
                    f"Assistant label span for message {message_idx} extends past its turn: the span "
                    f"decodes to {decoded[:80]!r}, which contains message {later_idx} "
                    f"({messages[later_idx]['role']}) content {later_content[:40]!r}. Labels would "
                    f"include prompt tokens in the loss."
                )
...
def _tokenize_tulu_sft_with_assistant_labels(
    ...
    # Set when a prefix render is not a literal prefix of the full render, which makes char
    # offsets into `rendered` meaningless. Templates that special-case the final turn (eos
    # instead of <|im_end|>, or a trailing newline emitted only for non-final turns) hit this
    # routinely; before #1746 they tokenized correctly via prefix token counts, so fall back
    # to that rather than refusing the conversation outright.
...
def _tokenize_row_or_mask_out(
    ...
    Some conversation/template combinations have no derivable assistant span -- consecutive
    assistant turns are the common case, because every derivation has to render a prefix
    ending in an assistant turn, and templates that special-case the final turn render that
    prefix differently from the full conversation. Those rows are rare (~0.005% of
    tulu-3-sft-olmo-2-mixture) but a raise inside `dataset.map` aborts the entire job.

    Returning an all-masked row instead lets `sft_tulu_filter_v1` drop it and the run
    proceed; the count surfaces as `instances_filtered_after_tokenization` in the dataset
    statistics. Masking out is only ever a response to a *detected* failure -- a span that
    cannot be verified is never trained on.
    """
