...
def ensure_index_for_model(model, combined_dir, index_dir, overwrite=False):
    ...
    if not os.path.exists(src):
        print(f"[DEBUG] Model file missing: {src}")
    ...
    print(f"[DEBUG] {model}: {total_lines} total lines, {indexed_lines} indexed ({indexed_lines/max(1,total_lines)*100:.1f}%)")
    con.close()
    return dst
...
class ModelReader:
    ...
    def get_prompt_and_response(self, custom_id):
        ...
        if not row:
            self.debug_stats['not_in_index'] += 1
            self._cache_put(custom_id, (None, None))
            return (None, None)
        
        if not self.fh:
            self.debug_stats['file_missing'] += 1
            self._cache_put(custom_id, (None, None))
            return (None, None)
        ...
        try:
            ...
            self.debug_stats['seek_errors'] += 1
            self._cache_put(custom_id, (None, None))
            return (None, None)

        msgs = rec.get("messages")
        if not isinstance(msgs, list):
            self.debug_stats['no_messages'] += 1
            self._cache_put(custom_id, (None, None))
            return (None, None)
        
        try:
            prompt, resp = split_prompt_and_last_assistant(msgs)
            self.debug_stats['success'] += 1
            self._cache_put(custom_id, (prompt, resp))
            return (prompt, resp)
        except Exception as e:
            self.debug_stats['message_parse_errors'] += 1
            self._cache_put(custom_id, (None, None))
            return (None, None)
...
def process_parsed_file(parsed_path, out_part_path, selected_path, combined_dir, index_dir, allow_mismatch):
    ...
    missing_models_counter = Counter()  # Track which models are missing most
    ...
    # Collect debug stats from readers
    reader_stats = {}
    for model, reader in readers.items():
        reader_stats[model] = reader.get_debug_stats()
        reader.close()
    ...
    return {
        ...
        "missing_models_counter": dict(missing_models_counter),
...
def main():
    ...
    # Debug: Count model usage in selected_models
    model_usage = Counter()
    for models in selected.values():
        model_usage.update(models)
    print(f"\n=== Model Usage in Selected Models ===")
    ...
    if args.join_workers <= 1 or len(args.parsed) == 1:
        ...
        miss_model_total = 0
        mismatches_total = 0
        missing_models_counter = Counter()
        all_reader_stats = defaultdict(lambda: defaultdict(int))
                            ...
                            if any(json_compact(p) != first for p in non_null[1:]):
                                ...
                                if not args.allow_prompt_mismatch:
                                    ...
                                    sys.exit(
                                        ...
                                        f"Use --allow-prompt-mismatch to continue."
                                        f"Models involved: {models}"
                                        f"Prompts: {[json_compact(p) for p in non_null]}",
                                    )
        ...
        for model, reader in readers.items():
            reader_stats = reader.get_debug_stats()
            for stat_key, value in reader_stats.items():
                all_reader_stats[model][stat_key] += value
            reader.close()

        stats.append({
            "file": "(serial)", 
            "written": written_total, 
            "not_sel": not_sel_total,
            "miss_model": miss_model_total, 
            "mismatches": mismatches_total,
            "missing_models_counter": dict(missing_models_counter),
            "reader_stats": dict(all_reader_stats)
        })
    ...
    # Aggregate missing models counter
    total_missing_models = Counter()
    all_reader_stats = defaultdict(lambda: defaultdict(int))
    
    for s in stats:
        missing_counter = s.get("missing_models_counter", {})
        for model, count in missing_counter.items():
            total_missing_models[model] += count
            
        reader_stats = s.get("reader_stats", {})
        for model, model_stats in reader_stats.items():
            for stat_key, value in model_stats.items():
                all_reader_stats[model][stat_key] += value
    ...
    # Show most problematic models
    if total_missing_models:
        print(f"\n=== Models with Most Missing Entries ===")
        for model, count in total_missing_models.most_common(10):
            pct = count / model_usage.get(model, 1) * 100
            print(f"  {model}: {count:,} missing ({pct:.1f}% of {model_usage.get(model, 0):,} expected)")
    
    # Show detailed reader statistics
    if all_reader_stats:
        print(f"\n=== Detailed Reader Statistics ===")
        for model in sorted(all_reader_stats.keys()):
            stats_dict = all_reader_stats[model]
            total_requests = sum(stats_dict.values())
            if total_requests > 0:
                print(f"\n{model} ({total_requests:,} total requests):")
                for stat_name, count in sorted(stats_dict.items()):
