#!/bin/bash
# Qwen3-4B policy + Qwen3-8B value model with GT conditioning (1000-step warmup)
# 1 node (8 GPUs): 2 learner + 6 vLLM engines
# Requires hamish/ppo-value-model branch
DDMM=$(date +"%d%m")
exp_name=vip_ppo_gt_8bval_${DDMM}_qwen3_4b_math
...
uv run python mason.py \
    ...
    --decoupled_gae \
    --length_adaptive_gae \
    --value_model_name_or_path Qwen/Qwen3-8B \
