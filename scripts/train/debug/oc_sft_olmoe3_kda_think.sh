...
#                    CKPT_ROOT=<dir> STEP=stepNNNN. Must run in-image: uses
#                    /stage/.venv/bin/python directly because `uv run` resyncs
#                    and reverts the torch 2.11 companion installs (torchvision
#                    0.26+cu130, transformer-engine, the cuBLAS symlink).
...
#   probe            1 GPU on holmes: probe_kda_forward.py, one KDA layer built
#                    from CONFIG_NAME, forward at increasing lengths. Checks the
#                    image's torch/CUDA/fla/KDA kernels (torch 2.10 dies here with
#                    CUBLAS_STATUS_NOT_INITIALIZED); NOT a memory gate -- per-rank
#                    optimizer state shards 1/world_size, so memory needs 1x8.
#
...
# The Olmo 3.5 hero-small presets live on kda-think-sft only: their configs need
# the hero olmo-core lineage (89e7dcb7), not olmo-core main.
BASE="${BASE:-proxy}"
case "$BASE" in
  proxy)
    ...
    ;;
  *)
    echo "Unknown BASE=$BASE (expected proxy; hero-small presets are on branch kda-think-sft)" >&2
    exit 1
    ;;
esac
...
# olmo123 is not a CHAT_TEMPLATES entry: before #1878 it silently fell back to the
# tokenizer's own template, and since #1878 it raises. tokenizer_default names that
# same template explicitly. The name is part of the cache hash, so caches built
# under olmo123 (15bfc110a1 legacy, 062b8a3d20 aligned) are not reused by name.
CHAT_TEMPLATE="${CHAT_TEMPLATE:-tokenizer_default}"
...
# CPU tokenize/discover jobs. A full Dolci-Think tokenize is ~12 h; without a
# timeout a stuck one sits forever (the 11.5 h datasets/torchvision death was silent).
CPU_TIMEOUT="${CPU_TIMEOUT:-24h}"
...
case "$MODE" in
  tokenize_subset|tokenize_full)
    ...
    $PY mason.py \
        ...
        $EXTRA_BUCKET_FLAGS \
        -- /stage/.venv/bin/python open_instruct/olmo_core_finetune.py \
  ...
  probe)
    $PY mason.py \
        ...
        --priority "$PRIORITY" \
        --image "$BEAKER_IMAGE" \
        --description "KDA kernel probe (1 GPU): one KDA layer from $CONFIG_NAME" \
        --pure_docker_mode \
        $PREEMPTIBLE_FLAG \
        --timeout "${PROBE_TIMEOUT:-30m}" \
        --num_nodes 1 \
        --gpus 1 \
        --non_resumable \
        --no_auto_dataset_cache \
        -- /stage/.venv/bin/python scripts/train/debug/probe_kda_forward.py \
        --config $CONFIG_NAME \
        --seq-lens 128 2048 8192 32768 65536
    ;;
  ...
  *)
    ...
    echo "Expected one of: tokenize_subset, tokenize_full, discover_cache, probe, gate, smoke_2node, lr_probe, train, convert, convert_rl" >&2
    exit 1
    ;;
esac
