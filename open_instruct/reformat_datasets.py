...
def convert_super_ni_data(
    ...
    def _encode_instruction_example(instruction, input, output, random_template=True, eos_token=None):
        if random_template:
            if input is not None and input.strip() != "":
                ...
                prompt_template, completion_template, _ = random.choices(
                    _encoding_templates_w_input, weights=[w for _, _, w in _encoding_templates_w_input]
                )[0]
                ...
                prompt_template, completion_template, _ = random.choices(
                    _encoding_templates_wo_input, weights=[w for _, _, w in _encoding_templates_wo_input]
                )[0]
    ...
    def _encode_few_shot_example(instruction, examplars, input, output, eos_token=None):
        prompt = instruction.strip() + "\n\n"
    ...
    with open(os.path.join(output_dir, "super_ni_data.jsonl"), "w") as fout:
        for task in train_tasks:
            ...
            for instance in instances[:zero_shot_examples_per_task]:
                encoded_example = _encode_instruction_example(
                    instruction=instruction,
            ...
            for instance in instances[zero_shot_examples_per_task:]:
                ...
                encoded_example = _encode_few_shot_example(
                    instruction=instruction,
                    examplars=examplars,
                    input=instance["input"],
                    output=instance["output"][0],
