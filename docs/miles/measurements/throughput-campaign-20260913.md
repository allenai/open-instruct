# September 13 throughput campaign log

This is the chronological qualification record. Early profile sizes and pending
statements below describe stages of the investigation. Use the
[current throughput guide](../throughput-profiles.md) for recommended configurations.
...
| [tiny](https://github.com/allenai/open-instruct/blob/fe4d9f2bdc994adb35f839718d86e420d8481e12/configs/miles/examples/tiny.toml) | 1 trainer + 1 inference GPU | Disaggregated mechanics and very small jobs | Same small model fixture for the initial exercise. A full SFT model is not assumed to fit one trainer GPU. |
...
### Decode graphs and completed-queue waiting

The completed [2T/6I batch-128 graph run](https://beaker.org/ex/01M2EAA3NKFZRTS5QQ6FKGDEHB)
passed 16 updates and clean shutdown. Updates 7–16 delivered 3,750 consumed
response tokens/s, with zero dequeued token drops. Scoring/training and publication
occupied 95.3% of the awaited driver cycle. Nearly all the remaining 4.7% was other
collection/handoff work: actual completed-buffer gets took 0.014 seconds across
the entire ten-update window. Increasing inference capacity would not address
this overhead. All 1,280 delivered responses were in groups whose oldest behavior
policy was two updates old; none of the consumed tokens were sampled under the
current trainer version. This is bounded async throughput, not an on-policy claim.
There were no mixed responses in this arm.

The [2T/4I batch-32 graph run](https://beaker.org/ex/01M2EC9EXRGJZBW3EPHHQRP654)
also passed 16 updates and clean shutdown. Its warm window delivered 2,842 tokens/s,
with scoring/training and publication occupying 91.9% of the cycle. But it discarded
112 response attempts while delivering 320: 25.9% of attempts and 35.7% of tokens.
The 8.1% collection stage splits into 4.8% awaiting/filtering completed groups and
3.3% other collection/handoff work. The sampled completed queue alternated between
full and empty, averaging five of eight groups and empty for 12.9% of observed time.
Excess production and waiting can coexist because eligible supply is bursty and
old completed groups must be rejected.

Replay audits passed on both ranks: 512 checked microbatches per rank across 19
routed layers, including scoring, training and recomputation. The run consumed 176
mixed responses, so it exercised graph-decoded responses across weight refreshes.
This validates the measured route-replay contract, not downstream learning parity.

A final focused follow-up keeps that model, batch, fleet and lag limit unchanged,
and reduces `async_max_concurrent_samples` from 64 to 32. A 64-sample producer plus
32-sample FIFO can hold three future batches; the reduced budget allows two.
The result will determine whether lower ahead-of-training work improves the small
configuration without starving training. FIFO and the age rule remain unchanged.

The graph trials use full decode graphs only (through the requested admission),
with prefill graphs disabled. They also enable replay diagnostics, unlike the
graphs-off performance controls, so trainer overhead is not an isolated graph
comparison. The [2T/16I graphs-off run](https://beaker.org/ex/01M2E8YXBZ2E3B7JZW6RH6KYMV)
