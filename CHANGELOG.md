...
### Added
- Support GRPO and DPO training of the OLMo-hybrid small suite (`olmo_hybrid_small` architecture): pin transformers + vLLM + olmo-core to the maintainer forks via `[tool.uv.sources]`, plus single-GPU debug scripts for the 275M SFT checkpoint. GRPO consumes the maintainer's HF-converted (`-hf`) checkpoint directly via transformers/vLLM (no olmo-core at runtime). DPO builds the olmo-core-native model by deserializing the sibling native checkpoint's own `TransformerConfig` (no hardcoded preset needed, with the training-time `flash_3` attention backend overridden to match `--attn_backend`), and loads/saves weights against the `-hf` checkpoint via new hand-written HF<->olmo-core converters for the hybrid GatedDeltaNet linear-attention layers (the olmo-core fork's generic `load_hf_model`/`save_hf_model` don't support them). Also fixes flash-attn silently building without its CUDA extension (`FLASH_ATTENTION_SKIP_CUDA_BUILD` was left enabled) (https://github.com/allenai/open-instruct/pull/TODO).
