...
# OPD: same-lineage distillation at 4B — pure-distill the released tmax-4b
# teacher into its own base (hamishivi/Qwen3.5-4B) on the tmax-15k terminal
# tasks. 4B twin of the successful 9B experiment (base <- tmax-9b), which hit
# teacher parity in ~40-60 steps; matched attention geometry means the teacher
# uses the efficient sharded (tiled) scoring path.
#
# Released 4B recipe: 4 nodes = 16 learners (8 8, SP=4) + 16 engines, 64k
# response / 67584 pack, 8 prompts x 32 samples. Pure OPD: rewards logged, no
# gradient; reward-variance filtering disabled. Watch objective/opd_reverse_kl.
...
EXP_NAME=swerl_qwen35_4b_opd_from_tmax4b_4node_64k

uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "OPD: base Qwen3.5-4B student <- tmax-4b teacher (same-lineage pure distill; 4-node; 64k)" \
       --pure_docker_mode \
