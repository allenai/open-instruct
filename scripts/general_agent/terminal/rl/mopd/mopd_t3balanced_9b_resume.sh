...
EXP_NAME=swerl_mopd_t3balanced_9b_4node_64k

# Resume of exhausting-retries run 01M02NEMD084S3H9J6KZK3A7DG (node death:
# missing heartbeats; beaker retries reuse the dying allocation). Fresh launch
# = fresh nodes; state resume via the original mason-auto state dir.
STATE_DIR=/weka/oe-adapt-default/allennlp/deletable_checkpoint_states/shashankg/1786796003_217901
RESUME_STEP=$(cat "$STATE_DIR/latest" 2>/dev/null || echo "none")

uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "MOPD Option-2 BALANCED ablation RESUME: state resume at ${RESUME_STEP} after node death (4-node; 64k; 500 steps)" \
       --pure_docker_mode \
       ...
       -- source scripts/docker/docker_login.sh \&\& source configs/beaker_configs/ray_node_setup.sh  \&\& python scripts/data/prepare_terminal3_mopd.py --task-data-dir /tmp/terminal3_task_data --jsonl-dir /tmp/mopd_terminal3 \&\& python open_instruct/grpo_fast.py \
    --dataset_mixer_list /tmp/mopd_terminal3/termigen.jsonl 14601 /tmp/mopd_terminal3/endless.jsonl 14601 /tmp/mopd_terminal3/tmax.jsonl 14601 \
