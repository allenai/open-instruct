...
### Fixed
- `get_successful_tests_fast` no longer counts worker-process creation against `max_execution_time`, which is documented as bounding the graded code. Under the `spawn` start method a child costs ~0.2s to create, so a sub-second budget could score correct code as failing before it ran; the stdio sibling already allowed for this with its flat `+ 5.0` (https://github.com/allenai/open-instruct/pull/1831).
