...
class TestConfigToJsonSerializable(unittest.TestCase):
    ...
    @parameterized.expand(
        [
            ("enum_conversion", {"loss_type": dpo_utils.DPOLossType.dpo}, {"loss_type": "dpo"}),
            ("nested_dict", {"outer": {"inner": dpo_utils.DPOLossType.simpo}}, {"outer": {"inner": "simpo"}}),
            ("list_conversion", [dpo_utils.DPOLossType.dpo, dpo_utils.DPOLossType.wpo], ["dpo", "wpo"]),
            (
    ...
    def test_config_to_json_serializable(self, name, input_val, expected):
        self.assertEqual(dpo_utils.config_to_json_serializable(input_val), expected)
