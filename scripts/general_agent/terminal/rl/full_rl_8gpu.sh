...
# Full-scale terminal RL on a single 8-GPU Jupiter node — no Beaker/mason.py.
# Mirrors the production 2-node config scaled to one node:
#   4 learner GPUs (SP=2) + 4 vLLM engine GPUs = 8 GPUs total.
#
# Requirements:
...
# Podman service config (mirrors production)
export SWERL_DOCKER_AUTO_REMOVE=1
export SWERL_SANDBOX_TIMING_LOGS=1
export SWERL_PODMAN_SERVICE_COUNT=8
export SWERL_DOCKER_START_CONCURRENCY=128
...
# Start Podman services and authenticate with Docker Hub
source scripts/docker/docker_login.sh

# Start local Ray head (single-node; no ray_node_setup.sh needed)
ray stop --force
ray start --head --port=8888 --dashboard-host=0.0.0.0

uv run python open_instruct/grpo_fast.py \
    --exp_name terminal_local_rl_qwen35_4b_base_tmax_10k_8gpu \
    ...
    --output_dir output/tmax_rl_8gpu_local \
    --push_to_hub false
