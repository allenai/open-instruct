...
class EnvironmentTool(Tool):
    """
    Tool that wraps an RL environment for interactive rollouts.

    This is a special tool type that manages environment actors via an EnvironmentPool.
    It provides reset/step semantics for RL environments during generation.

    Unlike regular tools that execute and return immediately, environment tools
    maintain state across multiple calls within an episode.
    """

    config_name = "environment"
    description = "RL environment interaction tool"
    # Parameters are dynamically provided by the environment at reset() time
    ...
    def __init__(
        ...
        **env_kwargs: Any,
        ...
        Args:
            call_name: Name used when function calling
            env_name: Name of registered environment (e.g., "sandbox")
            env_class: Full class path for unregistered environment
            pool_size: Number of environment actors to create
            response_role: Role for responses ("tool" or "user")
    ...
    async def initialize_pool(self) -> None:
        """Initialize the environment pool. Called lazily on first use."""
        if self._initialized:
            return

        from open_instruct.environments import EnvironmentPool

        self._pool = EnvironmentPool(
        ...
        logger.info(f"Environment pool initialized: {self.call_name} (size={self.pool_size})")

    async def acquire_env(self):
        """Acquire an environment actor from the pool."""
        if not self._initialized:
            await self.initialize_pool()
        return await self._pool.acquire()

    def release_env(self, env_actor) -> None:
        """Release an environment actor back to the pool."""
        if self._pool is not None:
            self._pool.release(env_actor)

    async def shutdown(self) -> None:
        """Shutdown the environment pool."""
        if self._pool is not None:
    ...
    async def _execute(self, **kwargs: Any) -> ToolOutput:
        """
        Environment tools don't use the standard _execute pattern.

        Instead, they expose reset/step via acquire_env/release_env
        and are called directly by the rollout loop in vllm_utils.py.
        """
        return ToolOutput(
            output="Environment tools should be called via env actor, not _execute",
            called=False,
            error="Direct _execute not supported for environment tools",
            timeout=False,
            runtime=0,
        )
...
class EnvironmentToolConfig(BaseToolConfig):
    ...
    Usage in --tool_configs:
        {
            "tool_type": "environment",
            "call_name": "sandbox",
            "env_name": "sandbox",     # OR env_class for custom envs
            "pool_size": 64,
            "backend": "e2b",          # env-specific kwargs
            "task_prompt": "Solve the task"
        }
    """
    ...
    env_class: str | None = None
    """Full class path for unregistered environment (e.g., 'mymodule.MyEnv')."""

    pool_size: int = 64
    """Number of environment actors to create."""

    response_role: str = "tool"
    """Role for responses ('tool' or 'user' for multi-turn emulation)."""

    # Common environment kwargs (passed through to env __init__)
    backend: str | None = None
    """Backend type for sandbox envs ('e2b' or 'docker')."""

    base_url: str | None = None
    """Base URL for OpenEnv client environments."""

    task_prompt: str | None = None
    """Task prompt/description for the environment."""
...
TOOL_REGISTRY: dict[str, type[BaseToolConfig]] = {
    ...
    EnvironmentToolConfig.tool_class.config_name: EnvironmentToolConfig,
}
