...
class VLLMConfig:
    ...
    vllm_attention_backend: str | None = "FLASH_ATTN"
...
class StreamingDataLoaderConfig:
    ...
    dataset_mixer_eval_list: list[str] | None = None
    ...
    def __post_init__(self):
        ...
        if self.active_sampling_max_samples_multiplier is not None and self.active_sampling_max_samples_multiplier < 0:
            raise ValueError(
                f"`active_sampling_max_samples` must be non-negative, got {self.active_sampling_max_samples_multiplier}."
            )
