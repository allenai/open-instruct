# Deprecated vLLM GRPO reference

> **Deprecated:** `open_instruct/grpo.py` and `open_instruct/grpo_fast.py` are
> retained for existing runs and historical reproduction. For new GRPO work, use
> `python -m open_instruct.miles` and the [MILES guide](../miles/index.md).
> Check [model/workload support](../miles/models-and-checkpoints.md); a missing
> capability needs an explicit decision, not an automatic legacy fallback.

The commands, flags and performance measurements below describe the deprecated
vLLM implementations. They do not apply to MILES, which has separate configuration,
SGLang serving and a pinned runtime. Existing commands still run and emit a
startup deprecation notice. They do not redirect to MILES or translate flags.
