...
class CodeSearchVerifierOld(VerifierFunction):
    """
    Verifier for code search tasks that checks if the model correctly identifies
    and views files in a repository.

    The label should contain information about the expected file path and content
    to verify. The verifier extracts file viewing operations from the prediction
    and scores based on whether the correct files were accessed.
    """

    def __init__(self, verifier_config: CodeVerifierConfig) -> None:
        super().__init__("code_search_old", verifier_config=verifier_config, weight=1.0)
        self.pass_rate_reward_threshold = verifier_config.code_pass_rate_reward_threshold
        self.apply_perf_penalty = verifier_config.code_apply_perf_penalty

    def extract_viewed_files(self, prediction: str) -> List[Dict[str, Any]]:
        """Extract file view operations from the model's prediction."""
        viewed_files = []

        # Pattern to match tool calls for viewing files
        tool_call_pattern = r"<tool_call>\s*(.*?)\s*</tool_call>"
        tool_calls = re.findall(tool_call_pattern, prediction, re.DOTALL)
        ...
        for tool_call_str in tool_calls:
            try:
                ...
                if tool_call.get("arguments", {}).get("command") == "view":
                    ...
                    viewed_files.append({"path": path, "view_range": view_range})
    ...
    async def async_call(
        ...
        """
        Asynchronously verify code search operations.

        Args:
            tokenized_prediction: Unused tokenized representation
            prediction: The model output containing file view operations
            label: Expected file paths or search criteria
            query: Original query (may contain repo information)

        Returns:
            VerificationResult with score based on correctness of file operations
        """
        # Extract viewed files from prediction
        viewed_files = self.extract_viewed_files(prediction)

        if not viewed_files:
            return VerificationResult(score=0.0, reasoning="No file view operations found")

        # Parse label to get expected files
        expected_files = []
        ...
        if isinstance(label_data, dict):
            # If label contains specific file paths
            if "files" in label_data:
                expected_files = label_data["files"]
            elif "path" in label_data:
                expected_files = [label_data["path"]]
            elif "bug_fn_file" in label_data:
                expected_files = [label_data["bug_fn_file"]]
        elif isinstance(label_data, list):
            expected_files = label_data
        elif isinstance(label_data, str):
            # Single file path
            expected_files = [label_data]

        if not expected_files:
            # If no expected files specified, check if any reasonable files were viewed
            score = 1.0 if len(viewed_files) > 0 else 0.0
            return VerificationResult(score=score, reasoning=f"Viewed {len(viewed_files)} files")

        # Calculate score based on overlap between viewed and expected files
        viewed_paths = {f["path"] for f in viewed_files}
        expected_paths = set(expected_files) if isinstance(expected_files, list) else {expected_files}

        # Normalize paths for comparison
        normalized_viewed = {p.strip("/").lower() for p in viewed_paths}
        normalized_expected = {p.strip("/").lower() for p in expected_paths}

        # Check for matches
        matches = normalized_viewed & normalized_expected

        if matches:
            # Full score if at least one expected file was viewed
            score = 1.0
            reasoning = f"Correctly viewed {len(matches)}/{len(normalized_expected)} expected files"
        else:
            # Partial score based on any reasonable file viewing activity
            score = 0.3 if len(viewed_files) > 0 else 0.0
            reasoning = f"Viewed {len(viewed_files)} files but none matched expected: {expected_paths}"

        # Apply performance penalty if configured
        if self.apply_perf_penalty and score > 0.0:
            # Penalize for viewing too many unnecessary files
            excess_views = max(0, len(viewed_files) - len(expected_files))
            penalty = min(0.5, excess_views * 0.1)  # Max 50% penalty
            score = score * (1 - penalty)
            reasoning += f" (penalty for {excess_views} excess views)"

        return VerificationResult(score=score, reasoning=reasoning)
