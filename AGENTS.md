...
- The `olmo-eval-internal` directory is required in the Docker image for experiments that use `--try_launch_olmo_eval_jobs_on_weka`. If it's missing, clone it with: `git clone --depth=1 https://github.com/allenai/olmo-eval-internal.git olmo-eval-internal`. The Docker image installs it via `uv pip install -e './olmo-eval-internal[beaker]'`.
