...
def create_model_and_optimizer(args: Args, tc: TokenizerConfig, model_config: ModelConfig, 
                               beaker_config: BeakerRuntimeConfig, wandb_url: str, 
                               tokenizer: PreTrainedTokenizer):
...
def sync_weights_and_prepare_prompts(training_step: int, args: Args, train_dataset, iter_dataloader,
                                     policy_group: ModelGroup, queries_prompt_Q: Queue, param_prompt_Q: Queue,
                                     queries_next=None, ground_truths_next=None, datasets_next=None):
    ...
    if training_step != 1:
        ...
        with Timer("[Main Thread] 🔄 Loading weights using shared memory" if args.async_mode else "🔄 Loading weights using shared memory"):
            ray.get([m.broadcast_to_vllm.remote() for m in policy_group.models])
...
def one_training_step(args: Args, policy_group: ModelGroup, collated_data, tokenizer, 
                      data_thread_metrics, average_metrics, episode, training_step, 
                      num_total_tokens, start_time, train_dataset, writer, wandb_url):
...
def maybe_evaluate(args: Args, training_step: int, evaluation_inference_results_Q: Queue,
                   tokenizer, eval_prompt_token_ids, eval_ground_truths, eval_dataset_names,
                   reward_fn, episode, writer):
    ...
    try:
        for training_step in range(resume_training_step, args.num_training_steps + 1):
            ...
            queries_next, ground_truths_next, datasets_next = sync_weights_and_prepare_prompts(
                training_step, args, train_dataset, iter_dataloader, policy_group, queries_prompt_Q, param_prompt_Q,
            ...
            average_metrics = one_training_step(
                args, policy_group, collated_data, tokenizer, data_thread_metrics, {},
                episode, training_step, num_total_tokens, start_time, train_dataset, writer, wandb_url
            )

            # ------------------------------------------------------------------------------------------------
            # Optionally evaluate the model
            maybe_evaluate(
                args, training_step, evaluation_inference_results_Q, tokenizer,
                eval_prompt_token_ids, eval_ground_truths, eval_dataset_names,
                reward_fn, episode, writer
            )
