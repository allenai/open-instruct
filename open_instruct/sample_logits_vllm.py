...
from open_instruct.distillkit import DistributionQuantizationConfig, LogprobCompressor
from open_instruct.distillkit.sample_common import OutputHandler, StreamingParquetWriter, compressed_logit_schema
...
def process_prompt_logprobs(prompt_logprobs: PromptLogprobs, k: int) -> tuple[torch.LongTensor, torch.Tensor]:
...
def main(args: SampleLogitsArguments, tc: TokenizerConfig) -> None:
    ...
    try:
        with OutputHandler(output_dir, logger=logger) as output_handler:
            local_output = output_handler.local_output_path
            os.makedirs(local_output, exist_ok=True)
            with open(os.path.join(local_output, "compression_config.yaml"), "w", encoding="utf-8") as f:
                ...
                with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
                    ...
                    for i0 in tqdm.tqdm(range(0, len(train_dataset), args.macrobatch_size), desc="Logit Batches"):
                        ...
                        outputs = llm.generate(
                            [{"prompt_token_ids": x.tolist() if hasattr(x, "tolist") else x} for x in batch_input_ids],
