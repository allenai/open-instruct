...
# RL on Qwen/Qwen3.5-9B + hamishivi/agent-task-terminal-traj
...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "SWERL agent-task-terminal-traj GRPO with Qwen3.5-9B" \
       --pure_docker_mode \
    ...
    --dataset_mixer_list hamishivi/agent-task-terminal-traj 1.0 \
    ...
    --save_traces \
    --tools swerl_sandbox \
    --tool_configs '{"task_data_hf_repo": "hamishivi/agent-task-terminal-traj", "test_timeout": 120, "image": "python:3.12-slim"}' \
    --pool_size 512 \
    --max_steps 100 \
    ...
    --output_dir /output \
    --exp_name swerl_qwen35_9b_base_agent_task_terminal_traj_grpo \
    --local_eval_every 10 \
    --save_freq 20 \
