...
# RL on Qwen/Qwen3.5-9B + hamishivi/swerl-tmax-15k-verified.
...
# done) instead of inferring truncation from finish_reason / response length.
# Combined with env max_steps=100, this avoids the perverse "give up early"
...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "SWERL tmax-10k-verified GRPO with Qwen3.5-9B + max_steps=100 + mask_non_submitting + 10pct_unmask" \
       --pure_docker_mode \
    ...
    --dataset_mixer_list hamishivi/swerl-tmax-15k-verified 1.0 \
    ...
    --tool_configs '{"task_data_hf_repo": "hamishivi/swerl-tmax-15k-verified", "test_timeout": 120, "image": "python:3.12-slim"}' \
    --pool_size 512 \
    --max_steps 100 \
    ...
    --output_dir /output \
    --exp_name swerl_qwen35_9b_base_tmax_10k_verified_grpo_mask_no_submit_10pct \
    --local_eval_every 10 \
    --save_freq 20 \
