...
#
# Global batch = 2 nodes * 8 GPUs * per_device 1 * grad_accum 4 = 64 sequences,
# so 1000 steps is ~64k sequences (a partial pass over Dolci, not a full epoch).
...
uv run python mason.py \
    ...
    --priority urgent \
    --image "$BEAKER_IMAGE" \
    --description "Olmo-3-7B SFT, Dolci-Instruct-SFT, 1000 steps, seq4096 (issue 1800 fix)" \
    --pure_docker_mode \
    --preemptible \
    --num_nodes 2 \
    --gpus 8 \
