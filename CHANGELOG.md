...
### Added
- Support GRPO and DPO training of the OLMo-hybrid small suite (`olmo_hybrid_small` architecture): pin transformers + vLLM to the maintainer forks via `[tool.uv.sources]`, plus single-GPU debug scripts for the 275M SFT checkpoint. Both paths consume the maintainer's HF-converted (`-hf`) checkpoints directly (no olmo-core at runtime) (https://github.com/allenai/open-instruct/pull/TODO).
