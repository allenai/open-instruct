...
# OPD H1 test: does an RLVR'd teacher distill WORSE than a raw one?
#
# Identical to qwen35_4b_opd_from_tmax9b_4node_64k.sh except the teacher is the
# RAW (non-RL'd) hamishivi/Qwen3.5-9B instead of allenai/tmax-9b — same 2.25x
# size gap, same student, same everything else. Compare directly against the
# existing 4B <- tmax-9b arm (best TB2.1 0.202, KL floor ~0.095).
#
# Caveat for reading results: the raw teacher is itself much weaker on terminal
# tasks (TB2.1 0.193 vs tmax-9b 0.276), so "RLVRed teacher is worse" means the
# student under the raw teacher matching/beating the tmax-9b arm DESPITE the
# weaker teacher. Teacher is loaded learner-side (mixed attention geometry ->
# full-sequence scoring under SP, same as the tmax-9b arm).
#
# Workspace: ai2/general-tool-use (second H-test arm goes to oe-agents).

BEAKER_IMAGE="${1:?Usage: $0 <beaker-image>}"

MODEL=hamishivi/Qwen3.5-4B
TOKENIZER=hamishivi/Qwen3.5-4B
TEACHER_MODEL=hamishivi/Qwen3.5-9B

EXP_NAME=swerl_qwen35_4b_opd_from_raw9b_4node_64k
...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "OPD H1: base Qwen3.5-4B student <- RAW hamishivi/Qwen3.5-9B teacher (is an RLVRed teacher worse than a raw one? vs the tmax-9b-teacher arm; 4-node; 64k)${RESUME_NOTE}" \
       --pure_docker_mode \
