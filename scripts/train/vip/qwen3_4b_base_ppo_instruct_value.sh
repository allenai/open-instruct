#!/bin/bash
# Qwen3-4B-Base PPO with Qwen3-4B-Instruct as value model
# lambda=1.0, no decoupled/adaptive GAE (known working config)
# 1 node (8 GPUs): 4 learner + 4 vLLM engines
DDMM=$(date +"%d%m")
exp_name=vip_ppo_base_ival_${DDMM}_qwen3_4b_math
...
uv run python mason.py \
    ...
    --push_to_hub False \
    --use_value_model \
    --value_model_name_or_path Qwen/Qwen3-4B-Instruct-2507 \
