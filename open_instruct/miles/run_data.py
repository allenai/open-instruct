...
class ManifestIFVerifier(MultiplicationVerifier):
    ...
    async def async_call(self, tokens, prediction, label, **kwargs):
        ...
        if isinstance(target, list):
            if len(target) != 1:
                raise ValueError("IF target requires exactly one constraint bundle")
            target = target[0]
            if isinstance(target, str):
                target = json.loads(target)
        if not isinstance(target, dict) or "instruction_id" not in target or "kwargs" not in target:
            raise ValueError("IF target requires instruction_id and kwargs")
...
def _rows(raw):
    ...
    if any(not isinstance(row, dict) for row in values):
        raise ValueError("Data JSONL must contain objects")
    return values
...
def _scalar(target):
    if isinstance(target, list):
        if len(target) != 1:
            raise ValueError("Expected a singleton answer")
        target = target[0]
    if target is None or not str(target).strip() or isinstance(target, dict):
        raise ValueError("Expected a nonempty scalar answer")
    normalized = str(target).strip()
    if normalized.startswith("[") and normalized.endswith("]"):
        raise ValueError("Scalar answer must not be a stringified list")
...
def validate_data(data):
    ...
    if len(modes) != 1:
        raise ValueError("Choose exactly one of data.tasks, recipe, rl_manifest, or prompt_data")
    if "prompt_data" not in modes and any(data.get(key) is not None for key in ("eval_prompt_data", "reward_config")):
        raise ValueError("eval_prompt_data and reward_config belong to prepared prompt_data mode")
    if "recipe" in modes:
        raise ValueError("Named data recipes are not yet ported; use tasks or an immutable rl_manifest")
    if "prompt_data" in modes and not data.get("reward_config"):
        raise ValueError("Prepared prompt_data requires a trusted reward_config")
    if "tasks" in modes:
        tasks = data["tasks"]
        if not isinstance(tasks, list) or not tasks:
            raise ValueError("data.tasks must be a nonempty list")
        names = []
        for task in tasks:
            if not isinstance(task, dict) or set(task) - {"task", "train_count", "eval_count", "prompt_wrapper"}:
                raise ValueError("Task entries require task, train_count/eval_count and optional prompt_wrapper")
            name = task.get("task")
            if name not in TASKS:
                raise ValueError(f"Unsupported task {name!r}; supported tasks: {sorted(TASKS)}")
            names.append(name)
            ...
            if not (task.get("train_count") or task.get("eval_count")):
                raise ValueError(f"{name} requires a train_count or eval_count")
            if task.get("prompt_wrapper", "none") not in ("none", "auto", "open_instruct_rlzero_answer"):
                raise ValueError("Unsupported prompt_wrapper")
        if len(set(names)) != len(names) or not any(task.get("train_count") for task in tasks):
            raise ValueError("Tasks must be unique and include training data")
    if type(data.get("shuffle", True)) is not bool or type(data.get("seed", 17)) is not int:
        raise ValueError("data.shuffle must be boolean and data.seed must be an integer")
...
def _messages(row, *, strip_answer):
    ...
    if messages is None:
        ...
        if not isinstance(question, str) or not question:
            raise ValueError("Source requires messages or a question/prompt")
    ...
    if not messages or messages[-1]["role"] != "user":
        raise ValueError("Prepared prompt must end in a user turn; reference answers are not prompts")
    return messages


def _render(messages, tokenizer, template):
    if not isinstance(template, str) or not template:
        raise ValueError("Supply an explicit HF or manifest chat template")
...
def _verify_row(row, tokenizer, limit, known):
    if not isinstance(row.get("input"), str) or not row["input"]:
        raise ValueError("Prepared row needs nonempty rendered input")
    metadata = row.get("metadata")
    if not isinstance(metadata, dict) or not isinstance(metadata.get("verifiers"), list) or not metadata["verifiers"]:
        raise ValueError("Prepared row requires metadata.verifiers")
    for spec in metadata["verifiers"]:
        if not isinstance(spec, dict) or spec.get("name") not in known or "target" not in spec:
            raise ValueError(f"Unsupported or malformed verifier: {spec!r}")
        weight = spec.get("weight", 1.0)
        if isinstance(weight, bool) or not isinstance(weight, (int, float)) or not math.isfinite(weight):
            raise ValueError("Verifier weight must be finite")
    ids = tokenizer.encode(row["input"], add_special_tokens=False)
    if not 0 < len(ids) <= limit:
        raise ValueError(f"Prompt has {len(ids)} tokens, outside 1..{limit}; immutable input cannot be truncated")
    expected = metadata.get("prompt_token_ids")
    if expected is not None and expected != ids:
        raise ValueError("Prepared prompt token IDs differ under this tokenizer/template")
...
def _adopt(data, tokenizer, template, inputs):
    ...
    manifest = json.loads(_read(path, inputs))
    if manifest.get("schema_version") != 1:
        raise ValueError("Only baseline rl_manifest schema_version=1 is supported")
    options = manifest["miles"]
    if options.get("custom_rm_path") != "olmo_miles.rl.rewards.registered_reward":
        raise ValueError("Manifest reward function is not the supported baseline registered reward contract")
    if options.get("apply_chat_template") is not True:
        raise ValueError("Baseline manifest must declare apply_chat_template=true")
    descriptor = options.get("chat_template")
    if descriptor is not None:
        raw = _read(path.parent / descriptor["path"], inputs)
        if _sha(raw) != descriptor["sha256"]:
            raise ValueError("Manifest chat template hash mismatch")
        template = raw.decode()
    partitions = {}
    for split, artifact in manifest["artifacts"].items():
        if split not in ("train", "eval"):
            raise ValueError(f"Unsupported manifest partition: {split}")
        raw = _read(path.parent / artifact["path"], inputs)
        if _sha(raw) != artifact["sha256"]:
            raise ValueError(f"Manifest {split} artifact hash mismatch")
        source = _rows(raw)
        if len(source) != artifact.get("records"):
            raise ValueError(f"Manifest {split} record count mismatch")
        partitions[split] = []
        for row in source:
...
def _prepared(data, inputs):
    registry = json.loads(_read(data["reward_config"], inputs))
    if not isinstance(registry, dict) or not registry:
        raise ValueError("Trusted reward registry must be a nonempty mapping")
    for name, spec in registry.items():
        if (
            ...
            raise ValueError("Trusted registry entries require explicit factory paths")
    evaluation = data.get("eval_prompt_data", [])
    if not isinstance(evaluation, list) or len(evaluation) % 2:
        raise ValueError("eval_prompt_data must alternate dataset names and paths")
    partitions = {"train": _rows(_read(data["prompt_data"], inputs))}
    for name, path in zip(evaluation[::2], evaluation[1::2], strict=True):
        if (
            ...
            raise ValueError("Evaluation names must be unique safe identifiers distinct from train")
        partitions[name] = _rows(_read(path, inputs))
...
def _tasks(data, tokenizer, template, seed):
    ...
    for task in data["tasks"]:
        ...
        if name == "multiplication":
            if total > 8100:
                raise ValueError("Generated multiplication supports at most 8100 unique ordered two-digit pairs")
            ...
            if len(rows) < total:
                raise ValueError(f"{name}: requested {total} rows but source has {len(rows)}")
        ...
        for position, index in enumerate(indices):
            ...
            if verifier in ("gsm8k", "multiplication"):
                ...
                if not isinstance(target, str) or not target.strip():
                    raise ValueError("Math target must be a nonempty scalar string")
            elif not isinstance(target, str) or "func_name" not in json.loads(target):
                raise ValueError("Named ifeval source requires legacy func_name target")
...
def prepare_data(data: dict, hf_checkpoint: Path, output: Path, *, max_prompt_length: int, seed: int) -> dict:
    ...
    if output.exists():
        if not manifest_path.is_file():
            raise ValueError("Preparation directory is incomplete; use a fresh output directory")
        manifest = json.loads(manifest_path.read_text())
        if manifest["contract"] != contract:
            raise ValueError("Preparation contract changed; refusing to alter resumed run data")
    ...
    if not partitions.get("train"):
        raise ValueError("Preparation requires nonempty training data")
    ...
    if train_prompts & eval_prompts or train_ids & eval_ids:
        raise ValueError("Training and held-out data overlap by prompt or source identity")
    if registry is None:
    ...
    try:
        ...
        for path, digest in inputs.items():
            if _sha(Path(path).read_bytes()) != digest:
                raise ValueError(f"Preparation source changed while preparing: {path}")
