#!/bin/bash
# Score a scalar value model with correct-demo conditioning (single sibling, prefer correct).
set -euo pipefail

VALUE_MODEL_PATH="${1:?Value model path required}"
INPUT_DATASET_PATH="${2:-./value_estimation_data/dapo_math_100pairs.parquet}"
OUTPUT_PATH="${3:-./value_estimation_data/scalar_value_correct_demo_scores.parquet}"
...
uv run python -m open_instruct.value_estimation score_dataset \
    ...
    --run_name scalar_value_correct_demo
