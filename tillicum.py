...
def build_slurm_script(args: argparse.Namespace, commands: list[list[str]], experiment_dir: str) -> str:
    ...
    if args.nodes > 1:
        ...
        lines.append("    # Check ray status with explicit address")
        lines.append("    RAY_STATUS=$(ray status --address=$RAY_ADDRESS 2>&1) || true")
        lines.append("    NODE_COUNT=$(echo \"$RAY_STATUS\" | grep -c 'node:' || echo 0)")
        ...
        lines.append("        ray status --address=$RAY_ADDRESS || true")
        lines.append("        exit 1")
        lines.append("    fi")
