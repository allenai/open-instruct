...
| 8 | Throughput and performance comparison, Open Instruct vs Miles, for RL (verifier DPPO/GRPO) and OPD, sync and async, matched one-node topology. Plan in [`opd_handoff_2026-09-21.md`](opd_handoff_2026-09-21.md) §7 (matrix T1–T6, phases P0–P3). | Decide which stack carries production OPD and RL at 4B scale on measured GPU-min per step and tokens per GPU-second, not on the unmatched phase-2 numbers (Miles 136 vs Open Instruct 83 GPU-min per 4B step). | **Planned 2026-09-21.** P0 (metric map, Miles Qwen3.5-4B GRPO run file, one-node Open Instruct scripts, re-derived baseline row) needs no GPU; the ~36 node-hour short-run matrix needs Kevin's OK. Miles async for OPD is unproven (refused on the Core backend, never run on Megatron). |
...
Updated 2026-09-21 20:21Z (was 2026-09-21 19:56Z; details in the latest Log entries).
