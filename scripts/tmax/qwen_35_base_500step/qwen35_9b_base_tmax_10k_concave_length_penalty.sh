...
# weighted combination of model tokens, tool-output tokens, and tool calls.
...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "SWERL tmax-10k GRPO with Qwen3.5-9B + mask_truncated_completions + concave length penalty" \
       --pure_docker_mode \
    ...
    --concave_length_penalty_w_model 1.0 \
    --concave_length_penalty_w_obs 0.25 \
    --concave_length_penalty_w_call 200.0 \
    --concave_length_penalty_normalizer 1000.0 \
    ...
    --output_dir /output \
    --exp_name swerl_qwen35_9b_base_tmax_10k_grpo_mask_overlong_concave_length \
    --local_eval_every 10 \
    --save_freq 20 \
