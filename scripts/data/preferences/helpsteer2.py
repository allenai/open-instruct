...
def main(aspects_to_ignore: List[str], min_score: Optional[float], margin: Optional[float], push_to_hub: bool, hf_entity: Optional[str]):
    ...
    print(f"Removed {len(ds['train']) - len(binarized_ds)} datapoints due to ties, single responses, below threshold, or insufficient margin")
