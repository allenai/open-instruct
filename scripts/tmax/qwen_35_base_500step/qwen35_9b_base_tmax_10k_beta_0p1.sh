...
# Runs without non-submission or overlong masking, with beta=0.1.
...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "SWERL tmax-10k GRPO with Qwen3.5-9B + beta=0.1 + no masking" \
       --pure_docker_mode \
       --workspace dr-tulu-ablations \
       --priority urgent \
       --preemptible \
       --num_nodes 4 \
       --max_retries 5 \
    ...
    --output_dir /output \
    --exp_name swerl_qwen35_9b_base_tmax_10k_grpo_beta_0p1 \
    --local_eval_every 10 \
    --save_freq 20 \
