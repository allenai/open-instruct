...
### Fixed
- `olmo_core_finetune.py` forced `ephemeral_save_interval=500` through `parser.set_defaults`, so any run with `--checkpointing_steps` below 500 was rejected at startup by olmo-core's `'ephemeral_save_interval' must be less than 'save_interval'` check, and omitting the flag yielded 500 rather than `None` so ephemeral checkpoints could not be turned off at all. A non-positive value now disables them, matching the existing `-1` convention for `max_checkpoints` (https://github.com/allenai/open-instruct/pull/1810).
