#!/bin/bash
# Qwen3-4B-Base PPO with 'rollout_context' value conditioning template
# Prepends 4 sibling rollouts (with CORRECT/INCORRECT labels) + GT answer before scoring
# 1 node (8 GPUs): 4 learner + 4 vLLM engines, 32 prompts x 16 rollouts
DDMM=$(date +"%d%m")
exp_name=vip_ppo_gt_rollout_ctx_${DDMM}_qwen3_4b_math
