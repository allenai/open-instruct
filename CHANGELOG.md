...
### Added
- Add `ModalBackend`, a `SandboxBackend` that runs RL sandboxes in Modal's cloud instead of on-node Podman/Docker. Select it with `backend: modal` in the env config; requires Modal credentials and network egress (check with `scripts/modal/check_modal_egress.sh`). See `docs/sandbox_modal_vs_podman.md` for the trade-offs.
