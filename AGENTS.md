...
- Use `normal` priority unless the workload qualifies for a higher level under Ai2 compute policy. `urgent` is reserved for work critical to an annual goal that must finish within six weeks; exceptions require documented CEO approval. Several checked-in debug scripts still specify `--priority urgent`, so do not assume their priority is appropriate for a new launch.
