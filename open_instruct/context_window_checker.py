        ...
        for msg in other_messages:
            ...
            if current_tokens + message_tokens <= remaining_tokens:
                ...
                if available_for_content > 0:
                    ...
                    logger.warning(f"Truncated message content to fit judge context window")
                break
        
        # append judgment format to the last message, only if there are messages
        if truncated_messages[-1]["role"] == "user" and not truncated_messages[-1]["content"].endswith('Respond in JSON format. {"REASONING": "[...]", "SCORE": "<your-score>"}'):
