#!/bin/bash
# A/B test script: compare vLLM fork vs official vLLM on weight sync performance.
# Uses the same config as large_test_script.sh but with a distinct exp_name.
num_prompts=25376
exp_name=vllm_ab_test
...
uv run python mason.py \
	...
	--preemptible \
        --num_nodes 2 \
	--description "vLLM A/B test: weight sync performance." \
        --timeout 1h \
        --max_retries 0 \
