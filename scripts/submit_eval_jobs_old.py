# DEPRECATED: This script is kept for backwards compatibility. New eval jobs
# should use scripts/submit_eval_jobs.py.
import argparse
import copy
import os
import re
...
warnings.warn(
    "scripts/submit_eval_jobs_old.py is deprecated; use scripts/submit_eval_jobs.py instead.",
    DeprecationWarning,
    stacklevel=2,
)
...
if args.run_oe_eval_experiments:
    ...
    # Forward custom tokenizer path so the eval image can load a known-compatible
    # tokenizer instead of the checkpoint's bundled one.
