...
def sparse_tvd_inner(
    ...
    tvd_sparse_terms_sum = torch.sum(torch.abs(sparse_teacher_probs - sparse_student_probs), dim=-1)

    if missing == MissingProbabilityHandling.SYMMETRIC_UNIFORM:
        teacher_prob_sum_sparse = sparse_teacher_probs.sum(dim=-1)
        teacher_missing_prob_mass = (1.0 - teacher_prob_sum_sparse).clamp(min=0.0, max=1.0)
        student_prob_sum_sparse = sparse_student_probs.sum(dim=-1)
        student_missing_prob_mass = (1.0 - student_prob_sum_sparse).clamp(min=0.0, max=1.0)
        tvd_missing_contrib = torch.abs(teacher_missing_prob_mass - student_missing_prob_mass)
    else:
        student_prob_sum_sparse = sparse_student_probs.sum(dim=-1)
