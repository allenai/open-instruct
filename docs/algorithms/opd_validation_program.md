...
|-----:|------|-----|--------|
| 1 | Close out the Qwen3.5 4B Miles v5 replication: eval hf-89, hf-99; manual `opd_audit` on the 4B root; final docs pass. | Completes the 2-framework cross-check (2B done 10/10, 4B 8/8 so far). | In progress: 4B job R8CZMP running; hf-89 landed 00:30Z 2026-09-18, greedy eval `01M2RYZ320TBZWY8C89W92D84E` running (compare to sync step 90: 0.90/0.77/0.43/0.57); hf-99 + audit ~03:20Z. |
...
| 2b | Open Instruct hardening: real-dump OPD audit (advantage identity + alignment), NaN-inside-mask becomes an error, document the temperature coupling. | Flaws 3, 5, 4. | In progress: `validate_opd_logprobs` guard + tests landed; caveats documented; `open_instruct/opd_trace_audit.py` re-derives the advantage from a real `--save_traces` dump (teacher logprobs + advantages now dumped); Beaker real-dump job `01M2RZ8YYFT927SM1EDC7646ST` running (campaign image + branch cloned in-job via `CODE_REF` and copied over `/stage/open_instruct`, because local Docker cannot build the CUDA image: out of memory); trace dir `deletable_rollouts/opd_trace_audit/20260918T003942Z`. First attempt `01M2RYWF3QVT89HX1Y3VA4FHTQ` failed on an import mismatch (PYTHONPATH overlay ignored by the image's editable install), not a defect. |
