...
Updated 2026-09-18 07:30Z (details in the latest Log entries).
...
- Step 2d (async fix, "fix not fence" per Kevin): implemented as `--fixed_prompt_batches` in
  ...
  vs the sync rerun, `qwen35_math_opd_fixed_batches_rerun.sh`): **approved 07:14Z**; patch
  dataset `01M2SNQYARKRCZPZ6AN8CEJPGX`, smoke `01M2SNTT1XKJZW2B7B2QZK1VCX` passed 07:26Z; full
  run `01M2SPKXVMR0QT3FCZEYE5AKXD` running since 07:28Z (4 nodes, ~5h, no auto-resume, min
  runtime 8h, timeout 12h; relaunch if it dies before step 100).
- Overnight plan (Kevin asleep from 07:15Z; Claude monitors on a 30-min cadence): keep arm 2
  alive through its 8h windows (relaunch on the same image `01M2SEQPBZRNP1HBWC72N71JCQ` if a
  window dies without auto-resume); keep the full fixed-batches run alive (relaunch with
  `PATCH_DATASET=01M2SNQYARKRCZPZ6AN8CEJPGX RUN_MODE=full` if it dies early); when arm 2 finishes collect
  Avg@8 and launch the EOPD arm (`[distillation] eopd=true, eopd_alpha=1.0, eopd_tau=0.8,
  eopd_top_k=16`, root `-eopd-v1`, same image). Push notifications only for unfixable failures
  or milestones.
