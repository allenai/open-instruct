...
for step in 40 80 120 160 200 240 280 320 360 400; do
python oe_eval/launch.py \
  --model test_grpo_rm_with_valpy_code__1__1747948379_step_${step} \
  --beaker-workspace ai2/tulu-3-results \
  --beaker-budget ai2/oe-adapt \
  --task $eval \
  --model-type vllm \
  --batch-size 10000 \
  --model-args "{\"model_path\":\"gs://ai2-llm/post-training//jacobm/output/test_grpo_rm_with_valpy_code__1__1747948379_checkpoints/step_${step}\", \"max_length\": 32768}" \
  ...
  --push-datalake \
  --datalake-tags run_id=https://wandb.ai/ai2-llm/open_instruct_internal/runs/4ocmvxyb,step=${step}
done
done