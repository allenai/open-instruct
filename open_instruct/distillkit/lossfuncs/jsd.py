...
from open_instruct.distillkit.lossfuncs.common import MissingProbabilityHandling, accumulate_over_chunks, get_logprobs
...
def sparse_jsd_inner(
    ...
    if missing == MissingProbabilityHandling.SYMMETRIC_UNIFORM:
        teacher_prob_sum_sparse = sparse_teacher_probs.to(torch.float32).sum(dim=-1, keepdim=True)
        log_teacher_missing_prob = torch.log1p(-teacher_prob_sum_sparse.clamp(min=eps, max=1.0 - eps))
        teacher_missing_prob = torch.exp(log_teacher_missing_prob)

        student_prob_sum_sparse = sparse_student_probs.to(torch.float32).sum(dim=-1, keepdim=True)
        log_student_missing_prob = torch.log1p(-student_prob_sum_sparse.clamp(min=eps, max=1.0 - eps))
    ...
    kl_p_m_sparse_sum = torch.sum(
        torch.where(sparse_teacher_probs > eps, kl_p_m_sparse_terms, torch.zeros_like(kl_p_m_sparse_terms)), dim=-1
    )

    if missing == MissingProbabilityHandling.SYMMETRIC_UNIFORM:
        kl_p_m_missing_term = teacher_missing_prob * (log_teacher_missing_prob - log_m_missing)
    ...
    kl_q_m_sparse_terms = sparse_student_probs * (sparse_student_logprobs - log_m_sparse)
    kl_q_m_sparse_sum = torch.sum(
        torch.where(sparse_student_probs > eps, kl_q_m_sparse_terms, torch.zeros_like(kl_q_m_sparse_terms)), dim=-1
    )

    if missing == MissingProbabilityHandling.SYMMETRIC_UNIFORM:
        kl_q_m_missing_term = student_missing_prob * (log_student_missing_prob - log_m_missing)
