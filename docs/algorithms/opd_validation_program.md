...
- **Kevin approved the two remaining harness evals**; launched at 18:51Z on one H100 each with the
  `qwen25math_eval_arm2_hf219.yaml` spec and only the model path / paper-student swapped:
  EOPD `hf-219` (4B) `01M32MXFB7R8M00VSR9EKBDE0A`, arm 1 OPD `hf-175` (1.7B,
  `--paper-student qwen3-1.7b-base`) `01M32MXG1T097RYYTHQH3804HR`. `ai2/ceres` had to be dropped
  from the cluster list (`ai2/olmo-instruct` has no allocation there for min-runtime jobs). Outputs
  land under `deletable_checkpoint/kevinfarhat/eopd/qwen25math_eval/`. ~40 min each; the EOPD one
  is the six-benchmark Avg@8 / Pass@8 readout that decides the paper's +1.8 / +5.05 claim.
- **Reporting:** Kevin asked for a tables-first page instead of slides. Published
  "OPD Validation Results" (all program tables: phase 1 async runs, sync vs async, Miles vs Open
  Instruct, the six-flaw audit, both baseline reproductions, the paper-grader table with the
  Minerva re-grade, EOPD vs OPD at every eval, EOPD internals, teacher entropy, conclusions).
  The two decks remain as they were.

### 2026-09-21 19:06Z

- **Arm 1 harness eval `01M32MXG1T097RYYTHQH3804HR` failed (exit 1 at 19:01Z):** it landed on
  `holmes-cs-aus-515` (B300, `compute_103a`); the campaign image's vLLM JIT-builds flashinfer
  kernels with an nvcc that does not know that architecture (`nvcc fatal: Unsupported gpu
  architecture 'compute_103a'`). Not a code problem. **Relaunched as `01M32NNHA27J7W1ZT282CQWCV1`
  (19:04Z) with the cluster list cut to `ai2/jupiter` + `ai2/saturn` (H100).** The EOPD eval is
  on a jupiter H100 and sampling normally (MATH500 500×8 in 422 s, AMC23 in 98 s). Lesson for the
  eval yaml: keep `ai2/holmes` out until the image is rebuilt with a Blackwell-capable toolchain.
- **Kevin asked why EOPD does not show the paper's gain when everything else matched.** Answer
  recorded on the results page (new "What the paper's +1.80 / +5.05 is made of" and "Are we
  running it the same way?" tables) and summarised here:
  - Paper Table 2 at 4B, OPD → EOPD Avg@8: MATH500 +1.39, AMC23 +3.61, Minerva −0.37,
    OlympiadBench +1.14, AIME24 −0.41, AIME25 +5.42 (mean +1.80). Pass@8: +2.20 / +7.50 / +2.99 /
    +4.31 / +10.00 / +3.33 (mean +5.05). Two Avg@8 deltas are negative and the two largest sit on
    30-40-problem sets where our own A/B swung −3.5…+4.7 between evals of the same pair. The one
    Avg@8 delta above our noise floor is MATH500 +1.39 vs our +0.25 in-run; our OPD reads 79.45
    under the paper's grader vs 78.05 in-run, so the like-for-like EOPD MATH500 number is the
    running harness eval. Pass@8 (their strongest column) has no in-run measurement at all.
  - Recipe: every Table 9 / App. C setting and every fixed choice in the authors' verl fork
    matches (models, non-thinking teacher, data, B 128 / mini 32 / 1 sample, LR 3e-6 cosine,
    verl AdamW, T 1.0 top-p 1.0, 4096 tokens, 2 epochs, PPO clip 0.2, trainer-side student
    log-prob, token-mean, training suffix, eos remap, α 1.0 τ 0.8 k 16, Eq. 10 FKL). Known
    deviations: gate uses the renormalised top-16 entropy rather than the exact full-vocabulary
    entropy (98 % agreement on this pair, 6.5 % false negatives → slightly fewer gated tokens);
    SGLang bf16 teacher log-probs vs an in-trainer verl ref worker; k 16 (paper) vs 32 (their
    README launch config); 8×H100 vs 4×A100; their launch script is absent from the repo so
    anything beyond Table 9 is an assumed verl default; single seed on both sides.

### 2026-09-21 19:39Z

- **EOPD harness eval complete: `01M32MXFB7R8M00VSR9EKBDE0A` exit 0 at 19:37Z, 43 min on a
  jupiter H100, result dataset `01M32MXFBDWAV4WGPY6QZM5H01`** (`harness_eopd/` locally). Same
  protocol as the OPD export (App. C sampling, Qwen2.5-Math harness `a45202b`, 12,368 samples).

  | Set | EOPD Avg@8 | OPD Avg@8 | Δ | EOPD Pass@8 | OPD Pass@8 | Δ | Paper Δ Avg / Pass |
  |---|---|---|---|---|---|---|---|
  | MATH500 | 79.55 | 79.45 | +0.10 | 92.40 | 92.00 | +0.40 | +1.39 / +2.20 |
  | AMC23 | 58.12 | 57.50 | +0.62 | 85.00 | 82.50 | +2.50 | +3.61 / +7.50 |
  | Minerva | 29.78 | 29.78 | 0.00 | 44.49 | 45.59 | −1.10 | −0.37 / +2.99 |
  | OlympiadBench | 45.70 | 45.40 | +0.30 | 64.99 | 64.99 | 0.00 | +1.14 / +4.31 |
  | AIME24 | 18.75 | 18.75 | 0.00 | 30.00 | 33.33 | −3.33 | −0.41 / +10.00 |
  | AIME25 | 19.17 | 20.42 | −1.25 | 46.67 | 43.33 | +3.33 | +5.42 / +3.33 |
  | **mean** | **41.84** | **41.88** | **−0.04** | **60.59** | **60.29** | **+0.30** | **+1.80 / +5.05** |

  Truncation and length are the same for both exports (MATH500 0.8 % vs 1.1 %, 958 vs 940
  tokens); Minerva had 5 grader timeouts on EOPD vs 0 on OPD (out of 2,176). **Verdict: on the
  paper's own protocol, EOPD is indistinguishable from plain OPD on both Avg@8 and Pass@8.** The
  paper's 4B gain (+1.80 / +5.05) does not reproduce with a matched baseline that itself lands on
  the paper's OPD numbers. Caveat on both sides: one seed per arm. Sizing the seed variance (a
  second OPD and EOPD seed, 2 × 16 h on one node) is the next question for Kevin; not launched.
- Arm 1 harness eval `01M32NNHA27J7W1ZT282CQWCV1` is sampling on an H100 (MATH500, AMC23,
  Minerva done by 19:20Z); watcher re-armed for it alone.
- Results page updated (header verdict, paper Table 2 decomposition now carries our grader
  columns for both exports, status table, conclusions).

### 2026-09-21 19:56Z

- **Arm 1 harness eval complete: `01M32NNHA27J7W1ZT282CQWCV1` exit 0 at 19:53Z (46 min on an
  H100), result dataset `01M32NNHA91ZRZJJRDCNFAF1AP`** (`harness_arm1/` locally). Paper Table 2,
  Qwen3-1.7B-Base OPD row in brackets: MATH500 68.12 / 86.80 (67.76 / 84.80), AMC23 36.88 / 72.50
  (39.06 / 70.00), Minerva 25.18 / 42.28 (29.83 / 47.06), OlympiadBench 32.20 / 54.45 (30.09 /
  51.56), AIME24 11.25 / 20.00 (8.33 / 20.00), AIME25 6.25 / 23.33 (6.25 / 16.67); six-benchmark
  mean Avg@8 29.98 vs 30.22, Pass@8 49.89 vs 48.35. Minerva is again the only set below the paper
  (same notation artefact as the 4B export); truncation at 8,192 tokens is higher for the 1.7B
  (AIME24 25 %). **Both of the paper's OPD baselines replicate under its own grader.** All
  compute for steps 4 and 6 is done; nothing running; watcher idle.
- Results page: arm 1 harness table added to section 5, status table complete. Kevin asked for
  a runs-and-links list (W&B / Beaker) for a colleague covering sync vs async, Open Instruct vs
  Miles, EOPD vs OPD; added as a section on the page (Miles was never run async: its rollout
  loop is strictly on-policy by construction, so there is no Miles async arm).

### 2026-09-21 20:21Z

- **Correction: Miles does have an async mode.** The 19:56Z entry (and the results page) said Miles
  "was never run async because its rollout loop is strictly on-policy by construction". Kevin
  checked with the maintainers; the wrapper has a bounded fully-asynchronous pipeline
  (`miles.fully_async = true`, explicit `core.max_policy_lag`, resident disaggregated engines,
  FIFO completed-group buffer with stale discards, TIS on; `docs/miles/async-pipeline.md`,
  `configuration.md`, example `configs/miles/examples/grpo-async-disaggregated.toml`). What is true:
  every Miles OPD run in this program used the default synchronous barrier mode, and
  `open_instruct/miles/config.py` refuses async for Core-backend OPD ("Core OPD requires
  synchronous barrier publication and trainer-scored log probabilities"); on the Megatron backend
  async OPD is not refused but has never been exercised. Results page section "Runs and links"
  reworded (Version 5).
- **Handoff written for the next session:** [`opd_handoff_2026-09-21.md`](opd_handoff_2026-09-21.md)
  (repos, branches and push state, where sync-vs-async lives in each stack, results summary with
  run ids, standing constraints, open decisions, and the throughput/performance plan). Both
  branches (`codex/qwen35-math-opd` `13866c4d5`, `robertb/miles-qwen35-opd` `14080e06c`) were fully
  pushed at handoff. Session scratch (harness yamls, watchers, page source, harness summaries)
  copied to `/Users/kevinfarhat/repos/opd-campaign-scratch/`. Step 8 added to the program table.

### 2026-09-21 23:48Z

- Kevin approved **2B async OPD in Miles**, with policy age, discarded work, response lengths, truncation, and sync-versus-async trainer wait times. This narrows the handoff's broad matrix to the informative 2B arm.
- Pinned-image inspection found Megatron OPD still unconditionally selected `train.py` and legacy generation. Added explicit async dispatch, isolated shared-engine math evaluation, measured completed queue, consuming-optimizer-step age checks, and a checkpoint ledger that retains prefetched prompts until their update is saved. Miles commit `4d47a7832`, pushed directly to `robertb/miles-qwen35-opd`.
- Full config: `configs/miles/opd/qwen35-2b-opd-async-lag3.toml`, 100 updates, same student/teacher/data/seed/LR/batch/16K cap as sync v5; max age 3, producer 1024 responses, completed FIFO 512 responses, group submission, native retract publication. Preserve rollout student logprobs and PPO clipping; **TIS off** because native Miles forbids it with rollout-logprob OPD. This is not Core async OPD.
- Hardware correction: actual sync v5 Beaker node metadata and perf logs show **B300**, not H100. Async matches 8 B300s on Holmes, 4 trainer (TP2) + 3 student + 1 teacher.
...
- Async qualification [01M335TSWA8MTK1YAEZF0CXJEE](https://beaker.org/ex/01M335TSWA8MTK1YAEZF0CXJEE) completed at 00:28:59Z with exit 0 and workflow complete. All 24 runtime tests, four optimizer updates, periodic/final evaluation, OPD audit, and fresh-process `hf-3` export reload passed. Audit: maximum advantage error 0.0 across all four updates, 270 tensors changed from initialization and 250 since the preceding export; 18 FP32 A_log tensors preserved. W&B `ztwdi5ok`; consumed maximum ages 0/1/2/3. This 512-token smoke qualifies mechanics, not learning or full-recipe throughput. Evidence retained in `opd-campaign-scratch/miles-async2b/smoke-results/`.
- Full 100-update 2B async comparison launched as [01M338D7FBJYJX6YWJ0NX5RA03](https://beaker.org/ex/01M338D7FBJYJX6YWJ0NX5RA03) through the committed-image wrapper. Image `01M2V7V946ZN9N9STPK0H04YWM`, code overlay `4d47a783228ab2ad90e40724f6de51a3c2ec1249`, config `configs/miles/opd/qwen35-2b-opd-async-lag3.toml`, root `qwen35-2b-opd-from-verifier-2b-math-async-lag3-v1`. Same 8-B300 allocation and baseline recipe, 128x2 responses/update, 16K cap, age bound 3; automatic resume enabled. Config plan/validate passed; campaign state, receipts and workspace search found no duplicate before submission.
- Retained all ten sync greedy checkpoint scores and the exact completed sync `hf-99` evaluation spec (`01M2RD932J83K39RG0NN6FJNVN`) in scratch. Matched post-hoc evaluations must explicitly retain the base tokenizer/initial vLLM model and load each trained export through learner weight sync, as in the reference. Timing comparison now intersects ordinary update IDs after excluding warmup in either run; lint and mismatched-resume/baseline checks passed.
...
### 2026-09-22T06:30:15Z

- Original full async training job remains running with 29 updates confirmed. In-run greedy evaluations: update 10 DAPO 52.73%, MATH-500 85.2%, AIME 23.33%, BRUMO 36.67%; update 20 DAPO 48.05%, MATH-500 82.6%, AIME 23.33%, BRUMO 40%. These are encouraging early results; matched post-hoc scores and the full 100-update comparison remain pending.
- Read-only diagnostic `01M33GPFWZ7T0XHYK7HDANKZS0` exited 0 and verified `hf-9/.complete`, model index and all eight model shards. Submitted its matched greedy evaluation [01M33WT3V4KVNGW2GNWEGN96G5](https://beaker.org/ex/01M33WT3V4KVNGW2GNWEGN96G5) at 06:28:19Z, fresh W&B ID `3447bf9c`. Replayed the completed sync evaluation Beaker specification with exact image `01M01EPKXMXR4502S1HNJVYN0M`, patch/data, base tokenizer/vLLM model, learner weight synchronization, 16K cap, greedy pass@1 and seed 42; changed model/experiment/output identity and removed unrelated credential references. This preserves the immutable historical harness rather than rebuilding its image through the generic wrapper. Spec, receipt and duplicate check are retained locally. Approval review initially rejected the unnecessary BEAKER_TOKEN reference; removal resolved it and submission succeeded.
- Evaluation job `01M33WT3YXEAH3C73P9DBKJ650` is queued: scheduler reports workspace 57/64 slots and Jupiter budget 162/168, each short of this eight-GPU request. Wait for capacity; do not relaunch elsewhere. Update-20 export verification and matched evaluation remain next.
- Matched ordinary IDs 2–28 excluding 10/20 (25 updates): async/sync mean trainer wait 472.2/558.9 s, mean interval 534.5/620.7 s, weighted wait fraction 88.3/90.0%. Eight-GPU minutes/update 71.3/82.8, ordinary training only; checkpoint/evaluation/startup costs excluded and retained separately for final cost accounting.

### 2026-09-22T16:37:36Z

- Async W&B confirms 79 optimizer updates; latest completed in-run evaluation is update 70, DAPO 52.54%, MATH-500 83.8%. Original Beaker training attempt remains running.
- Matched ten-update evaluation `01M33WT3V4KVNGW2GNWEGN96G5` exited 0 at 07:00:51Z, W&B `3447bf9c`: async/sync DAPO **53.3203/52.5391%**, MATH-500 **82.2/80.8%**, AIME **33.33/26.67%**, BRUMO **30/40%**. Differences +0.7813/+1.4/+6.67/−10 percentage points; early single-checkpoint evidence only. Beaker metadata lookup warnings after dropping the unused Beaker credential were nonfatal; evaluation completed and logged all datasets.
...
### 2026-09-22T17:47:14Z

- Full async training remains on the original running job, 84 optimizer updates confirmed. All eight matched evaluations at updates 10–80 completed with exit 0; no evaluation remains queued/running. Updates 90/100 and final training audit/export reload remain outstanding.
- New matched async/sync scores (percent):
  - Update 50: dapo 48.0469/51.1719, math500 81.0000/81.6000, aime2025 36.6667/33.3333, brumo2025 40.0000/26.6667.
  - Update 60: dapo 51.9531/49.4141, math500 78.2000/80.2000, aime2025 23.3333/26.6667, brumo2025 30.0000/26.6667.
  - Update 70: dapo 50.7812/45.7031, math500 81.8000/79.4000, aime2025 33.3333/30.0000, brumo2025 26.6667/43.3333.
...
### 2026-09-23T02:26:30Z

- Final matched update90 evaluation completed exit0 at02:15:06Z: DAPO49.21875%, MATH50081.2%, AIME26.6667%, BRUMO23.3333%. All10matched checkpoints are now complete. Full audit and fresh export reload passed; no follow-up jobs remain.
- Final report/figure saved under `miles-async2b/final-comparison.md` and `matched-learning-curves.png`, plus the MILES dated measurement document. Endpoint MILES async-minus-sync DAPO is+4.1015625pp (52.9296875vs48.828125), versus OI−46.6796875pp (0.9765625vs47.65625), verified from exact W&B endpoint summaries. The MILES run does not reproduce collapse; differing age, cancellation, loss and topology prevent a single-cause conclusion.
- Final timing: 87paired ordinary updates, async552.409vs sync619.297s; wait490.072vs555.112s; wait fraction88.715vs89.636%. Response lengths differ:10,713.98vs12,432.88tokens; truncation24.00%vs36.83%; useful delivered tokens per allocated GPU-second621.32vs642.00. Thus10.80%shorter updates is not a token-throughput gain. Running-job GPUhours161.65vs190.73; sync had two preemptions.
- Cancellation accounting:49,517upstream aborted-group filter events, counters reset perbatch, separate from zero completed-queue stale drops. Missing partial-token waste and its age/length distribution explicitly documented; no claim of zero total wasted generation. One seed; allcheckpoint mean DAPO delta+1.27pp. Completion validation and monitor shutdown follow.

### 2026-09-23T04:01:28Z

- New 2B async root-cause investigation: the exact corrected-depth patch from clean commit `a6aa78b19` was uploaded as dataset `01M365QV8P4295DQHH9TW7E79G`. A four-update fixed-batch smoke [01M365REVQMC6D95W7CYG3YCBR](https://beaker.org/ex/01M365REVQMC6D95W7CYG3YCBR) started on one Jupiter H100 node. It reached training step 4 but deadlocked: step 3 accumulation began with no prompt batch queued, then the trainer blocked requesting that batch. Stopped the job after 14m39s to free eight GPUs.
- Root cause in `DataPreparationActor._data_preparation_loop`: `_queue_due_fixed_batches()` ran after the previous accumulation and on each arriving result, but trainer consumption could advance while the data actor waited at the depth throttle. After the wait, it entered `accumulate_inference_batches()` without another refill, so no new results could arrive to trigger the callback. A one-call refill immediately after the throttle is applied for GPU retest. This is a mechanics bug in the previously unverified depth fix, not evidence about OPD learning. Local formatting, compile and diff checks passed; full local test import is unavailable on macOS because vLLM is absent.

### 2026-09-23T04:18:13Z

- Repaired fixed-batch depth smoke [01M366W01255G03BK40D90C1ST](https://beaker.org/ex/01M366W01255G03BK40D90C1ST) passed all four optimizer updates and final evaluation; newest job `01M366W04Y49RG51762JZYDZVE` exited 0 at 04:16:18Z. Its step-3 prompt batch was received after the throttle, confirming the refill deadlock is fixed. Versioned patch dataset `01M366VFEBYESEQY4G1Y0S59SV` is from committed/pushed OI revision `c3b784e74`.
- Submitted the controlled 100-update, 4-node corrected-depth async-4 run [01M367PWBF8QDKKX688M7MEZHK](https://beaker.org/ex/01M367PWBF8QDKKX688M7MEZHK) with that patch, seed 42, canonical teacher/prompt/DPPO/LR/response settings, fixed prompt batches and checkpoints every ten updates. Fresh run name `qwen35_2b_opd_fixed_depth_async4_lr1e6_100step_4node_20260922_211713`. Beaker has four jobs, all initially queued. Scheduler events say the 32-slot replica group cannot fit while workspace `ai2/olmo-instruct` uses 41/64 allocated slots and budget `ai2/oe-other` uses 167/168 Jupiter allocation slots. Wait for slots; do not duplicate or change clusters based on this global slot limit.

### 2026-09-23T17:32:44Z

- Controlled corrected-depth 2B fixed-prompt async-4 run [01M367PWBF8QDKKX688M7MEZHK](https://beaker.org/ex/01M367PWBF8QDKKX688M7MEZHK) finished 100 updates; head job exited 0 at 17:29:10Z. W&B `ahftfxg3` in-run DAPO 0/20/40/60/80/100: 41.02/6.84/46.48/48.63/53.91/38.09%. Natural stop at 80/100: 72.66/32.62%; final mean DAPO response length 14,165.65. Consumed policy age was five updates at observed 20/40/60/80/100 checkpoints and completed-result stale drops were zero. Fixed prompt batching at corrected depth mitigated the historical near-zero endpoint but did not remove the early crash or late regression; the matched greedy endpoint remains pending.
- Replayed the exact historical Open Instruct sync step-100 greedy evaluation spec (image `01M01EPKXMXR4502S1HNJVYN0M`, historical eval patch/data, base tokenizer and vLLM model, 16K cap, seed 42, weight sync) against the new `step_100` export with fresh run/output identities. Submitted [01M37N523P93CQ3E8FX1T80MH6](https://beaker.org/ex/01M37N523P93CQ3E8FX1T80MH6), W&B `c800ec9b`. Historical Beaker description text was inherited from the source spec; the actual model path and W&B identity in the submitted task are new. Follow its newest job and scheduler events if queued.

### 2026-09-23T17:38:47Z

- The first fixed-depth step-100 greedy eval `01M37N523P93CQ3E8FX1T80MH6` was stopped before model loading: `beaker experiment spec` omitted the historical job's `replicas: 1`, leaving `BEAKER_REPLICA_RANK` unset and `ray_node_setup.sh` starting a worker without a head. Recreated the same eval with `replicas: 1`, fresh W&B ID/output and clean description as [01M37NH10J0HBD9GZKATMVMNFM](https://beaker.org/ex/01M37NH10J0HBD9GZKATMVMNFM), W&B `91b9ae4f`; its Beaker job reports rank 0 and one replica. No training configuration changed.
- Submitted a four-update, one-node fixed-prompt async-depth-4 mechanics smoke [01M37ND0GNVDB63JTET14PK69A](https://beaker.org/ex/01M37ND0GNVDB63JTET14PK69A) with the same committed patch and OPD recipe, changing `inflight_updates=false` and shortening response length to 512 only for the smoke. This tests whether draining active requests before weight publication works before a potential full causal ablation. It was queued at submission; check scheduler events before diagnosing delay.

### 2026-09-23T17:50:22Z

- The depth-4 `inflight_updates=false` mechanics smoke [01M37ND0GNVDB63JTET14PK69A](https://beaker.org/ex/01M37ND0GNVDB63JTET14PK69A) finished four optimizer updates and exited 0 at 17:48:41Z. W&B `wfnlayqk` confirms effective `async_steps=4`, `fixed_prompt_batches=true`, `inflight_updates=false`; logs show an active request draining before weight publication. Its 512-token evaluation is only a mechanics check.
- After checking campaign receipts and Beaker for duplicates, submitted the full 100-update four-node drain ablation [01M37P6A8YJDSY79KE46TKRB01](https://beaker.org/ex/01M37P6A8YJDSY79KE46TKRB01) with committed patch `01M366VFEBYESEQY4G1Y0S59SV`, canonical 16K cap, seed 42, fixed prompt batches, async depth 4, teacher/DPPO/LR unchanged, and fresh name/output. Its only learning-arm config delta from the just-completed fixed-depth run is `inflight_updates=false`. The four jobs were created and queued at submission; scheduler had not yet emitted a constraint. Corrected step-100 greedy evaluation `01M37NH10J0HBD9GZKATMVMNFM` remains running separately.

### 2026-09-23T17:53:56Z

- Corrected matched greedy step-100 evaluation [01M37NH10J0HBD9GZKATMVMNFM](https://beaker.org/ex/01M37NH10J0HBD9GZKATMVMNFM) exited 0 at 17:51:26Z; W&B `91b9ae4f` loaded the completed `step_100` export. DAPO 38.8671875%, MATH-500 76.6%, AIME25 26.6667%, BRUMO25 33.3333%; DAPO natural stopping 64/512 (12.5%) and mean response 15,397.49 tokens. Exact same-harness OI sync is 47.65625% DAPO, 235/512 stop, 12,387.13 tokens; canonical OI async is 0.9765625% DAPO, 2/512 stop, 16,356.02 tokens. Fixed prompt batching at corrected age recovers +37.890625 points versus canonical async but remains −8.7890625 points versus sync. Per-dataset source summaries are in `oi-async-root-cause/matched-greedy-endpoints.json`. This substantially narrows but does not close the Open Instruct/MILES disparity.
- Full `inflight_updates=false` drain ablation `01M37P6A8YJDSY79KE46TKRB01` has been scheduled on Beaker. Its result will test whether avoiding weight changes during active requests closes the remaining gap; no causal conclusion is assigned yet.

### 2026-09-23T18:37:58.476180+00:00

- Full drain ablation [01M37P6A8YJDSY79KE46TKRB01](https://beaker.org/ex/01M37P6A8YJDSY79KE46TKRB01) failed before any optimizer update at 18:12:01Z. Worker job `01M37P6B1DYD1BYPB4VT3GWGMN` tried Ray GCS at leader hostname IP `10.93.1.139:8888`, while rank 0 started the Ray head at `10.93.1.68:8888`. The launcher assumes Beaker leader is rank 0; this placement violated that assumption. Sibling jobs were canceled by propagate-failure. Repair launcher head selection and submit the same learning configuration under a fresh output root; no learning outcome can be inferred from this failure.
- Across 79 matched ordinary optimizer updates (steps 3–99 excluding step IDs ending 0 or 1), W&B measured mean `time/total` 57.02 s and trainer idle wait 0.42 s for corrected fixed-batch async (`ahftfxg3`), versus 123.76 s and 68.03 s for sync (`rg1a2gel`). Canonical async (`v171addf`) was 50.27 s and 0.34 s. The fixed-batch run generated 3.32M step tokens on average versus 2.88M for canonical async, so its 13.4% higher seconds/update cannot be assigned solely to the batching rule. Data: `oi-async-root-cause/fixed-batch-timing-comparison.json`.

### 2026-09-23T18:42:55.597791+00:00
...
### 2026-09-24T02:18:49Z

- Team W&B report draft, at Kevin's request:
