...
class PolicyTrainerRayProcess(RayProcess):
    ...
    def forward(
        ...
        # TODO: we are completely ignoring proper masking practice here.
    ...
    def train(
        ...
        def vllm_generate(
            ...
            for training_step in range(resume_training_step, num_training_steps + 1):
                ...
                with Timer(f"🔥🔥🔥 Generation time", self.rank):
        ...
        kl4_stats = torch.zeros((args.local_total_prompts), device=device)
        ...
        queries_next = global_data[INPUT_IDS_PROMPT_KEY]
        ground_truths_next = global_data[GROUND_TRUTHS_KEY]
        datasets_next = global_data[DATASET_SOURCE_KEY]
        if self.rank == 0:
        ...
        for training_step in range(resume_training_step, args.num_training_steps + 1):
            ...
            if args.async_mode:
                if training_step != 1:
                    ...
                    with Timer(f"🔥🔥🔥 Loading weights using shared memory", self.rank):
                        broadcast_to_vllm()
                if self.rank == 0:
            ...
            with torch.no_grad():
                packed_ref_logprobs = []
                if self.rank == 0:
                    with Timer(f"🔥🔥🔥 Getting response ids", self.rank):
                        global_response_ids = response_ids_Q.get()
                    with Timer(f"🔥🔥🔥 Packing sequences", self.rank):
                        packed_sequences = pack_sequences(
                            queries=queries,
                            responses=global_response_ids,
                            pack_length=args.pack_length,
                            pad_token_id=tokenizer.pad_token_id,
                        )
                    with Timer(f"🔥🔥🔥 decoding responses", self.rank):
                        global_decoded_responses = tokenizer.batch_decode(
                            global_response_ids, skip_special_tokens=True
                        )

                    global_scores = [0] * len(global_response_ids)
                    with Timer(f"🔥🔥🔥 calculating scores", self.rank):
                        ...
                        if args.add_r1_style_format_reward:
                            ...
                            global_scores = [format_scores[i] + global_scores[i] for i in range(len(format_scores))]
                    with Timer(f"🔥🔥🔥 applying verifiable reward", self.rank):
                        if args.apply_verifiable_reward:
                            ...
                            global_scores = [
                                verifiable_rewards[i] + global_scores[i] for i in range(len(verifiable_rewards))
                            ]
                    # with Timer(f"🔥🔥🔥 applying non-stop penalty", self.rank):
                    #     if args.non_stop_penalty:
                    #         pass # TODO: EOS penalty is just a special case of the verifier logic

                    with Timer("🔥🔥🔥 Calculating advantages", self.rank):
                        global_scores = torch.tensor(global_scores, device=device)
                        # calculate the advantages (GRPO change)
                        global_mean_grouped_rewards = global_scores.view(-1, args.number_samples_per_prompt).mean(
                            dim=-1
                        )
                        global_mean_grouped_rewards = global_mean_grouped_rewards.repeat_interleave(
                            args.number_samples_per_prompt, dim=0
                        )
                        global_std_grouped_rewards = global_scores.view(-1, args.number_samples_per_prompt).std(dim=-1)
                        global_std_grouped_rewards = global_std_grouped_rewards.repeat_interleave(
                            args.number_samples_per_prompt, dim=0
                        )
                        global_advantages = (global_scores - global_mean_grouped_rewards) / (
                            global_std_grouped_rewards + 1e-8
                        )

                        global_advantages_lst = global_advantages.tolist()
                        packed_response_masks_cpu = packed_sequences.response_masks
                        packed_sequences.advantages = np.zeros_like(
                            packed_response_masks_cpu, dtype=np.float32
                        )  # important to use float32 for advantages

                        # essentially setting: packed_sequences.advantages[i, j] = global_advantages_lst[packed_response_masks[i, j] - 1]
                        print(f"Shape of packed_response_masks_cpu: {packed_response_masks_cpu.shape}")
                        print(f"Length of global_advantages_lst: {len(global_advantages_lst)}")
                        print(f"Max value in packed_response_masks_cpu: {packed_response_masks_cpu.max()}")
                        for i in range(packed_response_masks_cpu.shape[0]):
                            for j in range(packed_response_masks_cpu.shape[1]):
                                if packed_response_masks_cpu[i, j] >= 1:
                                    packed_sequences.advantages[i, j] = global_advantages_lst[
                                        packed_response_masks_cpu[i, j] - 1
                                    ]
                        packed_sequences.advantages = packed_sequences.advantages

                    objects = [packed_sequences]
                else:
                    objects = [None]
                with Timer("🔥🔥🔥 Broadcasting objects", self.rank):
                    dist.broadcast_object_list(objects, src=0, device=device)
                packed_sequences = objects[0]
                BB = packed_sequences.query_responses.shape[0]  # the global number of samples
                # Distribute samples evenly across ranks using integer division (//).
                # (1) If BB % world_size != 0: this may discard some samples but prevents NCCL hangs from uneven distribution.
                # (2) If BB < world_size, use modulo to ensure each rank gets at least 1 sample.
                # TODO: @vwxyzjn: there is a few possible alternative impls: we could try to fill completely "padding" samples
                # if BB < world_size. We could also just try to generate more samples to "fill" the packed sequences.
                B = max(BB // args.world_size, 1)  # the number of samples per rank
                packed_range_per_rank = range(self.rank * B % BB, (self.rank + 1) * B % BB)
                packed_query_responses = torch.tensor(packed_sequences.query_responses[packed_range_per_rank]).to(
                    device, non_blocking=True
                )
                packed_response_masks = torch.tensor(packed_sequences.response_masks[packed_range_per_rank]).to(
                    device, non_blocking=True
                )
                packed_advantages = torch.tensor(packed_sequences.advantages[packed_range_per_rank]).to(
                    device, non_blocking=True
                )
                if self.rank == 0:
                    print(f"Shape of packed_response_masks: {packed_response_masks.shape}")
                    print(f"Shape of packed_advantages: {packed_advantages.shape}")
                packed_responses = torch.zeros_like(packed_query_responses)
                packed_responses = torch.where(packed_response_masks >= 1, packed_query_responses, args.stop_token_id)
                # TODO: HF's 4d attention masks does not work... so the impl is not correct but ignore it for now
                # attention_masks = torch.tensor(packed_sequences.attention_masks[packed_range_per_rank]).to(
                #     device, non_blocking=True
                # )

                for i in range(0, packed_query_responses.shape[0], args.local_rollout_forward_batch_size):
                    packed_query_response = packed_query_responses[i : i + args.local_rollout_forward_batch_size]
                    packed_response = packed_responses[i : i + args.local_rollout_forward_batch_size]

                    # calculate the logprob of the reference policy
                    packed_ref_logprob = self.forward(
                        self.ref_policy,
                        packed_query_response,
                        packed_response,
                        tokenizer.pad_token_id,
                        args.temperature,
                    )
                    torch.cuda.empty_cache()
                    packed_ref_logprobs.append(packed_ref_logprob)

                packed_ref_logprobs = torch.cat(packed_ref_logprobs, 0)
                del packed_ref_logprob
                gc.collect()
                torch.cuda.empty_cache()
                packed_ref_logprobs = torch.masked_fill(
                    packed_ref_logprobs, ~packed_response_masks.bool(), INVALID_LOGPROB
                )
            ...
            print(f"{B=}, {BB=}, {packed_advantages.shape=}")
            packed_old_logprobs = torch.zeros_like(packed_ref_logprobs)
            for epoch_idx in range(args.num_epochs):
                b_inds = np.random.permutation(B)
                num_complete_minibatches = B // args.local_mini_batch_size

                for minibatch_idx in range(num_complete_minibatches):
                    mini_batch_start = minibatch_idx * args.local_mini_batch_size
                    mini_batch_inds = b_inds[mini_batch_start : mini_batch_start + args.local_mini_batch_size]
                    num_micro_batches = args.local_mini_batch_size // args.per_device_train_batch_size

                    for micro_batch_idx in range(num_micro_batches):
                        micro_batch_start = micro_batch_idx * args.per_device_train_batch_size
                        micro_batch_end = micro_batch_start + args.per_device_train_batch_size
                        micro_batch_inds = mini_batch_inds[micro_batch_start:micro_batch_end]
                        mb_advantage = packed_advantages[micro_batch_inds]
                        mb_responses = packed_responses[micro_batch_inds]
                        mb_query_responses = packed_query_responses[micro_batch_inds]
                        # mb_logprobs = packed_logprobs[micro_batch_inds]
                        mb_reflogprobs = packed_ref_logprobs[micro_batch_inds]

                        new_logprobs = self.forward(
                            self.model, mb_query_responses, mb_responses, tokenizer.pad_token_id, args.temperature
                        )
                        new_logprobs = torch.masked_fill(
                            new_logprobs, ~packed_response_masks[micro_batch_inds].bool(), INVALID_LOGPROB
                        )

                        # cache the old logprobs
                        if epoch_idx == 0:
                            with torch.no_grad():
                                for i, idx in enumerate(micro_batch_inds):
                                    packed_old_logprobs[idx] = new_logprobs[i]
                        logprobs_diff = new_logprobs - packed_old_logprobs[micro_batch_inds]
                        ...
                        loss = masked_mean(
                            pg_loss_max + (args.beta * kl), ~packed_response_masks[micro_batch_inds].bool()
                        )
                        ...
                        with torch.no_grad():
                            ...
                            pg_clipfrac = masked_mean(
                                (pg_losses2 > pg_losses).float(), ~packed_response_masks[micro_batch_inds].bool()
                            )
                            non_score_reward = -args.beta * kl
                            non_score_reward_sum_stats[epoch_idx, minibatch_idx, micro_batch_idx] = (
                                non_score_reward.sum(1).mean()
                            )
                            approxkl = 0.5 * (logprobs_diff**2).mean()
                            approxkl_stats[epoch_idx, minibatch_idx, micro_batch_idx] = approxkl
                            pg_clipfrac_stats[epoch_idx, minibatch_idx, micro_batch_idx] = pg_clipfrac
                            pg_loss_stats[epoch_idx, minibatch_idx, micro_batch_idx] = masked_mean(
                                pg_loss_max, ~packed_response_masks[micro_batch_inds].bool()
                            )
                            loss_stats[epoch_idx, minibatch_idx, micro_batch_idx] = loss
                            ratio_stats[epoch_idx, minibatch_idx, micro_batch_idx] = ratio.mean()
                    # del everything and empty cache
                    # fmt: off
                    del mb_advantage, mb_responses, mb_query_responses, mb_reflogprobs
            ...
            with torch.no_grad():
                ...
                local_metrics.add("objective/entropy", (-packed_old_logprobs).sum(1).mean())
                ...
                reduced_metrics = local_metrics.get_reduced_metrics()
                if accelerator.is_main_process:
                    metrics = {
                        "episode": episode,
                    ...
                    sequence_lengths = [len(response) for response in global_response_ids]
                    metrics["val/sequence_lengths"] = np.mean(sequence_lengths)
                    metrics["val/sequence_lengths_min"] = np.min(sequence_lengths)
                    metrics["val/sequence_lengths_max"] = np.max(sequence_lengths)
                    metrics["objective/scores"] = global_scores.mean().item()
                    metrics["objective/rlhf_reward"] = (
                        metrics["objective/scores"] + metrics["objective/non_score_reward"]
                    )
                    if args.apply_verifiable_reward:
                        metrics["objective/verifiable_correct_rate"] = sum(verifiable_rewards) / len(
                            verifiable_rewards
                        )
                    if args.add_r1_style_format_reward:
                        metrics["val/format_scores"] = sum(format_scores) / len(format_scores)
