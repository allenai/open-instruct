...
and `incorrect` rollouts. Baseline (blue) climbs in accuracy on every reasoning
task while length gradually drops as the model gets more efficient. Treatment
(orange) length collapses to single-digit tokens within the first ~200 steps
across every verifier task; the accuracy curves diverge from baseline at exactly
that point and stay below it for the rest of training. AlpacaEval — which is
absent from the training mix — is the only task where the two runs track each
other closely on both axes.

The plot is regenerated by `cse-579-experiments/plot_trajectories.py` from the
per-step result files in `results/`; the underlying numbers are in
[`results/trajectories.csv`](results/trajectories.csv).
