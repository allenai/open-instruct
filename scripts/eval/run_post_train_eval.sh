...
for TASK in "${TASKS[@]}"; do
    ...
    if [ "$EVALUATE_ON_WEKA" == "true" ]; then
        python oe-eval-internal/oe_eval/launch.py \
            ...
            --task-args "{ \"generation_kwargs\": { \"temperature\": 0.6, \"do_sample\": true, \"max_gen_toks\": ${MAX_LENGTH}, \"truncate_context\": false${STOP_SEQUENCES_JSON} } }" \
            ${HF_UPLOAD_ARG} \
            --gpus "$GPU_COUNT" \
            --use-gantry \
            --gantry-args '{"env-secret": "OPENAI_API_KEY=openai_api_key", "weka": "oe-adapt-default:/weka/oe-adapt-default", "env":"VLLM_ALLOW_LONG_MAX_MODEL_LEN=1"}' \
            ${REVISION_ARG} \
