...
class TermDtype(Enum):
    ...
    def bit_width(self) -> int:
        return (
            16
            if self in {TermDtype.FLOAT16, TermDtype.BFLOAT16}
            else (32 if self == TermDtype.FLOAT32 else 64)
        )
...
class DistributionQuantizationConfig:
    ...
    def from_dict(cls, data: dict) -> "DistributionQuantizationConfig":
        ...
        if terms_raw is None:
            ...
            for term in terms_raw:
                if isinstance(term, int):
                    terms.append(term)
                elif isinstance(term, str) and term in {
                    SpecialTerm.SQRT.value,
                    SpecialTerm.EXP.value,
                }:
        ...
        cfg = cls(
            ...
            residual_bins=[
                QuantizationBin.from_dict(b) for b in data.get("residual_bins", [])
            ],
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            raise ValueError(
                "If exact_k < k, polynomial_terms or residual_bins must be provided."
            )
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "polynomial_terms": [
                t.value if isinstance(t, SpecialTerm) else t
                for t in (self.polynomial_terms or [])
            ],
            "term_dtype": self.term_dtype.value,
            "residual_bins": [
                {
                    "scale_dtype": b.scale_dtype.value,
                    "element_bits": b.element_bits,
