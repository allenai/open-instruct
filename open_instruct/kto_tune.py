...
def encode_with_messages_format(example, tokenizer, max_seq_length, add_bos=False):
    ...
    messages_encoded = encode_messages(messages)
    # labels are useful for working out where the loss is valid.
    return {
        "input_ids": messages_encoded["input_ids"],
        "labels": messages_encoded["labels"],
        "attention_mask": messages_encoded["attention_mask"],
        "label": 1 if bool(label) else 0
    }
...
def main(args: FlatArguments):
    ...
    if "prompt" in raw_datasets["train"].column_names and "completion" in raw_datasets["train"].column_names:
        ...
        raise ValueError("Sorry, chosen/rejected format is not supported for KTO training.")
        # encode_function = partial(
        #     encode_with_messages_format,
        #     tokenizer=tokenizer,
        #     max_seq_length=args.max_seq_length,
        ...
        raise ValueError("You need to have 'prompt' and 'completion' in your column names.")
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                elif args.dpo_loss_type == "kto":
                    with torch.no_grad():
                        reference_chosen_logps, reference_rejected_logps = concatenated_forward(
                            reference_model, batch, average_log_prob=average_log_prob
                        )
                    losses, _, _, _ = kto_loss(
                        policy_chosen_logps,
                        policy_rejected_logps,
                        reference_chosen_logps,
                        reference_rejected_logps,
