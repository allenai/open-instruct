...
def build_slurm_script(args: argparse.Namespace, commands: list[list[str]], experiment_dir: str) -> str:
    ...
    if args.nodes > 1:
        ...
        lines.append("# Get all nodes and head node IP")
        lines.append("nodes=$(scontrol show hostnames $SLURM_JOB_NODELIST)")
        ...
        lines.append("")
        lines.append("# Give head node time to start")
        ...
        lines.append("done")
        lines.append("")
        lines.append("# Wait for workers to connect")
        lines.append("sleep 20")
        lines.append('echo "Checking Ray cluster status..."')
