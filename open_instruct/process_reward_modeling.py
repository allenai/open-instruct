...
from open_instruct.reward_modeling_eval import evaluate, evaluate_prm
...
def main(args: Args, dataset_config: DatasetConfig, model_config: ModelConfig):
    ...
    if config.architectures == "LlamaForCausalLM" and config.bos_token_id == 128000:
        ...
        # tokenizer.add_special_tokens({"end_step_token": "ки"})
    ...
    dataset_processor = PRMDatasetProcessor(tokenizer=tokenizer, config=dataset_config)
    train_dataset = combine_dataset(
        args.dataset_mixer_dict,
        splits=args.dataset_train_splits,
        columns_to_keep=[dataset_config.sft_messages_key, dataset_config.prm_label_key], #[dataset_config.preference_chosen_key, dataset_config.preference_rejected_key],
    )
    ...
    if accelerator.is_main_process:
        ...
        visualize_token(train_dataset[0][INPUT_IDS_KEY], tokenizer) #INPUT_IDS_CHOSEN_KEY
        if args.with_tracking:
    ...
    model: PreTrainedModel = AutoModelForSequenceClassification.from_pretrained(
        model_config.model_name_or_path, revision=model_config.model_revision, num_labels=2
    )
    ...
    # TODO Faeze
    data_collator = SimplePRMCollator(pad_token_id=tokenizer.pad_token_id)
    ...
    for _ in range(args.num_train_epochs):
        for data in dataloader:
            ...
            with accelerator.accumulate(model):
                ...
                ### find label tokens and flatten them into 1D vector to compute loss wrt predicted step logits that are flatten logits for each label position
                ...
                optimizer.step()
                optimizer.zero_grad()
                # Step 1: Get the predicted labels by finding the index of the maximum logit
                _, predicted_labels = torch.max(predcited_step_logits, dim=1)
                # Step 2: Compare the predicted labels with the actual targets
                accuracy = (predicted_labels == flatten_labels).float().mean().item()
            ...
            if args.num_evals > 0 and training_step > 1 and training_step % args.eval_freq == 0:
                eval_metrics, table = evaluate_prm(model, eval_dataloader, tokenizer, max_sampled_texts=10)
                for key in table:
