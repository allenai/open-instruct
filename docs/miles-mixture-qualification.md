...
Fourteen focused CPU tests passed with the real MILES dataset/grouping implementation and real GSM8K, math, and legacy IFEval verifiers. They include duplicate-prompt identities and adversarial target, source, group, token, version, multiplicity, and reward corruption. The full mixed-source GPU generation/training lifecycle remains to be qualified; no mixture job has been launched by this change.
