...
# ECHO smoke test on Beaker: Qwen3.5-4B + tmax-15k, 2 nodes (8 learners + 8 engines).
# Validates the ECHO world-modeling loss (arXiv:2605.24517, --echo_loss_alpha 0.1)
# on the production Terminal RL stack: vanillux sandbox + podman services,
# liger tiled DPPO loss, vllm logprobs, with SP=2 (validates echo mask sharding + SP-reduced metrics on Beaker).
#
# Success criteria after ~5 steps in wandb:
#   - loss/echo_nll_per_token > 0 and trending down
#   - echo/observation_token_count > 0
#   - RL metrics (dppo_mask_frac_kept, ratios) look normal vs a non-echo run.
#
# NOTE: verify MIRROR_URL points at the current live registry mirror before launching
# (mirrors move; check the general-tool-use workload chain).
...
EXP_NAME=swerl_qwen35_4b_echo_smoke_2node_sp2

uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "ECHO loss smoke: tmax-15k DPPO Qwen3.5-4B (2-node; SP=2; echo_loss_alpha=0.1)" \
       --pure_docker_mode \
