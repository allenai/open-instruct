...
def create_loss_function(config: dict[str, Any]) -> tuple[str, float, Any]:
    ...
    if func_name == "kl":
        missing = MissingProbabilityHandling(config.get("missing_probability_handling", "zero"))
        return (
            func_name,
            weight,
            KLDLoss(
...
class DistillationLossComputer:
    ...
    def __init__(self, loss_functions: list[dict[str, Any]], compressor_config: dict[str, Any], vocab_size: int):
        ...
        if self.total_loss_weight <= 0:
            raise ValueError(f"Sum of loss weights must be > 0, got {self.total_loss_weight}")

        if self.has_distill_loss:
            compression_cfg = DistributionQuantizationConfig.from_dict(compressor_config)
            compressor = LogprobCompressor(config=compression_cfg)
            self.signal_source = OfflineSignalSource(
                compressor=compressor, vocab_size=vocab_size, preapplied_temperature=1.0, log_values=True
            )
        else:
    ...
    def get_teacher_signal(self, batch: dict[str, torch.Tensor]) -> SparseSignal:
        if self.signal_source is None:
            raise ValueError("No distillation loss configured, cannot get teacher signal")
