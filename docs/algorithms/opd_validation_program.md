...
- Step 4 arm 2 OPD baseline (Qwen3-4B-Base ← Qwen3-8B on DAPO-Math-14k): attempt 1
  ...
  Decision for Kevin: attempt 3 = OPD with the eos fix on a rebuilt image (one node, ~24h across
  8h windows), then the EOPD arm on the same image; or stop attempt 2 now to free the node.
