#!/bin/bash
# Value warmup + RL: SAE + answer_prefix GT conditioning for 100 value-only
# steps, then 1000 regular policy/value RL steps on Qwen2.5-32B-Base.
...
#                  = 9011200
DDMM=$(date +"%d%m")
exp_name=vip_vpretrain100_rl1k_dapo_sae_ap_${DDMM}_qwen25_32b_math
...
uv run python mason.py \
    ...
    -- source configs/beaker_configs/ray_node_setup.sh \&\& source configs/beaker_configs/code_api_setup.sh \&\& uv run huggingface-cli download Qwen/Qwen2.5-32B --include "*.json" "merges.txt" --max-workers 1 \&\& python open_instruct/grpo_fast.py \
    --exp_name ${exp_name} \
    --beta 0.0 \
    --backend_timeout 240 \
    --async_steps 8 \
