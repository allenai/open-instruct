...
from api_generate import ANTHROPIC_MODEL_LIST, OPENAI_MODEL_LIST, GEMINI_MODEL_LIST, LLMProcessorGemini
...
from open_instruct.rejection_sampling.api_generate import (  # Import your classes
    LLMGenerationConfig,
    LLMProcessorOpenAI, LLMProcessorAnthropic,
)
...
async def generate_with_openai(model_name: str, data_list: list, args: Args, gen_args: GenerationArgs):
    ...
    processor = LLMProcessorOpenAI(config)
    results = await processor.process_batch(data_list, args, gen_args)
    return results


async def generate_with_anthropic(model_name: str, messages: list, args: Args, gen_args: GenerationArgs):
    """
    Generates responses from an Anthropic model.

    Parameters:
        model_name (str): The name of the Anthropic model.
        messages (list): List of prompt messages to send to the model.
        args (Args): Arguments related to model and configuration.
        gen_args (GenerationArgs): Arguments related to the generation process.
    ...
    processor = LLMProcessorAnthropic(config, api_dict)
    results = await processor.process_batch([{"prompt": msg} for msg in messages], args, gen_args)
    return results


async def generate_with_gemini(model_name: str, prompt_token_ids: list, gen_args: GenerationArgs):
    """
    Generates responses from a Gemini model.

    Parameters:
        model_name (str): The name of the Gemini model.
        prompt_token_ids (list): List of token IDs representing the prompts.
        gen_args (GenerationArgs): Arguments related to the generation process.
    ...
    processor = LLMProcessorGemini(config, api_key)
    results = await processor.process_batch([{"prompt_token_ids": pt} for pt in prompt_token_ids], args, gen_args)
    return results
...
def is_local_model(model_name_or_path: str) -> bool:
...
def is_huggingface_model(model_name_or_path: str) -> bool:
    ...
    # Check if the model path is a local directory
    if is_local_model(model_name_or_path):
        return True

    # Check if the model is on Hugging Face Hub
    api_url = f"https://huggingface.co/api/models/{model_name_or_path}"
    try:
...
def process_model(ds, dataset_args, args, gen_args):
    """
    Processes the dataset and generates responses based on the specified model.

    Parameters:
        ds (Dataset): The dataset to be processed.
        dataset_args (DatasetArgs): Arguments related to the dataset, such as split names.
        args (Args): Arguments related to model and configuration.
        gen_args (GenerationArgs): Arguments related to the generation process.

    Returns:
        list: A list of outputs generated by the specified model.
    """
    if args.model_name_or_path in OPENAI_MODEL_LIST:
    ...
    elif args.model_name_or_path in ANTHROPIC_MODEL_LIST:
        ...
        messages = ds[dataset_args.dataset_train_split]["prompt"]
        responses = asyncio.run(generate_with_anthropic(args.model_name_or_path, messages, args, gen_args))
        outputs = [{"outputs": [{"text": response} for response in responses]}]

    elif args.model_name_or_path in GEMINI_MODEL_LIST:
        ...
        outputs = generate_with_gemini(args.model_name_or_path, prompt_token_ids, gen_args)

    elif is_local_model(args.model_name_or_path) or is_huggingface_model(args.model_name_or_path):
        tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path)
        ...
        raise ValueError(f"Model {args.model_name_or_path} is not supported.")
...
def main(args: Args, dataset_args: DatasetArgs, gen_args: GenerationArgs):
    ...
    outputs = process_model(ds, dataset_args, args, gen_args)
