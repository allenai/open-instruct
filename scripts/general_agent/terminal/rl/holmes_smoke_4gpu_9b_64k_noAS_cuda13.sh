...
EXP_NAME=swerl_holmes_smoke_9b_tmax_64k_noAS_cuda13

uv run --no-default-groups --group dev --group cuda13 python mason.py \
       --cluster ai2/holmes \
       --image "$BEAKER_IMAGE" \
       --description "CUDA-13/B300 4-GPU terminal RL smoke, 9B tmax step_360_cg (vanillux, DPPO, SP=2, fa4, NO active_sampling, 64k ctx)" \
       --pure_docker_mode \
