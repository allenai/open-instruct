...
for model_name_or_path in "$@"; do
    ...
    uv run python mason.py \
        --cluster ai2/holmes \
	      --non_resumable \
        --image "$image_name" \
        --description "Running Holmes single GPU non-eager benchmark with response length of $response_length at commit $git_hash on branch $git_branch with model $model_name_or_path." \
        --pure_docker_mode \
