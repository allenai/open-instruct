...
- When a Beaker job stays queued or pending, run `beaker job events <job-id>` before diagnosing why. It prints the scheduler's own reason; don't infer one from cluster documentation. A common reason is the workspace slot limit (`workspace "ai2/open-instruct-dev" using 64/64 allowed unallocated slots`), which applies workspace-wide and so blocks every cluster at once — relaunching elsewhere cannot help, so wait or request fewer GPUs.
