...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Type of rubrics to use: all, positive, negative.",
    )
    ...
    all_positive_rubrics = []
    all_negative_rubrics = []
    for item in data:
        for rubric in item["adaptive_rubric_scores"][0]["positive_rubrics"]:
            all_positive_rubrics.append(rubric["description"])
        for rubric in item["adaptive_rubric_scores"][0]["negative_rubrics"]:
            all_negative_rubrics.append(rubric["description"])
    print(len(all_positive_rubrics))
    print(len(all_negative_rubrics))
    
    # randomly sample num-criteria criteria from all_positive_rubrics and all_negative_rubrics
    # all_rubrics = all_positive_rubrics + all_negative_rubrics
    if args.rubrics_type == "all":
        all_rubrics = all_positive_rubrics + all_negative_rubrics
    elif args.rubrics_type == "positive":
        all_rubrics = all_positive_rubrics
    elif args.rubrics_type == "negative":
        all_rubrics = all_negative_rubrics
    else:
        raise ValueError(f"Invalid rubrics type: {args.rubrics_type}")
    random.seed(42)
    random.shuffle(all_rubrics)
    sampled_rubrics = all_rubrics[:args.num_criteria]
    print(len(sampled_rubrics))
    
    
    hierarchy = run_example(
        rubric_id="toy_rag_survey_bs_1_rollout_8_1_sample_search_based__1__1761283810",
        num_criteria=args.num_criteria,
        n_top_clusters=8,
        num_levels=4,
        diversity_sample_size=20000,
        output_root=os.path.join("outputs", "rubric_analysis", args.rubrics_type),
        criteria=sampled_rubrics,
    )
