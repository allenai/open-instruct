...
EXP_NAME=swerl_qwen35_4b_opd_from_tmax9b_256x1_4node_64k
...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "OPD shape: 256x1 rollouts (max prompt diversity) base Qwen3.5-4B <- tmax-9b, pure distill; vs the 8x32 arm; 4-node; 64k${RESUME_NOTE}" \
       --pure_docker_mode \
       --workspace ai2/oe-agents \
       --priority urgent \
       --preemptible \
       ...
       --env MIRROR_URL=jupiter-cs-aus-165.reviz.ai2.in:5000 \
