...
# OPD: pure-distill the released tmax-9b teacher into the released (already
# RL-trained) tmax-4b student on tmax-15k terminal tasks. Completes the OPD
# experiment matrix: vs tmax-27b->tmax-4b it isolates teacher SIZE; vs
# tmax-9b->base-4B it isolates whether the student being RL'd (having
# competence to unlearn) causes the cross-policy transition trough.
# tmax-9b and tmax-4b share attention geometry -> sharded teacher path.
#
...
EXP_NAME=swerl_tmax4b_opd_from_tmax9b_4node_64k

uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "OPD: tmax-4b student <- tmax-9b teacher (RLd student, moderate foreign teacher; 4-node; 64k)" \
       --pure_docker_mode \
