...
def get_target_logprobs(
    ...
    temperature_change = not math.isclose(distillation_temperature, target_generation_temperature)
    if log_target:
        if not temperature_change and missing == MissingProbabilityHandling.SYMMETRIC_UNIFORM:
            return values_in
        ...
        alpha_lse = torch.logsumexp(alpha_values_in - max_alpha, dim=-1, keepdim=True) + max_alpha
    ...
    logits = (
        values_in * (target_generation_temperature / distillation_temperature) if temperature_change else values_in
    )
    sparse_max = torch.max(logits, dim=-1, keepdim=True).values
    sparse_lse = (torch.logsumexp((logits - sparse_max).to(torch.float32), dim=-1, keepdim=True) + sparse_max).to(
        values_in.dtype
    )
...
def get_logprobs(
    ...
    if (not log_target) and (missing != MissingProbabilityHandling.ZERO):
        raise ValueError("MissingProbabilityHandling.SYMMETRIC_UNIFORM is only valid for log_target=True")
    ...
    student_lse = torch.logsumexp(logits.to(torch.float32), dim=-1, keepdim=True).to(out_dtype)
...
def accumulate_over_chunks(
    ...
    **kwargs,
    ...
    for start_idx in range(0, seq_len, chunk_length):
        cur_mask = mask[:, start_idx : start_idx + chunk_length] if mask is not None else None
