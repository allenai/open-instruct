...
class VLLMGenerator:
    def __init__(
        ...
        self.queue_pair = QueuePair(
            param_query_Q=Queue(maxsize=1),
    ...
    def generate(self):
        ...
        for _ in range(1, self.num_training_steps + 1):
            # NOTE: we need `self.queue_pair.param_query_Q.get()` to block for `train` queue
            # but not for `eval` queue
            task_name, unwrapped_model, g_queries_list, generation_config = self.queue_pair.param_query_Q.get()
            ...
            response_token_ids = [list(output.outputs[0].token_ids) for output in outputs]
            print(f"putting response_token_ids")
            self.queue_pair.response_ids_Q.put(task_name, response_token_ids)
            print(f"putted response_token_ids")

    def start_generation_thread(self):
        self.thread = threading.Thread(target=self.generate)
        self.thread.start()

    def send_param_query(self, task_name: str, unwrapped_model: PreTrainedModel, queries_list: List[List[int]], generation_config: SamplingParams) -> None:
        # remove padding
        queries_list = [
            [token for token in query if token != self.pad_token_id]
            for query in queries_list
        ]
        self.queue_pair.param_query_Q.put((task_name, unwrapped_model, queries_list, generation_config))

    def get_responses(self):
        return self.queue_pair.response_ids_Q.get()
...
def main(args: Args, dataset_config: DatasetConfig, model_config: ModelConfig):
    ...
    eval_generation_config = SamplingParams(
        temperature=(0.01 + 1e-7), # evaluation temperature
        top_p=1.0,
        max_tokens=args.response_length,
        include_stop_str_in_output=True,
    )
    max_sampled_texts = 10
    eval_data = eval_dataset[:max_sampled_texts * args.world_size][INPUT_IDS_PROMPT_KEY]
    vllm_generator = None
    if accelerator.is_main_process:
        ...
        vllm_generator.start_generation_thread()
    ...
    g_queries_next = gather_object(queries_next.tolist())
    if accelerator.is_main_process: vllm_generator.send_param_query("train", None, g_queries_next, generation_config)
        ...
        with unwrap_model_for_generation(model, accelerator) as unwrapped_model:
            ...
            if args.async_mode:
                if training_step != 1:
                    ...
                    queries_next = queries_next.repeat(args.num_generation_per_prompt, 1)
                    g_queries_next = gather_object(queries_next.tolist())
                if accelerator.is_main_process: vllm_generator.send_param_query("train", generation_model, g_queries_next, generation_config)
            else:
                if training_step != 1:
                    data = next(iter_dataloader)
                    queries_next = data[INPUT_IDS_PROMPT_KEY].to(device)
            ...
            with torch.no_grad():
                g_response_token_ids = vllm_generator.get_responses("train")
                ...
                if accelerator.is_main_process:
                    ...
                    g_padded_response_ids = [response + [DUMMY_PAD_TOKEN] * (args.response_length - len(response)) for response in g_response_token_ids]
                ...
                local_vllm_responses = g_vllm_responses[accelerator.local_process_index * queries.shape[0] : (accelerator.local_process_index + 1)* queries.shape[0]]
