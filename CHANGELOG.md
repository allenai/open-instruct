...
### Changed
- Switch CLI argument parsing from `transformers.HfArgumentParser` / `ArgumentParserPlus` to `tyro` via a new `open_instruct.parsing.parse` helper. Gives real static typing for `Literal`/`Union`/`Enum`/nested dataclasses, removes per-call `assert isinstance` boilerplate, and keeps the existing flat CLI surface and YAML-config behaviour.
