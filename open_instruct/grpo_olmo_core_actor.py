...
class PolicyTrainerOLMoCoreProcess(RayProcess):
    ...
    def fit(self) -> dict:
        ...
        trainer_callbacks["vllm_sync"] = VLLMWeightSyncCallback(
            ...
            weight_state_provider=(
                self._gather_olmo_ddp_hf_state
                if self.grpo_config.olmo_core_train_module == "ddp"
        ...
        trainer_callbacks["checkpointer"] = olmo_core_utils.build_checkpointer_callback(
            checkpointing_steps=(
                self.grpo_config.checkpoint_state_freq if self.grpo_config.checkpoint_state_freq > 0 else None
            ),
    ...
    def save_model(
        ...
        if self.grpo_config.olmo_core_train_module == "ddp":
            assert self.hf_config is not None
            if self.hf_config.model_type == "olmo3moe":
                hf_state = gather_olmo_ddp_hf_state_to_cpu(self.train_module.model, self.hf_config)
            else:
