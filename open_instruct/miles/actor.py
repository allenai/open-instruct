...
class OLMoCoreTrainRayActor(TrainRayActor):
    ...
    def _train_steps(self, rollout, rollout_id, local_batch, decision, checked, difference, agreement, profile):
        ...
        for start in range(0, len(batches), local_batch):
            ...
            if capture:
                training_scores = [score for batch in step_batches for score in batch.pop("training_log_probs")]
                if checked:
                    ...
                    sums, maximum = contract.scoring_check(
                        rollout["log_probs"], training_scores, rollout["loss_masks"]
                    )
                    dist.all_reduce(sums)
                    dist.all_reduce(maximum, op=dist.ReduceOp.MAX)
                    report = self._agree(
                        lambda sums=sums, maximum=maximum: contract.validate_scoring_check(
                            sums, maximum, self.args.olmo_core.scoring_check_tolerance
                        )
                    )
                    logger.info(
