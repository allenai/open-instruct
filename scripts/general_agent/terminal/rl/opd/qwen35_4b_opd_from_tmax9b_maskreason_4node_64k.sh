...
# OPD H2 test: does distilling the KL on REASONING tokens hurt at cross-size?
#
# Identical to qwen35_4b_opd_from_tmax9b_4node_64k.sh except --opd_mask_reasoning:
# the reverse-KL advantage is zeroed on every token inside <think>...</think>
# spans, so distillation trains only action/tool-call/answer tokens. Compare
# directly against the existing unmasked 4B <- tmax-9b arm (best TB2.1 0.202,
# KL floor ~0.095). objective/opd_reasoning_masked_frac reports the masked
# share; objective/opd_reverse_kl stays computed over ALL tokens so the KL
# curves remain comparable across the two arms.
#
# Workspace: ai2/oe-agents (the H1 raw-teacher arm runs on general-tool-use).
...
EXP_NAME=swerl_qwen35_4b_opd_from_tmax9b_maskreason_4node_64k
...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "OPD H2: base Qwen3.5-4B student <- tmax-9b teacher with reasoning-token KL masking (does OPD on think-tokens hurt cross-size? vs the unmasked arm; 4-node; 64k)${RESUME_NOTE}" \
       --pure_docker_mode \
