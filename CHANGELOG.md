...
### Added
- Add an opt-in tiled GRPO lm-head loss (`--use_liger_grpo_loss`) that recomputes the lm-head projection and loss tile-by-tile to avoid materializing full-vocabulary logits, reproducing the DAPO/CISPO objective with much lower peak memory (https://github.com/allenai/open-instruct/pull/1747).
