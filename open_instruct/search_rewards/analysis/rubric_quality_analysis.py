...
def score_factuality(
    ...
    FACTUALITY_EVAL_PROMPT_LIST_CRITERIA = (
        ...
        "\n   - Example: 'Mention information about A' → NA (it only requires mentioning, not asserting a fact)."
        "\n   - Example: 'State that A is B' → factual claim (it asserts something verifiable about A)."
        "\n3. If the criterion is about writing style, tone, clarity, organization, or includes only vague references "
        "to what content should appear (without asserting factual content), return 'NA'."
        "\n4. If the criterion includes a factual claim (something that can be verified as true or false), evaluate "
        "its factual correctness based on reliable evidence or reasoning."
        "\n5. Assign a factuality score as follows:"
        "\n   - 1 → All factual criteria are correct."
        "\n   - Between 0 and 1 → Some factual criteria are correct, others are incorrect (average the factual scores)."
        "\n   - 0 → All factual criteria are incorrect."
        "\n   - 'NA' → None of the criteria contain factual claims (i.e., they only describe style, tone, clarity, or "
        "general inclusion without asserting any verifiable facts)."
        "\n6. When in doubt:"
        "\n   - If the criterion asks for inclusion of certain *information* without specifying its correctness, choose 'NA'."
        "\n   - If the criterion specifies *what the information should be* (e.g., 'A causes B', 'A is located in X'), "
        "then treat it as a factual claim."
        "\n7. You should perform a search to confirm the factual claims when they involve specific entities, numbers, or details. "
        "If the information cannot be found, assign a neutral score of 0.5."
        "\n8. Also report how many criteria were considered factual (non-NA) and how many were NA."
        "\n\nOutput Format:"
        ...
        try:
            ...
            if json_match:
                ...
                if score_match:
                    ...
                    return {"factual_score": score_value, "explanation": explanation}
                else:
