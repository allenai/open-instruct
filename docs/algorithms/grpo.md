...
| | `--off_policy_correction` | Off-policy correction mode: `none`, `importance_sampling`, `truncated_importance_sampling`, `icepop`, or `kpop` | `truncated_importance_sampling` |
| | `--rho_clamp_lower_bound` / `--rho_clamp_upper_bound` | Rho clamp bounds for truncated importance sampling | `0.0` / `2.0` |
| | `--rho_mask_lower_bound` / `--rho_mask_upper_bound` | Rho mask bounds for the IcePop mode | `0.0` / `0.0` |
| | `--kpop_binary_kl_upper_bound` | Binary-KL threshold for KPop | `0.0` |
| | `--kpop_binary_kl_direction` | Binary-KL direction for KPop: `symmetric`, `fwd`, or `rev` | `symmetric` |
...
GRPO can train on completions generated by a separate or stale inference engine. When the training policy and inference policy assign different probabilities to the sampled response tokens, the policy-gradient update is off-policy. The off-policy correction code uses token logprobs from both sides to decide how each response token contributes to the loss.

For response token `y_t`:

- `p_t = pi_train_old(y_t | x, y_<t)`: probability recomputed by the training model.
- `q_t = pi_infer_old(y_t | x, y_<t)`: probability recorded by the inference engine.
- `rho_t = p_t / q_t`: token-level importance ratio.

The `off_policy_correction` mode controls the correction algorithm:

| Mode | Filter | Weight on kept tokens | Main parameters |
|------|--------|-----------------------|------------|
| `none` | none | `1` | none |
| `importance_sampling` | none | `rho` | none |
| `truncated_importance_sampling` | none | clipped `rho` | `rho_clamp_*` |
| `icepop` | `rho` bounds | `rho` | `rho_mask_*` |
| `kpop` | symmetric binary KL | `rho` | `kpop_binary_kl_*` |

### IcePop

IcePop filters tokens using bounds on the token-level importance ratio `rho_t`. It keeps a response token when the ratio between the training-side old policy and the inference-side behavior policy is inside the configured interval:

```text
keep_t =
rho_mask_lower_bound <= rho_t <= rho_mask_upper_bound
```

where:

```text
rho_t = pi_train_old(y_t | x, y_<t) / pi_infer_old(y_t | x, y_<t)
```

Kept tokens are weighted by `rho_t`; dropped tokens receive zero policy-loss weight.

IcePop is described in "Every Step Evolves: Scaling Reinforcement Learning for Trillion-Scale Thinking Model" ([arXiv:2510.18855](https://arxiv.org/abs/2510.18855)).

Example IcePop config:

```yaml
off_policy_correction: icepop
rho_mask_lower_bound: 0.5
rho_mask_upper_bound: 5.0
```

IcePop emits rho-bound correction metrics:

| Metric | Meaning |
|--------|---------|
| `val/rho_weight` | Actual policy-loss multiplier after masking |
| `val/rho_drop_frac` | Fraction of response tokens dropped by the rho-bound mask |
| `val/rho_drop_low_frac` | Fraction of response tokens dropped because `rho_t` is below `rho_mask_lower_bound` |
| `val/rho_drop_high_frac` | Fraction of response tokens dropped because `rho_t` is above `rho_mask_upper_bound` |
| `val/rho_hist` | Histogram of raw importance ratios |

### KPop

KPop filters tokens using binary KL between the training-side old policy and the inference-side behavior policy. For a sampled token, KPop treats the vocabulary as a binary event: the sampled token versus all other tokens.
...
The symmetric KPop keep rule is:
...
where `phi` is `kpop_binary_kl_upper_bound`. KPop is described in "Ling and Ring 2.6 Technical Report: Efficient and Instant Agentic Intelligence at Trillion-Parameter Scale" ([arXiv:2606.15079](https://arxiv.org/abs/2606.15079)).

Example KPop config:

```yaml
off_policy_correction: kpop
kpop_binary_kl_upper_bound: 2.0
```

For MoE models, KPop does not replace separate router-load or expert-balance diagnostics.

KPop emits these correction metrics:

| Metric | Meaning |
|--------|---------|
| `val/rho_weight` | Actual policy-loss multiplier after masking |
| `val/binary_kl_fwd` | `KL_B(pi_train_old || pi_infer_old)` |
| `val/binary_kl_rev` | `KL_B(pi_infer_old || pi_train_old)` |
| `val/binary_kl_max` | `max(fwd, rev)`, the thresholded KPop statistic |
| `val/binary_kl_drop_frac` | Fraction of response tokens dropped by the KPop mask |
| `val/rho_hist` | Histogram of raw importance ratios |
| `val/binary_kl_hist` | Histogram of `binary_kl_max` |

`val/rho_drop_low_frac`, `val/rho_drop_high_frac`, and `val/rho_clipfrac` belong to rho-bound masking or rho clipping modes. They are not emitted for KPop because KPop drops tokens by binary KL, not by rho lower/upper bounds.
