...
def run_benchmark(dataset: Any, vllm_engines: List[Any], args: Args, model_flops_per_token: float, gpu_peak_flops: float, num_batches: int) -> List[Dict[str, Any]]:
    ...
    for batch_idx in range(num_batches):
        ...
        # Run generation
        batch_result = run_generation_batch(vllm_engines, args, model_flops_per_token, gpu_peak_flops, prompts, batch_idx)
...
def main() -> None:
    ...
    # Parse args to check which were explicitly provided
    import sys
    provided_args = set()
    for i, arg in enumerate(sys.argv[1:]):
        if arg.startswith('--'):
            provided_args.add(arg.lstrip('-'))
    
    args, tokenizer_config, model_config = parser.parse_args_into_dataclasses()
    
    # Hardcoded benchmark-specific values
    num_batches = 5
    max_model_len = 20480
    ...
    # Override dataset defaults for benchmark if not specified
    if args.dataset_mixer_list == ["ai2-adapt-dev/rlvr_gsm8k_zs", "1.0"]:  # default value
        args.dataset_mixer_list = ["hamishivi/hamishivi_rlvr_orz_math_57k_collected_all_filtered_hamishivi_qwen2_5_openthoughts2", "1.0"]
    
    # Override other defaults for benchmark
    if args.max_token_length == 512:  # default value
        args.max_token_length = 10240
    if args.max_prompt_token_length == 256:  # default value
        args.max_prompt_token_length = 2048
    if args.response_length == 256:  # default value
        args.response_length = 16384
    if args.temperature == 0.7:  # default value
        args.temperature = 1.0
    if args.vllm_top_p == 1.0:  # default value
        args.vllm_top_p = 0.9
    if args.num_unique_prompts_rollout == 16:  # default value
        args.num_unique_prompts_rollout = 256
    if 'num_samples_per_prompt_rollout' not in provided_args and args.num_samples_per_prompt_rollout == 4:  # default value
        args.num_samples_per_prompt_rollout = 16
    if 'vllm_num_engines' not in provided_args and args.vllm_num_engines == 1:  # default value
        args.vllm_num_engines = 8
    if args.dataset_local_cache_dir == "local_dataset_cache":  # default value
        args.dataset_local_cache_dir = "benchmark_cache"
    if args.seed == 1:  # default value
        args.seed = 42
    ...
    print(f"vLLM GPU memory utilization: {args.vllm_gpu_memory_utilization}")
    print(f"Temperature: {args.temperature}")
    print(f"Max tokens: {args.response_length}")
    print(f"Top-p: {args.vllm_top_p}")
    print(f"Num batches: {num_batches}")
    print(f"Batch size: {args.num_unique_prompts_rollout}")
    print(f"Num samples per prompt: {args.num_samples_per_prompt_rollout}")
    print(f"Dataset mixer list: {args.dataset_mixer_list}")
    print(f"Dataset mixer list splits: {args.dataset_mixer_list_splits}")
    print(f"Max token length: {args.max_token_length}")
    print(f"Max prompt token length: {args.max_prompt_token_length}")
    print(f"Chat template name: {tokenizer_config.chat_template_name}")
    print(f"Add BOS: {tokenizer_config.add_bos}")
    print(f"Num engines: {args.vllm_num_engines}")
