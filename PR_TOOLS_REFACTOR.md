...
```
tools/
├── __init__.py       # Clean exports for the package
├── utils.py          # Tool, ToolCall, ToolOutput, BaseToolConfig base classes
├── config.py         # ToolArgs (CLI), ToolConfig (internal), TOOL_REGISTRY, create_tool_parser
├── parsers.py        # ToolParser ABC + implementations (Vllm, Legacy, DRTulu) + VLLM_PARSERS registry
├── proxy.py          # ToolProxy + ToolActor (Ray wrapper pattern)
├── tools.py          # Actual tool implementations (PythonCodeTool, SerperSearchTool, etc.)
└── code_server/      # Python execution server
```
