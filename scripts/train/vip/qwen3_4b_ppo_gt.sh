#!/bin/bash
# Qwen3-4B-Base PPO with ground truth conditioning on math data (100 steps value pretrain)
# Value model receives GT answer as privileged info for better value estimation.
# 1 node (8 GPUs): 4 learner + 4 vLLM engines, 8k seqlen
# Requires hamish/ppo-value-model branch
DDMM=$(date +"%d%m")
exp_name=vip_ppo_gt_${DDMM}_qwen3_4b_math_8k
