...
Updated 2026-09-21 00:15Z (was 2026-09-20 07:26Z; details in the latest Log entries).

- **Three jobs running (2026-09-21 00:15Z), all wrapper code overlaid from Miles `14080e06c` on
  image `01M2V7V946ZN9N9STPK0H04YWM`:** (1) the arm 2 corrected continuation
  `01M30JAV4K6F2F7XQYV1HY0R6N` (revision `20763dfc2`) is healthy — W&B run `c4four8o` resumed at
  `train/step` 720 with `lr-pg_0` 2.35e-7, exactly the cosine value for step 720 of 880 (the
  LR-0 bug is gone); rollouts 180-219, eval 219 and the audit due ~03:30Z. (2) **Kevin
  approved both remaining arms ("ok let's run both of them with the fixes")**: arm 1 (Qwen3-1.7B-Base
  ← Qwen3-8B on MATH, 176 rollouts, paper target 67.8) launched as `01M30N24RT0JCCGAXPPG0X4F08`
  (root `-v2`), and (3) the EOPD arm's smoke on the paper-fidelity path (trainer-side student
  log-probs, token-mean loss, EOPD on) as `01M30N219Q0X3JG4YAT8HGC9YZ` (tiny root `-v2`); the
  full **arm 2 EOPD** run (`configs/miles/opd/eopd-eopd-qwen3-4b-base-dapo14k.toml`, alpha 1.0,
  tau 0.8, k 16, root `-v1`) launches when that smoke passes. New `training.keep_checkpoints`
  (Miles `14080e06c`) keeps only the newest 2 Megatron saves on the new arms (Weka bucket at
  100 %, 3.1T free at 00:08Z). wandb's API works from Kevin's laptop (the "geo-blocked" note
  was stale), so metrics are pulled locally now.
