...
def _work_dtype(*inputs: torch.Tensor | None) -> torch.dtype:
    """Select a stable compute dtype, promoting to float64 when present."""
    for x in inputs:
...
def _solve_least_squares(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    """Solve least-squares with SVD and a small singular-value threshold."""
...
def polynomial_terms(
    ...
    """Construct polynomial/special basis terms used for approximation."""
    if normalize_t:
...
def fit_polynomial(
    ...
    """Fit basis coefficients and return the residual to the fitted curve."""
...
def _get_quantize_range(element_bits: int) -> tuple[int, float]:
    """Return min/max representable quantized integer values."""
    if element_bits == 1:
        return 0, 1
...
def _get_quantize_scale_factors(values: torch.Tensor, element_bits: int) -> tuple[torch.Tensor, float]:
    """Compute per-row scaling stats for symmetric quantization."""
...
def error_diffuse_float(
    ...
    """Apply sequential error diffusion while casting to a lower-precision dtype."""
...
def quantize_naive(
    ...
    """Naively quantize values with per-row scales and symmetric integer bins."""
    work_dtype = _work_dtype(values)
    values = values.to(work_dtype)
...
def dequantize(quantized_values: torch.Tensor, scale: torch.Tensor, element_bits: int) -> torch.Tensor:
    """Recover floating values from quantized integers and per-row scales."""
    if element_bits == 1:
...
def compress_monotonic_logprobs(logprobs: torch.Tensor, config: DistributionQuantizationConfig) -> torch.Tensor:
    """Compress sorted logprobs into a compact byte representation."""
    ...
    chunks = []
    if config.exact_k > 0:
        exact_values = logprobs[..., : config.exact_k].to(config.exact_dtype)
        chunks.append(exact_values.contiguous().view(torch.uint8).reshape(*logprobs.shape[:-1], -1))

    if config.polynomial_terms:
        approx_values = logprobs[..., config.exact_k : config.k]
        coeffs, residual = fit_polynomial(
            approx_values, config.polynomial_terms, dtype=config.term_dtype, normalize_t=config.normalize_t
        )
        coeff_bytes = coeffs.to(config.term_dtype).contiguous().view(torch.uint8).reshape(*logprobs.shape[:-1], -1)
    ...
    for bin_ in config.residual_bins:
        ...
        scale, scaled, _ = quantize_naive(values, bin_.element_bits, bin_.scale_dtype, None)
        scale_bytes = scale.to(bin_.scale_dtype).contiguous().view(torch.uint8).reshape(*logprobs.shape[:-1], -1)
...
def decompress_monotonic_logprobs(
    bytes_: torch.Tensor, config: DistributionQuantizationConfig, out_dtype: torch.dtype | None = None
) -> torch.Tensor:
    """Decode bytes from `compress_monotonic_logprobs` back into logprobs."""
    device = bytes_.device
    if out_dtype is None:
        out_dtype = config.exact_dtype

    if config.exact_k > 0:
        exact_dtype_torch = config.exact_dtype
        bytes_per_exact = torch_dtype_bit_width(config.exact_dtype) // 8
    ...
    if config.polynomial_terms:
        ...
        coeff_bytes = terms_count * (torch_dtype_bit_width(config.term_dtype) // 8)
        coeff_part = remaining_bytes[..., :coeff_bytes].contiguous()
        coeffs = coeff_part.view(dtype=config.term_dtype).reshape(*remaining_bytes.shape[:-1], terms_count)
    ...
    residuals = []
    for bin_ in config.residual_bins:
        scale_bytes = torch_dtype_bit_width(bin_.scale_dtype) // 8
        scale_part = remaining_bytes[..., :scale_bytes].contiguous()
        scale = scale_part.view(dtype=bin_.scale_dtype).reshape(*remaining_bytes.shape[:-1], 1)
