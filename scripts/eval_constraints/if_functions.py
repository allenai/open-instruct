...
def verify_keywords(response, keyword_list):
	"""
...
def validate_title(answer: str) -> bool:
	pattern = r"<<(.*?)>>"
...
def validate_choice(answer: str, options: list) -> bool:
...
# section*
def validate_highlighted_sections(answer: str, N: int) -> bool:
	pattern = r"\*(.*?)\*"
	matches = re.findall(pattern, answer)
...
def validate_sections(answer: str, N: int, section_splitter: str) -> bool:
	sections = answer.split(section_splitter)
...
def validate_repeat_prompt(full_response: str, original_prompt: str) -> bool:
	response_split = full_response.split('. ', 1)
	if response_split[0] == original_prompt:
		return True
	else:
		return False
...
# symbols: ******.
def validate_two_responses(responses: str) -> bool:
...
def validate_uppercase(response: str) -> bool:
...
# allowed.
def validate_lowercase(response: str) -> bool:
...
# at most {N} times.
def validate_frequency_capital_words(response: str, N: int, quantifier: str) -> bool:
...
def validate_end(response: str, end_phrase: str) -> bool:
	# Check if the response ends with the end phrase
	if response.endswith(end_phrase):
		return True
	else:
		return False
...
def validate_no_commas(response: str) -> bool:
	if ',' not in response:
		return True
	else:
		return False
