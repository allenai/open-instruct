...
def build_slurm_script(args: argparse.Namespace, commands: list[list[str]], experiment_dir: str) -> str:
    ...
    if args.nodes > 1:
        ...
        lines.append("    if ray status 2>/dev/null | grep -q 'node'; then")
        lines.append('        echo "Ray cluster is ready!"')
        lines.append("        break")
        lines.append("    fi")
        ...
        lines.append("        ray status || true")
        lines.append("        exit 1")
        lines.append("    fi")
        lines.append('    echo "Waiting for Ray... (${RAY_ELAPSED}s elapsed)"')
