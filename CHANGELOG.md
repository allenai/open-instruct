...
### Changed
- Repoint `ai2-olmo-core` to the OLMo-core `vision` branch (`c54d228`), which adds the multimodal model, train module, and data pipeline consumed by the multimodal SFT design (`docs/design/multimodal_sft.md`); add a guarded import smoke test (`open_instruct/test_olmo_core_vision_smoke.py`) that pins the multimodal API contract at CI time; extend `DPOLMHead.forward` to accept the branch's new `response_logits_only`/`response_mask` LM-head kwargs; run CI on PRs targeting the `vision` branch (https://github.com/allenai/open-instruct/pull/1834).
