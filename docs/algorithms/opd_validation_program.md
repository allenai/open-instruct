...
- **Three training jobs running (2026-09-21 00:32Z), wrapper code overlaid on image
  `01M2V7V946ZN9N9STPK0H04YWM`:** (1) the arm 2 corrected continuation
  ...
  log-probs, token-mean loss, EOPD on) as `01M30N219Q0X3JG4YAT8HGC9YZ` (tiny root `-v2`) —
  **passed end to end at 00:28Z** (audit: EOPD block, advantage error 0, 271 changed tensors,
  export reload; LR on the cosine curve), so the full **arm 2 EOPD** run
  (`configs/miles/opd/eopd-eopd-qwen3-4b-base-dapo14k.toml`, alpha 1.0, tau 0.8, k 16, root
  `-v1`) **launched 00:32Z as `01M30P1BP5N6SJVSRJMNMQHN7A`**. New `training.keep_checkpoints`
