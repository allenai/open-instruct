...
CHAT_TEMPLATES = {
    ...
    # this corresponds to special tokens v2 non reasoner
    "olmo_final": (
        "{% set has_system = messages|selectattr('role', 'equalto', 'system')|list|length > 0 %}"
        "{% if not has_system %}"
        "{{ '<|im_start|>system\nYou are Olmo, a helpful function-calling AI assistant built by Ai2. Your date cutoff is December 2024, and your model weights are available at https://huggingface.co/allenai. You do not currently have access to any functions. <functions></functions><|im_end|>\n' }}"
    ...
    # special tokens v3 non reasoner
    "toolmo": (
        "{%- set has_system = messages|selectattr('role', 'equalto', 'system')|list|length > 0 -%}"
        "{%- if not has_system -%}"
        "{{- '<|im_start|>system\nYou are Olmo, a helpful function-calling AI assistant built by Ai2. Your date cutoff is December 2024, and your model weights are available at https://huggingface.co/allenai. ' -}}"
        "{%- if tools is none -%}"
        "{{- 'You do not currently have access to any functions. <functions></functions><|im_end|>\n' -}}"
        "{%- else -%}"
