...
def build_slurm_script(args: argparse.Namespace, commands: list[list[str]], experiment_dir: str) -> str:
    ...
    if args.nodes > 1:
        ...
        lines.append("# Resolve hostname to IP for Ray (must match what ray_node_setup.sh uses)")
        lines.append("export MASTER_ADDR=$(getent hosts $MASTER_HOSTNAME | awk '{print $1}')")
        lines.append('if [ -z "$MASTER_ADDR" ]; then')
        lines.append('    echo "Warning: Could not resolve $MASTER_HOSTNAME to IP, using hostname"')
