...
### Changed
- Pull GRPO checkpoint-state save into `one_training_step` so it counts toward `time/saving`, log checkpoint size and bandwidth, write driver state (dataloader + data-prep actor) once via rank 0 to a sibling `driver_state.pt`, gate ref-policy rewrites on the same `should_save_ref_policy` predicate as the EMA update, and drop the redundant per-device CUDA RNG dict (https://github.com/allenai/open-instruct/pull/1647).
