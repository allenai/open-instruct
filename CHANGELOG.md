...
### Added
- Exclude infrastructure-failed rollouts (Spot-preempted sandboxes, reset failures, circuit-breaker trips) from the GRPO loss via `--mask_infra_failed_completions`: group advantage statistics are computed over surviving samples only and the failed rollouts are dropped from the batch; `val/infra_failed_rate` is reported either way (https://github.com/allenai/open-instruct/pull/1887).
- Add `OpenSandboxBackend`, a `SandboxBackend` that runs RL sandboxes on a self-hosted [OpenSandbox](https://open-sandbox.ai) service (e.g. GKE Autopilot). Unlike Modal, the OCI image is pulled as-is (no rebuild/fidelity gap). Select it with `backend: opensandbox` in the env config; requires `SWERL_OPENSANDBOX_DOMAIN` + `OPEN_SANDBOX_API_KEY` and network egress (check with `scripts/opensandbox/check_opensandbox_egress.sh`; clean up leaked sandboxes with `scripts/opensandbox/cleanup_opensandbox_sandboxes.sh`). Also renames `docs/sandbox_modal_vs_podman.md` to `docs/sandbox_management.md` and rewrites it to document all four backends as implemented (https://github.com/allenai/open-instruct/pull/1887).
- Add `ModalBackend`, a `SandboxBackend` that runs RL sandboxes in Modal's cloud instead of on-node Podman/Docker. Select it with `backend: modal` in the env config; requires Modal credentials and network egress (check with `scripts/modal/check_modal_egress.sh`). See `docs/sandbox_management.md` for the trade-offs.
