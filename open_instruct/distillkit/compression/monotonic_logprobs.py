...
from open_instruct.distillkit.compression.bitpack import (
    pack_to_bytes,
    unpack_from_bytes,
)
...
def _get_quantize_scale_factors(
...
def quantize_naive(
    ...
    _error_buffer: torch.Tensor | None = None,
...
def dequantize(
    quantized_values: torch.LongTensor, scale: torch.Tensor, element_bits: int
) -> torch.Tensor:
    if element_bits == 1:
...
def compress_monotonic_logprobs(
    logprobs: torch.Tensor, config: DistributionQuantizationConfig
) -> torch.ByteTensor:
    if config.delta_encoding:
        deltas = (
            logprobs.to(_work_dtype(logprobs))[..., 1:]
            - logprobs.to(_work_dtype(logprobs))[..., :-1]
        )
    ...
    if config.polynomial_terms:
        ...
        coeff_bytes = (
            coeffs.to(config.term_dtype.dtype())
            .view(torch.uint8)
            .reshape(*logprobs.shape[:-1], -1)
        )
    ...
    for bin_ in config.residual_bins:
        ...
        scale, scaled, _ = quantize_naive(
            values, bin_.element_bits, bin_.scale_dtype.dtype(), None
        )
        scale_bytes = (
            scale.to(bin_.scale_dtype.dtype())
            .view(torch.uint8)
            .reshape(*logprobs.shape[:-1], -1)
        )
        chunks.append(scale_bytes)
        packed = pack_to_bytes(scaled, bin_.element_bits).reshape(
            *logprobs.shape[:-1], -1
        )
...
def decompress_monotonic_logprobs(
    ...
    if config.exact_k > 0:
        ...
        exact_values = exact_part.view(dtype=exact_dtype_torch).reshape(
            *bytes_.shape[:-1], config.exact_k
        )
        remaining_bytes = bytes_[..., exact_bytes:]
    else:
        exact_values = torch.empty(
            (*bytes_.shape[:-1], 0), dtype=out_dtype, device=device
        )
    ...
    if config.polynomial_terms:
        ...
        coeffs = coeff_part.view(dtype=config.term_dtype.dtype()).reshape(
    ...
    for bin_ in config.residual_bins:
        ...
        scale = scale_part.view(dtype=bin_.scale_dtype.dtype()).reshape(
    ...
    if residuals:
        ...
        if residual.shape[-1] < approx_terms:
            residual = torch.nn.functional.pad(
                residual, (0, approx_terms - residual.shape[-1])
            )
    else:
        residual = torch.zeros(
            (*bytes_.shape[:-1], approx_terms), dtype=out_dtype, device=device
        )
    ...
    if coeffs is not None and config.polynomial_terms:
        ...
        fit = torch.sum(
            x.to(coeffs.device, coeffs.dtype) * coeffs.unsqueeze(-2), dim=-1
        )
