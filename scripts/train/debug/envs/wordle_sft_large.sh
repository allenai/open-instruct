#!/bin/bash
# Wordle env with SFT model - larger scale training
#
# Settings:
# - 5 training steps
# - 64 unique prompts x 16 samples = 1024 rollouts per step
# - Context length: 4096
# - Total episodes: 5 * 1024 = 5120

# Install Prime Intellect Wordle environment
uv run --extra prime-intellect prime env install will/wordle 2>/dev/null || true

NCCL_P2P_DISABLE=1 NCCL_DEBUG=INFO VLLM_ALLOW_INSECURE_SERIALIZATION=1 uv run --extra prime-intellect open_instruct/grpo_fast.py \
