...
class IsHfCheckpointTest(unittest.TestCase):
    ...
    def test_local_dir_with_olmo_core_config_json_is_olmo_core(self) -> None:
        # An olmo-core checkpoint directory also carries a config.json -- the experiment
        # config -- so its presence alone must not mark the directory as HF. Getting this
        # wrong sends an olmo-core checkpoint down the HF loading path, where AutoConfig
