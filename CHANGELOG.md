...
### Added
- Add `--fixed_prompt_batches` to GRPO: with `async_steps>1`, form each training batch from exactly the prompt set queued for that step (early results of later batches are parked in the data-preparation actor, nothing is dropped for age) instead of the first responses to finish, so batch composition no longer follows response length while the generators keep working `async_steps` batches ahead. `PromptRequest`/`GenerationResult` carry a `batch_index`; logs `results_parked_for_later_batches`. Batch `t + async_steps` is queued when the trainer consumes batch `t`, bounding rollout staleness at `async_steps` versions like the default path (keying the refill off the data actor's own progress doubled the pipeline depth). Pure OPD with `async_steps>1` now warns unless this or `synchronous_rollouts` is set.
- Add `--synchronous_rollouts` to GRPO: the data-preparation actor holds each batch's prompts until the weight sync for the previous step has completed on every vLLM engine, so all rollouts are sampled from exactly the policy that step trains (strictly on-policy; requires `async_steps=1`, `inflight_updates=false`, no zero-std filtering/active sampling, ZeRO-3). Adds `qwen35_math_opd_sync_rerun.sh` and `make_math_patch_dataset.sh` for the on-policy reruns of the Qwen3.5 math OPD campaign.
