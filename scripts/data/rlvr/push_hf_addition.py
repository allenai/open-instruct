...
def push_to_huggingface(saved_files, hf_entity):
    """Push each addition dataset (e.g., `10x10_train_1000`, `2x2_test_100`) separately to Hugging Face."""
    api = HfApi()
    hf_entity = hf_entity or api.whoami()["name"]

    print("\n📤 Uploading selected addition datasets to Hugging Face...\n")
    ...
    for name, files in tqdm(saved_files.items(), desc="Uploading datasets"):
        ...
        repo_id = f"{hf_entity}/addition_{split}_{size}_{dimensions}"
...
def extract_dataset_info(filename):
    ...
    Example: "addition_1000_train_10x10.jsonl" → "10x10_train_1000"
    """
    match = re.match(r"addition_(\d+)_(train|test)_(\d+x\d+)\.jsonl", filename)
    if match:
...
def main():
    ...
    parser.add_argument("--input_dir", type=str, default="/weka/oe-adapt-default/nouhad/data/math_data/addition", help="Directory containing JSONL files")
    parser.add_argument("--hf_entity", type=str, default=None, help="Hugging Face entity")
    args = parser.parse_args()

    input_dir = args.input_dir
    train_files = ["addition_1000_train_7x8.jsonl"]
    test_files = []
    # test_files = ["addition_100_test_2x2.jsonl", "addition_100_test_6x6.jsonl", "addition_100_test_10x10.jsonl"]
