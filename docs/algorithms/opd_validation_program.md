...
Updated 2026-09-18 07:20Z (details in the latest Log entries).
...
- Step 2d (async fix, "fix not fence" per Kevin): implemented as `--fixed_prompt_batches` in
  ...
  dataset `01M2SNQYARKRCZPZ6AN8CEJPGX`, smoke `01M2SNTT1XKJZW2B7B2QZK1VCX` running (1 node, ~1h);
  full run (4 nodes, ~5h, no auto-resume, min runtime 8h) launches when the smoke passes.
- Overnight plan (Kevin asleep from 07:15Z; Claude monitors on a 30-min cadence): keep arm 2
  ...
  window dies without auto-resume); when the smoke passes launch the full fixed-batches run
  (`PATCH_DATASET=01M2SNQYARKRCZPZ6AN8CEJPGX RUN_MODE=full`); when arm 2 finishes collect
