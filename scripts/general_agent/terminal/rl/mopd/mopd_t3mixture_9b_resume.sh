...
EXP_NAME=swerl_mopd_t3mixture_9b_4node_64k

# Resume of the exhausted-retries run 01M01MRYK43NTC6KCF58PYQ197 (node fb171e50
# died: missing heartbeats; beaker retries reused the dying allocation). Fresh
# launch = fresh nodes; state resume via the original mason-auto state dir.
STATE_DIR=/weka/oe-adapt-default/allennlp/deletable_checkpoint_states/shashankg/1786761738_453633
RESUME_STEP=$(cat "$STATE_DIR/latest" 2>/dev/null || echo "none")

uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "MOPD Option-1 MIXTURE ablation RESUME: state resume at ${RESUME_STEP} after node death (4-node; 64k; 500 steps)" \
       --pure_docker_mode \
