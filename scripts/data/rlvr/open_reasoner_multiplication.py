...
def generate_large_number_multiplication_data(a_max, b_max, n_train, n_test):
    """Generate train and test datasets for multiplication of large-digit numbers."""
    datasets_dict = {}
    ...
    for a_digits in range(1, a_max + 1):
        for b_digits in range(1, b_max + 1):
            ...
            test_data = all_pairs[n_train+1: n_train+n_test+1]
...
def save_to_jsonl(data, file_path, split_name):
    ...
    with open(file_path, "w") as f, tqdm(total=len(data), desc=f"Saving {split_name} data") as pbar:
        for a, b in data:
            json.dump({"problem": f"What is {a} times {b}?", "answer": str(a * b), "split": split_name}, f)
            f.write("\n")
            pbar.update(1)

def prepare_datasets(output_dir):
    """Prepare train and test datasets ensuring each multiplication group is saved separately."""
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    all_datasets = generate_large_number_multiplication_data(a_max=15, b_max=15, n_train=1000, n_test=100)

    saved_files = {}
    for name, data in tqdm(all_datasets.items(), desc="Saving all datasets"):
        len_train = len(data["train"])
        len_test = len(data["test"])
        train_file = output_dir / f"multiplication_{len_train}_train_{name}.jsonl"
        test_file = output_dir / f"multiplication_{len_test}_test_{name}.jsonl"
...
def push_to_huggingface(saved_files, hf_entity):
    """Push each multiplication dataset (e.g., `9x5`, `3x4`) separately to Hugging Face."""
    api = HfApi()
    hf_entity = hf_entity or api.whoami()["name"]

    print("\n📤 Uploading each multiplication dataset separately to Hugging Face...\n")
    ...
    for name, files in tqdm(saved_files.items(), desc="Uploading datasets"):
        ...
        repo_id = f"{hf_entity}/multiplication_{name}"
