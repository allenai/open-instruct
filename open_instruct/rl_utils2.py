...
def pack_sequences(
    ...
    return PackedSequences(
        ...
        attention_masks=[torch.tensor(t) for t in attention_masks],
...
def test_pack_sequences_logits():
    ...
    s = model.forward(
        ...
        position_ids=reset_position_ids(packed_sequences.attention_masks[0].unsqueeze(0)),
    )
    ...
    all_logprobs = torch.nn.functional.log_softmax(s.logits[:, :-1, :], dim=-1)
