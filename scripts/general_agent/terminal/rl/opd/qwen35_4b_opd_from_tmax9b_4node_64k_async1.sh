...
# OPD arm A of the open-instruct-vs-slime comparison: identical to
# qwen35_4b_opd_from_tmax9b_4node_64k.sh (baseline run vjgol9zb, async_steps 4)
# EXCEPT --async_steps 1, so the vLLM student logprobs feeding the reverse-KL
# advantage come from a policy ~1 step stale instead of ~4 (slime is fully
# synchronous). Same image as the baseline (shashankg/open-instruct-opd), so the
# only diff is the flag. Twin cu13/holmes run: ..._async1_holmes_cu13.sh.
#
...
#
#   bash scripts/general_agent/terminal/rl/opd/qwen35_4b_opd_from_tmax9b_4node_64k_async1.sh shashankg/open-instruct-opd
...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "OPD arm A: async_steps 1 (baseline vjgol9zb used 4) -- base Qwen3.5-4B <- tmax-9b pure distill; jupiter/cu12; 4-node 64k" \
       --pure_docker_mode \
