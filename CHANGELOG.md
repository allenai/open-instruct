...
### Changed
- Refactor OLMo-core DPO metrics: defer token-weighted metric reduction to a `DPOMetricsCallback` (recording per-step numerators/denominators with `ReduceType.sum` so the trainer's batched per-interval all-reduce handles them, removing the explicit per-step `all_reduce` and host sync in `train_batch`), align wandb keys with `dpo_tune_cache.py` (`train_loss`, `logps/*`, `rewards/*`, `perf/mfu_step`, `perf/tokens_per_second_step`/`_total`), add `train/padding_fraction`, `train/sequences_per_rank`, and `train/global_sequences_per_step` metrics, and make `get_num_sequences` always return an `int` (https://github.com/allenai/open-instruct/pull/1719).
