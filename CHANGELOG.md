...
### Fixed
- `is_hf_checkpoint` now decides by config contents (a top-level `model_type`) instead of the mere presence of `config.json`, so olmo-core checkpoint directories — whose config.json is the experiment config — are no longer misrouted down the HF branch. mason also passes a `GOOGLE_APPLICATION_CREDENTIALS` secret to jobs so they can read `gs://` checkpoints on clusters without a GCE metadata server (https://github.com/allenai/open-instruct/pull/1840, fixes https://github.com/allenai/open-instruct/issues/1838).
