...
"""Helpers for the ``never_give_up`` (NGU) GRPO retry mechanism and grouped-advantage math.

NGU keeps retrying a prompt that came back with no learning signal (zero reward-std)
instead of dropping it: with probability ``never_give_up`` the same prompt is requeued
under a retry-suffixed id, forming a *chain*. Earlier attempts in the chain are buffered
here until an attempt finally has signal, at which point they are merged back in and the
group's advantage baseline is taken over the whole chain rather than the accepted batch.
"""
...
class NeverGiveUpAccumulationState:
    ...
    Every dict is keyed by chain id (the base ``epoch_index`` prompt id shared by all
    retries of one prompt, see :func:`get_never_give_up_chain_id`).
    """
...
class PendingNeverGiveUpState:
    """The buffered state for a single chain, popped out of :class:`NeverGiveUpAccumulationState`."""
...
def get_never_give_up_retry_suffix(prompt_id: str, epoch_number: int, index: int) -> str:
    """Return a numeric retry suffix (``_1``, ``_2``, ...) for a never_give_up requeue."""
...
def should_accept_never_give_up_batch(
    reward_scores: np.ndarray, pending_best_reward: float | None, filter_zero_std_samples: bool
) -> bool:
    """Decide whether to stop retrying a chain and train on this attempt's batch.

    Accept rule is always "better": once the chain has a previous best reward, a new
    attempt is only accepted when its max reward strictly beats that previous best.
    Before any previous best exists we fall back to the normal zero-std filter.
    """
    ...
    return float(reward_scores.max()) > pending_best_reward
...
def _apply_anchor_pos_advantage_rescaling(
    ...
    """Anchor the positive (max-reward) samples and rescale the rest to re-center each group.

    When the NGU baseline is taken over the whole chain (not just the accepted batch), a
    group no longer sums to zero. We keep the max-reward samples' advantages exactly as
    they are (they carry the learning signal we retried for) and scale the remaining
    negative-advantage samples so the group sums back to zero.
    """
...
def compute_grouped_advantages(
    ...
    With no baseline args this is the standard GRPO grouped advantage (``scores - group
    mean``, optionally std-normalized), supporting variable group sizes via
    ``prompt_sample_counts``.

    When ``prompt_baseline_*`` are supplied and ``filtered_ngu_baseline`` is True, the
    per-group mean is replaced by the chain-wide mean (``reward_sum / sample_count`` over
    every attempt in the NGU chain, not just this batch) and the resulting advantages are
    re-centered with :func:`_apply_anchor_pos_advantage_rescaling`. The NGU baseline is
    only meaningful when the batch is a strict subset of the chain; when all pending
    completions are kept it collapses to the standard grouped mean, so callers pass
    ``filtered_ngu_baseline=False`` to fall back to regular group advantages.
    """
    ...
    if have_baseline and filtered_ngu_baseline:
        ...
        mean_grouped_rewards = _expand_per_group_values(ngu_group_means, prompt_sample_counts, dtype=scores.dtype)
