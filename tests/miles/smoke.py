...
parser.add_argument("--model-type", choices=("qwen3", "olmo3"), default="qwen3")
...
if not hf.exists():
    ...
    core_models.save_hf_config(config, hf)
...
try:
    asyncio.run(train(args, export_hf=str(root / ("export-resumed" if options.resume else "export"))))
finally:
    ray.shutdown()
