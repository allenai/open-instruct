...
#   SERVE_PORT            HTTP port for vLLM (default: 8008). Deliberately not
#                        named VLLM_PORT: vLLM reserves that name as the BASE
#                        of its internal port range, so exporting it makes every
#                        data-parallel rank derive the same rendezvous port and
#                        die with EADDRINUSE.
...
# vLLM treats VLLM_PORT as the base of its internal port allocations (its own
# docs: "if VLLM_PORT is set ... the rest will be generated by incrementing").
# If one leaks in from the environment, every DP rank derives the same
# distributed-init port and startup dies with EADDRINUSE. We pass the HTTP port
# with --port instead, so make sure the variable is simply absent.
unset VLLM_PORT
...
log "waiting for vllm on :$SERVE_PORT (up to ${VLLM_READY_TIMEOUT}s)"
...
curl -s "http://localhost:$SERVE_PORT/v1/models" | head -c 600; echo
