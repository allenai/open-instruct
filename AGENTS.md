...
- For disposable runs that do not need checkpoint/resume testing or saved weights, set `training.save_checkpoints = false` and `output.export_hf = false`, and remove any explicit `save_interval`. Keep rollout capture enabled for analysis. Only enable checkpoint saving or HF export when the run has a specific need for those artifacts.
