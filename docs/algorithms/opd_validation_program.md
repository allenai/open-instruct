...
Updated 2026-09-18 07:35Z (details in the latest Log entries).
...
- Step 2d (async fix, "fix not fence" per Kevin): implemented as `--fixed_prompt_batches` in
  ...
  run `01M2SPXTBP04X0HD7FPDTKN3WZ` running since 07:33Z (4 nodes, ~5h, no auto-resume, min
  runtime 8h, timeout 12h; relaunch if it dies before step 100). The first full launch
  `01M2SPKXVMR0QT3FCZEYE5AKXD` died on a W&B tag-length cap before training; fixed.
