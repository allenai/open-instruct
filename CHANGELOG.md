...
### Fixed
- Make pad-token logprob handling explicit in `forward_for_logprobs`: pad positions are now clamped to a valid vocab index for the gather and then explicitly set to `INVALID_LOGPROB` (changed from `1.0` to `-1e9`) instead of being silently remapped to token 0. This is defense-in-depth — `response_mask` already excludes pad positions downstream — but removes the silent, fragile behavior where an unmasked pad token would produce a plausible-but-wrong logprob. Also make the ρ log-ratio clamp configurable via `rho_logprob_diff_clamp` (default 10.0) so user `rho_clamp_upper_bound` values above e^10 are honored (https://github.com/allenai/open-instruct/pull/1817).
