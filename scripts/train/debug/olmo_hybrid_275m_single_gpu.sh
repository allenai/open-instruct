...
# Single GPU GRPO (OLMo-core grpo.py) debug run for the OLMo-hybrid small suite
# (275M SFT checkpoint). Requires a Beaker image built from the transformers /
# vLLM / OLMo-core maintainer forks that implement the `olmo_hybrid_small`
# architecture (see pyproject.toml [tool.uv.sources]).
...
uv run python mason.py \
       ...
       --cluster ai2/ceres \
       --image "$BEAKER_IMAGE" \
       --description "Single GPU OLMo-hybrid 275M GRPO (OLMo-core) test script." \
       --pure_docker_mode \
