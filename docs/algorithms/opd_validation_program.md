...
Updated 2026-09-21 03:30Z (was 2026-09-21 02:55Z; details in the latest Log entries).
...
- **Status at 2026-09-21 02:15Z (wrapper code overlaid on image `01M2V7V946ZN9N9STPK0H04YWM`):**
  ...
  Qwen3-8B on MATH, 176 rollouts, paper target 67.8) is on rollout 21, `train/step` 87, LR
  2.90e-6, grad norms settled from 11 to ~2; ~4.5 min per rollout, first eval (rollout 43)
  ~04:15Z, finish ~2026-09-21 16:00Z plus one or two 8 h-window resumes. (3) **Arm 2 EOPD
  ...
  Qwen3-4B-Base (the OPD arm's base read 0.247, same weights). No train step logged yet at
  03:25Z (pre-eval of four sets at 8192 tokens plus the first rollout with teacher top-16
  scoring); first `train/*` rows expected shortly, at `lr-pg_0` 3.0e-6 with finite
  `eopd_fkl_loss` ≤ `eopd_fkl`. Both new arms run with
...
- **Paper-grader check approved and launched (02:54Z): `01M30Y52ZMSHZAJ5G2TGW1VYW2`** scores the arm 2
  OPD export `hf-219` the way Table 2 was scored (Qwen2.5-Math harness `a45202bd`: its
  `extract_answer` + `math_equal`, 3 s per comparison; App. C sampling T 1.0, top-p 0.8, 8192
  tokens, 8 samples) on all six benchmarks, so Minerva, OlympiadBench and Pass@8 get filled in and
  the grader term drops out of the comparison with the paper's 78.81 / 57.33 / 40.08 / 42.10 /
