...
def build_slurm_script(args: argparse.Namespace, commands: list[list[str]], experiment_dir: str) -> str:
    ...
    if args.nodes > 1:
        ...
        lines.append("export MASTER_HOSTNAME=$(hostname)")
        lines.append("# Get IPv4 address of current node (batch script runs on node 0)")
        lines.append("export MASTER_ADDR=$(hostname -I | awk '{print $1}')")
        ...
        lines.append("export BEAKER_LEADER_REPLICA_IP=$MASTER_ADDR  # Pre-resolved IP")
        lines.append('echo "Starting Ray cluster..."')
        lines.append(f"srun --nodes={args.nodes} --ntasks-per-node=1 --export=ALL bash -c 'export BEAKER_REPLICA_RANK=$SLURM_NODEID; bash configs/beaker_configs/ray_node_setup.sh' &")
