...
class GrpoIntegrationTests(TestGrpoFastBase):
    ...
    def test_kv_cache_specs_list_handling(self):
        ...
        # vLLM 0.9.1+ returns a list instead of dict
        mock_kv_cache_specs = [
            {
                "layer_0": {"block_size": 16, "num_heads": 32},
                "layer_1": {"block_size": 16, "num_heads": 32},
            }
        ]
        ...
        with unittest.mock.patch("torch.cuda.get_device_properties") as mock_cuda_props:
            ...
            with unittest.mock.patch("open_instruct.vllm_utils3.kv_cache_utils.get_uniform_page_size") as mock_page_size:
                mock_page_size.return_value = 16
                ...
                with unittest.mock.patch("open_instruct.vllm_utils3.kv_cache_utils.get_num_blocks") as mock_num_blocks:
                    ...
                    kv_cache_config = LLMRayActor.get_kv_cache_info(actor)
                    
                    # Should return a valid KVCacheConfig
                    self.assertIsNotNone(kv_cache_config)
                    # Verify it's the correct type
                    from vllm.v1 import kv_cache_interface
                    self.assertIsInstance(kv_cache_config, kv_cache_interface.KVCacheConfig)
