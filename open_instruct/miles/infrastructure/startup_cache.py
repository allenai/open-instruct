...
def worker_runtime_env(args, slot, env_vars):
    ...
    hook = "open_instruct.miles.infrastructure.startup_cache.setup_worker"
    runtime_env = {
...
def _serving_command(spec, context):
    ...
    command[module_index] = "open_instruct.miles.rollout.serving"
...
def setup_worker():
    ...
    if policy["observe"]:
        ...
        (hook / "sitecustomize.py").write_text(
            "import os\nfrom pathlib import Path\n"
            "from open_instruct.miles.infrastructure.startup_cache import observe_triton\n"
