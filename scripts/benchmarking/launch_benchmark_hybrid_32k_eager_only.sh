...
echo "Launching: response_length=$response_length, enforce-eager"

uv run python mason.py \
    --cluster ai2/saturn \
    --non_resumable \
    --image "$image_name" \
    --description "Hybrid benchmark 32k eager fix: response_length=$response_length, enforce-eager, commit=$git_hash, branch=$git_branch" \
    --pure_docker_mode \
...
echo "Launched: response_length=$response_length, enforce-eager"
