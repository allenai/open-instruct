...
### Fixed
- Make the numpy SFT cache-directory key independent of the lazily populated tracking-only `tokenizer_files_hash`, so identical tokenization arguments resolve to the same directory regardless of tokenizer access order (https://github.com/allenai/open-instruct/pull/1826).
