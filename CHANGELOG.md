...
### Fixed
- `--chat_template_name` no longer silently falls back to the tokenizer's own template on an unrecognised name. Use a `CHAT_TEMPLATES` key or the `tokenizer_default` sentinel; unknown names raise with the available keys and a close-match hint. `dataset_statistics.json` now records the resolved source and hash (https://github.com/allenai/open-instruct/issues/1805).
