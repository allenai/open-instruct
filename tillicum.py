...
def build_slurm_script(args: argparse.Namespace, commands: list[list[str]], experiment_dir: str) -> str:
    ...
    if args.nodes > 1:
        ...
        lines.append("export MASTER_HOSTNAME=$(scontrol show hostnames $SLURM_JOB_NODELIST | head -n 1)")
        lines.append("# Resolve hostname to IPv4 address for Ray")
        lines.append("# Use getent ahostsv4 to get only IPv4, take first result")
        lines.append("export MASTER_ADDR=$(getent ahostsv4 $MASTER_HOSTNAME | head -1 | awk '{print $1}')")
        lines.append('if [ -z "$MASTER_ADDR" ]; then')
        lines.append('    echo "Warning: Could not resolve $MASTER_HOSTNAME to IPv4, using hostname"')
        lines.append('    export MASTER_ADDR=$MASTER_HOSTNAME')
        lines.append('fi')
        lines.append('echo "Master: $MASTER_HOSTNAME -> $MASTER_ADDR"')
        ...
        lines.append("export RAY_PORT=8888  # Must match port in ray_node_setup.sh")
        ...
        lines.append("# Map Slurm env vars to Beaker env vars so we can reuse the same script")
        lines.append("export BEAKER_LEADER_REPLICA_HOSTNAME=$MASTER_HOSTNAME")
        lines.append('echo "Starting Ray cluster..."')
        lines.append(f"srun --nodes={args.nodes} --ntasks-per-node=1 bash -c 'export BEAKER_REPLICA_RANK=$SLURM_NODEID; bash configs/beaker_configs/ray_node_setup.sh' &")
        lines.append("RAY_SETUP_PID=$!")
        lines.append("")
        lines.append("# Wait for Ray cluster to initialize (up to 5 minutes)")
        lines.append('echo "Waiting for Ray cluster to be ready..."')
        lines.append("RAY_WAIT_START=$(date +%s)")
        lines.append("RAY_TIMEOUT=300  # 5 minutes")
        lines.append(f"EXPECTED_NODES={args.nodes}")
        lines.append("while true; do")
        lines.append("    # Check ray status - count nodes by looking for 'CPU' resource lines per node")
        lines.append("    # ray status output shows 'N node(s)' in the summary")
        lines.append("    RAY_STATUS=$(ray status --address=$RAY_ADDRESS 2>&1 || true)")
        lines.append("    # Look for 'N node' pattern in status output")
        lines.append("    NODE_COUNT=$(echo \"$RAY_STATUS\" | grep -oE '[0-9]+ node' | head -1 | grep -oE '[0-9]+' || echo 0)")
        lines.append("    # Fallback: count lines with IP:port pattern if the above fails")
        lines.append("    if [ \"$NODE_COUNT\" = \"0\" ]; then")
        lines.append("        NODE_COUNT=$(echo \"$RAY_STATUS\" | grep -cE '[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+:[0-9]+' || echo 0)")
        lines.append("    fi")
        lines.append("    if [ \"$NODE_COUNT\" -ge \"$EXPECTED_NODES\" ]; then")
        lines.append('        echo "Ray cluster is ready with $NODE_COUNT nodes!"')
        lines.append("        break")
        lines.append("    fi")
        lines.append("    RAY_ELAPSED=$(($(date +%s) - RAY_WAIT_START))")
        lines.append("    if [ $RAY_ELAPSED -ge $RAY_TIMEOUT ]; then")
        lines.append('        echo "ERROR: Ray cluster failed to start within ${RAY_TIMEOUT}s"')
        lines.append('        echo "Expected $EXPECTED_NODES nodes, got $NODE_COUNT"')
        lines.append('        echo "Ray status output:"')
        lines.append('        echo "$RAY_STATUS"')
        lines.append("        exit 1")
        lines.append("    fi")
        lines.append('    echo "Waiting for Ray... (${RAY_ELAPSED}s, $NODE_COUNT/$EXPECTED_NODES nodes)"')
        lines.append("    sleep 10")
        lines.append("done")
        lines.append('echo "Ray cluster status:"')
        lines.append("ray status --address=$RAY_ADDRESS")
