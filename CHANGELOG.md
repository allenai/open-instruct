...
### Added
- Remove `vllm_num_engines` from `VLLMConfig`; it is now auto-computed from `ray.cluster_resources()` after `ray.init()` so users no longer need to pass `--vllm_num_engines` (https://github.com/allenai/open-instruct/pull/1482).
