...
# Requires a Beaker image built from the transformers / OLMo-core maintainer forks
# that implement the `olmo_hybrid_small` architecture (see pyproject.toml
# [tool.uv.sources]).
# Points at the HF-converted checkpoint (-hf suffix); dpo.py deserializes the
