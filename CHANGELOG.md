...
### Fixed
- `--ephemeral_save_interval` now accepts a non-positive value to disable ephemeral checkpoints entirely. Previously `olmo_core_finetune.py` forced a default of 500 through `parser.set_defaults`, so omitting the flag yielded 500 rather than `None` and there was no way to turn them off; 500 also fails olmo-core's `'ephemeral_save_interval' must be less than 'save_interval'` check whenever `--checkpointing_steps` is smaller. Deleting an ephemeral checkpoint can overrun the bookkeeping soft timeout and kill the job with `RuntimeError: Application timeout caused pair closure` (https://github.com/allenai/open-instruct/pull/1810).
