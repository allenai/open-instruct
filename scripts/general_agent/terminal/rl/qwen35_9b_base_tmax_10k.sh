...
# RL on Qwen/Qwen3.5-9B + hamishivi/swerl-tmax-15k
# 4 nodes x 8 GPUs (32 GPUs total)
#
# 9B sibling of qwen35_4b_base_tmax_10k.sh. Same GRPO objective + sequence
# lengths, but sized for the larger model on 4 nodes:
...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "SWERL tmax-15k GRPO with Qwen3.5-9B" \
       --pure_docker_mode \
    ...
    --output_dir /output \
    --exp_name swerl_qwen35_9b_base_tmax_15k_verified_grpo \
    --local_eval_every 10 \
    --save_freq 20 \
