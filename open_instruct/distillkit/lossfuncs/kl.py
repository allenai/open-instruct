...
def sparse_kl_div_inner(
    ...
    inner_sum = torch.sum(teacher_sparse_probs * (sparse_target_logprobs - sparse_student_logprobs), dim=-1)
    ...
    if missing == MissingProbabilityHandling.SYMMETRIC_UNIFORM:
        ...
        missing_kl = torch.exp(log_teacher_missing) * (log_teacher_missing - log_student_missing)
    else:
        missing_kl = None
