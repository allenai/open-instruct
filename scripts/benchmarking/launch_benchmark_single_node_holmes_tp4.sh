...
for model_name_or_path in "$@"; do
    ...
    uv run python mason.py \
        --cluster ai2/holmes \
	      --non_resumable \
        --image "$image_name" \
        --description "Running Holmes single node TP=4 benchmark with response length of $response_length at commit $git_hash on branch $git_branch with model $model_name_or_path." \
        --pure_docker_mode \
        ...
        --task_name open_instruct-benchmark_generators-holmes-tp4 -- source configs/beaker_configs/ray_node_setup.sh \&\& HF_DATASETS_CACHE=/tmp/open_instruct_hf_datasets_cache_${BEAKER_JOB_ID} python -m open_instruct.benchmark_generators \
