...
# OPD H1b: base 4B student <- RAW 27B teacher (shatu/Qwen3.5-27B).
#
# Extends the raw-teacher test to a 6.75x capacity gap, because the raw-9B
# teacher (H1a) has little headroom over the base-4B student (TB2.1 0.193 vs
# 0.135) and may not be revealing. Two contrasts:
#   * vs qwen35_4b_opd_from_raw9b_4node_64k.sh  — raw teacher SIZE (2.25x vs 6.75x)
#   * vs tmax4b_opd_from_tmax27b_4node_64k.sh   — raw vs RLVRed 27B teacher at the
#     same 6.75x gap (the RL'd one produced the deep destructive trough; different
#     student though: base-4B here vs tmax-4b there)
#
# Teacher facts verified 2026-08-19: Qwen3_5ForConditionalGeneration, 64 layers
# (mixed attention geometry -> full-sequence teacher scoring under SP, same
# proven path as tmax-27b), tokenizer.json byte-identical to the student.
# ZeRO-3 shards the ~27B teacher across 16 learners (~3.4 GB/GPU, proven).
#
# Launch with the open-instruct-opd-maskreason image to get the reasoning/action
# KL decomposition metrics (masking itself stays OFF for this run).
#
...
TEACHER_MODEL=shatu/Qwen3.5-27B

EXP_NAME=swerl_qwen35_4b_opd_from_raw27b_4node_64k
...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "OPD H1b: base Qwen3.5-4B student <- RAW shatu/Qwen3.5-27B teacher (raw teacher at 6.75x gap; 4-node; 64k)${RESUME_NOTE}" \
       --pure_docker_mode \
       --workspace ai2/general-tool-use \
       --priority urgent \
       --preemptible \
