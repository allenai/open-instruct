...
# get, which keeps the token-mean gradient from being dominated by the
# previously-truncated tail without needing the overlong mask. Truncated /
# turn-capped rollouts now contribute negative gradient on long failures.
...
uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "SWERL tmax-10k GRPO with Qwen3.5-9B + turn cap 35 (no mask_truncated)" \
       --pure_docker_mode \
    ...
    --output_dir /output \
    --exp_name swerl_qwen35_9b_base_tmax_10k_grpo_turncap35 \
    --local_eval_every 10 \
    --save_freq 20 \
