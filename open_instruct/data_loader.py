...
class StreamingDataLoaderMonarch(data_loader.DataLoaderBase):
    """Monarch-compatible streaming dataloader that pulls pre-prepared data from DataPreparationMonarchActor."""
    ...
    def __init__(
        ...
        *,
        ...
        self.data_prep_actor = data_prep_actor
        self.tokenizer = tokenizer
        self.num_training_steps = num_training_steps
        self.training_step = 0
        self.current_epoch = 0
    ...
    def _iter_batches(self) -> Iterable[dict[str, Any]]:
        ...
        for step in range(self.training_step, self.num_training_steps):
            batch_data = loop.run_until_complete(self.data_prep_actor.get_data.call(rank=self.dp_rank, step=step))
            self.training_step = step + 1
            yield batch_data
