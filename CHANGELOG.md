...
### Fixed
- Publish wandb metrics from a background thread (`AsyncWandbLogger`) so a wedged wandb service can no longer block the GRPO training loop inside `wandb.log` — observed stalling long runs for 3–12 hours with all GPUs idle. If the wandb SDK stops draining, metrics are dropped with a warning instead of pausing training (https://github.com/allenai/open-instruct/pull/1841).
