...
> Superseded implementation history, archived September 13, 2026. Pending-work and support claims below reflect earlier development, not the current release. Use the [current guide](../../index.md) and [support matrix](../../feature-parity.md).
...
`~/proj/olmo-miles`** are recovery operations, checkpoint policies, broader
workflow qualification, and trainer topology/batching support. Shared MILES or
...
The new [researcher workflow](../../workflow.md) adds structured run files,
...
[Workflow evidence](../researcher-workflow-20260911.md) records
the exact qualified path; conversion/save/export/restart remain separate.

Use the [detailed audit](../feature-parity-audit-20260911.md),
[run-control guide](../../run-controls.md), and
[complete knob inventory](../knob-inventory.md) for option mappings and limits.
...
and engine scaling. Start with the structured [examples](../../../../configs/miles/examples);
the following low-level `[core]`/`[miles]` profiles remain available. Neither format
uses implicit inheritance or environment-variable expansion.
...
async recipe. Both full-SFT starters restore 8 prompts × 8 responses per
...
`validate` checks the installed MILES parser for low-level files, or the CPU-safe
structured schema for researcher run files. Native validation runs before training;
neither proves model memory fit. Beaker submission uses the committed
`./scripts/train/build_image_and_launch.sh --miles` workflow. Allocation,
mounts and automatic resubmission are explicit `[launch]` controls in structured
run files; use `run` to submit and `status` to inspect the Beaker attempt.
...
Both [local tiny-model trials](../core-datasources-local-20260910.json)
and [full-SFT EP2 trials](../core-datasources-sft-20260910.json)
...
A separate [GSM8K/math/IF mixture](../mixture-20260910.json)
...
| Serving admission and graphs | Actual 64 concurrent requests, decode graphs through 64, 524288 KV slots and 128 recurrent slots passed the larger B300 trial without observed OOM/retractions. [Evidence](../admission64-20260911.json) | Prefix caching remains disabled in profiles; cache/replay/publication invalidation and full-model colocated graphs need their own acceptance. |
| Compiler caches | Default-on fingerprinted per-worker Triton restore/publication with WEKA `tmp-30d` retention. Full-SFT EP2/TP1 reuse and local automatic miss/reuse/changed-setting gates passed. [Evidence](../startup-full-sft-20260911.json) | Multi-node and TP>1 serving remain unqualified. Broader compiler families are not covered; cache publication still costs minutes on WEKA. [Cache guide](../../compiler-cache.md) |
| Native checkpoint/resume | Schema-2 architecture/topology checks, optimizer/scheduler/RNG/cursor, completion marker. Full-model fast save/read gate passed exact restored state and two subsequent fixed-input updates/HF exports. [Evidence](../checkpoint-fast-audit-20260911.json) | Synchronous saves, same topology. Background writes, retention and token-per-expert cadence absent. Process writer unqualified at full model. Exact continuation used separate persistent per-rank caches; it does not promise identical future sampled rollouts. |
...
- **Dynamic rows:** identical full-model scoring across 154,531 logprobs;
  ...
  kernels can still compile. [Measurement](../control-exercise-20260911.md)
- **Checkpoints:** qualified direct save fell from 437.128 to 118.466 seconds;
  ...
  Ray startup/restart measurement. [Record](../checkpoint-perf-20260911.md)
- **Caches:** the full-SFT EP2/TP1 cold/restored trial reduced driver-entry to
  ...
  persistence is now opt-out. [Evidence](../startup-full-sft-20260911.json)
...
The [qualification plan](../../plans/qualification-plan.md) gives acceptance principles;
