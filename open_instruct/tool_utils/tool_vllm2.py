...
class ToolUseLLM(LLM):
    ...
    def _run_engine(self, *, use_tqdm: bool) -> list[Union[RequestOutput, PoolingRequestOutput]]:
        ...
        while True:
            ...
            dict_keys_to_delete = []
            for req_id, (future, prompt, last_text, last_token_ids) in self.pending_tool_futures.items():
                if future.done():
                    ...
                    if isinstance(prompt, str):
                        ...
                        # TODO: need to handle the edge case ValueError: Prompt length of 1999 is longer than the maximum model length of 1024.
                        ...
                        for o in output.outputs:
                            ...
                            for stop_str in self.sampling_params.stop:
                                if o.text.endswith(stop_str) and stop_str in self.tools:
                                    ...
                                    prompt = output.prompt if output.prompt is not None else output.prompt_token_ids
                                    self.pending_tool_futures[output.request_id] = (
                                        future,
                                        prompt,
                                        o.text,
                                        o.token_ids,
