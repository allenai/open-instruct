...
# becomes the advantage (--opd_pure), so the verifier reward is logged but
# carries no gradient. Drop --opd_pure to ADD the distillation term to the DPPO
# reward advantage instead (then consider restoring --active_sampling).
#
# vs the repro script: --active_sampling removed and --filter_zero_std_samples
# false, because reward-variance filtering would discard groups pure OPD can
# still learn from. Watch objective/opd_reverse_kl in wandb — it should fall.
...
EXP_NAME=swerl_qwen35_9b_dppo_opd_v2_4node_64k

uv run python mason.py \
       --cluster ai2/jupiter \
       --image "$BEAKER_IMAGE" \
       --description "tmax-15k OPD Qwen35 9b v2 (tiled teacher logprobs image fix; pure distill from allenai/tmax-9b; 4-node; 64k)" \
       --pure_docker_mode \
