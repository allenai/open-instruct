...
def specification(image, core_root, megatron_root, *, hf_only=False, evidence_only=False, reference_root=None):
    ...
    pairs = {"comparison": (roots["core"], roots["megatron"])}
    if reference_root is not None:
        reference = Path(reference_root)
        if (
            not hf_only
            or not reference.is_relative_to(ROOT)
            or ".." in reference.parts
            or reference.name not in BACKEND_NAMES
        ):
            raise ValueError("Reference suite requires an HF-only campaign backend directory")
        pairs = {
            "pinned-twins": (roots["core"], roots["megatron"]),
            "reference-vs-left": (reference, roots["core"]),
            "reference-vs-right": (reference, roots["megatron"]),
        }
    ...
    provenance = {
        ...
        "comparison_pairs": {name: {"left": str(left), "right": str(right)} for name, (left, right) in pairs.items()},
    }
    ...
    return {
        "version": "v2",
        "description": "Read-only fixed-prefix update-zero tensor comparisons with explicit run provenance",
        "tasks": [
            {
...
def main():
    ...
    parser.add_argument("--reference-root", help="HF-only suite: compare twins, then reference against each twin")
