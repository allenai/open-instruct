...
### Changed
- Upgrade the Linux stack to CUDA 13.0 for B300 support on the new `ai2/holmes` cluster: `nvidia/cuda:13.0.3` base image, cu130 torch/torchvision/torchaudio/flash-attn wheels, vLLM `0.19.1+cu130` from the vLLM wheel index, a `b300` entry in `GPU_SPECS`, `ai2/holmes` in the cluster lists, and regenerated generation goldens; requires NVIDIA driver >= 580 (https://github.com/allenai/open-instruct/pull/1758).
