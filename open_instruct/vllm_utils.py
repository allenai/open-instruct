...
class CompletionOutput:
    ...
    """Environment state from rollout (if using environments)."""
...
def process_completed_request(request_id, outs, current_time, use_tools, request_metadata):
    ...
    has_env_states = any(es is not None for es in env_states)
    ...
    result = GenerationResult(
        ...
        logprobs=logprobs,
        env_states=env_states if has_env_states else None,
    )
...
class LLMRayActor:
    """Ray actor for LLM generation with optional tool and environment support."""
    ...
    def __init__(
        ...
        *args,
        ...
        train_dataset=None,
        eval_dataset=None,
        env_pool_config: dict | None = None,
        **kwargs,
    ):
    ...
    def _init_config(
        ...
        self.env_pool_config = env_pool_config
        self.env_pools: dict[str, EnvironmentPool] = {}  # Lazy-initialized pools by env_name/env_class
...
async def _get_or_create_env_pool(actor: LLMRayActor, env_name: str | None, env_class: str | None) -> EnvironmentPool:
    """Get or create an environment pool for the given env name/class."""
    pool_key = env_name or env_class
    if pool_key not in actor.env_pools:
        pool_size = actor.env_pool_config.get("pool_size", 64) if actor.env_pool_config else 64
...
async def _process_env_request(
    ...
    # Get or create environment pool
    pool = await _get_or_create_env_pool(actor, env_config.env_name, env_config.env_class)

    # Acquire environment actor
    ...
    try:
        # Reset environment
        reset_result = ray.get(env_actor.reset.remote(task_id=env_config.task_id))
        env_tools = reset_result.tools
        env_tool_names = {t["function"]["name"] for t in env_tools if "function" in t}

        # Add initial observation to prompt if present
        if reset_result.observation:
            obs_tokens = actor.llm_engine.tokenizer.encode(
                f"\n{reset_result.observation}\n", add_special_tokens=False
            )
            current_prompt.extend(obs_tokens)
            response_tokens.extend(obs_tokens)
            response_logprobs.extend([0.0] * len(obs_tokens))
            response_masks.extend([0] * len(obs_tokens))  # Mask env observations

        max_steps = env_config.max_steps or 50

        while env_state.step_count < max_steps and not env_state.done:
            ...
            # Check for tool calls (environment actions)
            if actor.tool_parser is None:
                # No parser, can't extract tool calls
                env_state.done = True
                break

            tool_calls = actor.tool_parser.get_tool_calls(output.text)
            # Filter to only environment tools
            tool_calls = [tc for tc in tool_calls if tc.name in env_tool_names]

            if not tool_calls:
                # No tool call - treat as episode end with reward=0
                env_state.done = True
                break

            # Execute tool calls as environment steps
            for tool_call in tool_calls:
                ...
                try:
                    # Convert to environment ToolCall format
                    ...
                    env_state.info.update(step_result.info)
                    ...
                    if step_result.observation:
                        ...
                        response_tokens.extend(obs_tokens)
                        response_logprobs.extend(obs_logprobs)
                        response_masks.extend(obs_masks)
                        current_prompt.extend(obs_tokens)
                    ...
                    # Error in environment step - continue with error observation
                    error_msg = f"Environment error: {e}"
                    logger.warning(f"Environment step failed: {error_msg}")
                    ...
                    # Add error message as observation
                    error_tokens = actor.llm_engine.tokenizer.encode(
                        f"\nError: {error_msg}\n", add_special_tokens=False
                    )
                    response_tokens.extend(error_tokens)
                    response_logprobs.extend([0.0] * len(error_tokens))
                    response_masks.extend([0] * len(error_tokens))
                    current_prompt.extend(error_tokens)
    ...
    return CompletionOutput(
        ...
        mask=response_masks,
        num_calls=env_state.step_count,
        timeout=False,
        tool_error="",
        tool_output="",
        tool_runtime=sum(s.runtime for s in tool_call_stats),
        tool_called=env_state.step_count > 0,
        tool_call_stats=tool_call_stats,
        excess_tool_calls={},
        env_state=env_state,
    )
...
async def process_request(actor: LLMRayActor, sub_request_id: str, sampling_params: SamplingConfig):
    ...
    # If env_config is set, use environment rollout
    ...
    for i in range(num_engines):
        ...
        vllm_engines.append(
            ...
            .remote(
                ...
                env_pool_config=env_pool_config,
            )
        )
