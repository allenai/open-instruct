...
class FlatArguments:
    ...
    dataset_transform_fn: list[str] = field(
        default_factory=lambda: ["distill_pretokenized_v1", "distill_pretokenized_filter_v1"]
    )
    dataset_target_columns: list[str] = field(default_factory=lambda: TOKENIZED_DISTILL_DATASET_KEYS)
    ...
    # Optional GCS root for checkpoint sync (matches `finetune.py`; `launch_ai2_evals_on_weka` does not take this).
...
def _create_scheduler(args: FlatArguments, optimizer, num_training_steps: int):
    ...
    adjusted_num_training_steps: int | float = num_training_steps
    if args.final_lr_ratio is not None and args.lr_scheduler_type == "linear":
        adjusted_num_training_steps = (num_training_steps - args.final_lr_ratio * num_warmup_steps) / (
            1 - args.final_lr_ratio
        )
    return get_scheduler(
        name=args.lr_scheduler_type,
        optimizer=optimizer,
        num_training_steps=int(adjusted_num_training_steps),
        num_warmup_steps=num_warmup_steps,
    )


def main(args: FlatArguments, tc: TokenizerConfig):
    additional_model_arguments: dict = (
        args.additional_model_arguments if isinstance(args.additional_model_arguments, dict) else {}
    )
    ...
    if args.with_tracking:
        ...
        if accelerator.is_main_process and is_beaker_job() and beaker_config is not None:
    ...
    # Pre-download model files on main process to avoid race conditions when
    # multiple ranks on a shared filesystem access the HF cache concurrently.
    ...
    with deepspeed.zero.GatheredParameters(embeddings.weight, modifier_rank=None):
        embedding_size = int(model.config.vocab_size)
    ...
    if args.loss_functions is None:
        raise ValueError('loss_functions is required, e.g. \'[{"function": "cross_entropy", "weight": 1.0}]\'')
    loss_functions_config = (
        json.loads(args.loss_functions) if isinstance(args.loss_functions, str) else args.loss_functions
    )
    ...
    if args.compression_config is not None:
        ...
        if not isinstance(compressor_config, dict):
            raise ValueError(f"compression_config must parse to a dict, got {type(compressor_config).__name__}")
    elif args.logprob_compressor_config is None:
        distill_losses_requiring_signal = {"kl", "jsd", "tvd", "hinge", "logistic_ranking"}
        needs_compressor = any(cfg.get("function") in distill_losses_requiring_signal for cfg in loss_functions_config)
        if needs_compressor:
            raise ValueError("logprob_compressor_config is required for distillation losses.")
        compressor_config = {}
    ...
    distill_loss_computer = DistillationLossComputer(
        loss_functions=loss_functions_config, compressor_config=compressor_config, vocab_size=model.config.vocab_size
    )
    ...
    # Build scheduler after wrapping to avoid mismatch with distributed optimizer groups
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for batch in active_dataloader:
            ...
            with accelerator.accumulate(model):
                model_inputs = {
                    k: v for k, v in batch.items() if k in {"input_ids", "attention_mask", "labels", "position_ids"}
                }
                outputs = model(**model_inputs, use_cache=False)
                loss, loss_dict = distill_loss_computer.compute_loss(
                    student_logits=outputs.logits, model_loss=outputs.loss, labels=batch["labels"], batch=batch
                )
                del outputs
                total_loss += loss.detach().float()
                for key, value in loss_dict.items():
                    if key != "total_loss":
                        total_loss_components[key] = total_loss_components.get(key, 0.0) + value
            ...
            if accelerator.sync_gradients:
                ...
                if args.logging_steps and completed_steps % args.logging_steps == 0:
                    ...
                    total_fwd_passes = (
                        accelerator.gather(torch.tensor(fwd_passes_since_last_log, device=accelerator.device))
                        .sum()
                        .item()
                    )
                    ...
                    for key, value in total_loss_components.items():
                        metrics_to_log[f"train_{key}"] = value / fwd_passes_since_last_log
                    if args.with_tracking:
                        ...
                        scalar_metrics = {
                            ...
                            if k.startswith("train_") or k in {"learning_rate", "per_device_tps"}
                        }
                        metric_str = ", ".join(
    ...
    if (
        ...
        and args.hf_repo_revision is not None
    ):
        launch_ai2_evals_on_weka(
...
if __name__ == "__main__":
    ...
    parser = ArgumentParserPlus(cast(Any, (FlatArguments, TokenizerConfig)))
    args_dc, tc_dc = parser.parse_args_into_dataclasses()
    args = cast(FlatArguments, args_dc)
    tc = cast(TokenizerConfig, tc_dc)
    main(args, tc)
