...
def result_is_stale(model_step: int | None, training_step: int | None, max_off_policy_steps: int | None) -> bool:
    ...
    """
    if max_off_policy_steps is None or training_step is None or model_step is None:
        return False
    return training_step - model_step > max_off_policy_steps
...
def accumulate_inference_batches(
    ...
    if max_off_policy_steps is not None and not replenish_prompts:
        ...
        raise ValueError("max_off_policy_steps requires replenish_prompts=True to avoid draining the prompt pipeline.")
    ...
    while num_prompts_sampled < num_prompts:
        ...
        if result_is_stale(result.model_step, training_step, max_off_policy_steps):
            stale_results_dropped += 1
            logger.warning(
                "[accumulate_inference_batches] Dropping stale result for index=%s at training_step=%s: "
                "model_step=%s lag=%s max_off_policy_steps=%s",
                result.index,
                training_step,
                result.model_step,
    ...
    if stale_results_dropped > 0:
        logger.info(
            ...
            f"stale result(s) this batch (max_off_policy_steps={max_off_policy_steps})"
        )
