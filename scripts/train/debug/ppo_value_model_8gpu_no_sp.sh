...
# Uses 8 GPUs (single node) WITHOUT sequence parallelism for simpler testing
exp_name=ppo_value_model_8gpu_no_sp_test
...
uv run python mason.py \
    ...
    --preemptible \
    --num_nodes 1 \
    --description "PPO with value model test (8 GPU, no SP)" \
    --timeout 1h \
    --max_retries 0 \
